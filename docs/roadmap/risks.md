...
the record. Mitigations are planned requirements unless their backlog item is
done.
...
| R-03 | Live-provider keys leak or overspend | Low | High | Provider spend alert; unexpected job triggers | Environment-scoped secrets with required reviewer; enforced cap or fixed prepaid balance where available, otherwise minimum permissions/quotas plus alert-triggered disablement and a recorded overshoot bound; OIDC for AWS/GCP; cheapest models only | maintainer |
...
| R-06 | Budget semantics misunderstood (unpriced attempts accrue 0) | Medium | Medium | Support questions about "budget not enforced" | Publish the semantics sentence in README, CHANGELOG, the key detail view, and the 429 body; expose `unpriced_attempts` per key | maintainer |
...
| R-08 | GHCR anonymous pull limits hurt the quick start | Low | Medium | Users report pull failures | Make and keep the package public; document a Docker Hub mirror as a follow-up if it bites | maintainer |
...
| R-10 | Tracing accidentally records content | Low | High | Allowlist test fails; e2e "secret prompt" assertion fails | Enforce an attribute allowlist in a unit test and e2e collector assertion; review every new `span.record` call | maintainer |
