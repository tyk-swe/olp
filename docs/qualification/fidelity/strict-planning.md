...
| Native complete-history reasoning/function-tool interactions | Native data is retained. This path uses the native client history contract, without mandatory OLP persistence or helper. |
| Negotiated translated continuation | The `chat-anthropic-tools-v1` contract admits the qualified Chat→Anthropic tool workflow behind an encrypted continuation handle with an owner-scoped submission identity; see [continuation](continuation.md). Unqualified versions, surfaces or missing carriers fail `state_carrier`. |
| Provider-retained state, background work and native resource references | Authorized retained Responses and `strict_file`/`strict_batch` resources commit their complete native dependencies under the resource authority; see [continuation](continuation.md) and [durable lifecycle](durable-lifecycle.md). Unauthorized retention fails `policy_conflict`; unresolved references fail `resource_affinity`; unqualified retained surfaces fail `state_carrier`. |
| Provider-hosted tools and unqualified effects | Fail `state_carrier` before dispatch; no lifecycle or authority contract is admitted for them. Ordinary native function tools remain supported. |
| Management console playground projection | Still fails `state_carrier` on strict routes before dispatch. The console instead exposes the negotiated continuation carrier and registered public `/native/{dialect}/models/{route}` operations; see [console interaction](console-interaction.md). Legacy/transformed playground behavior remains available. |
| Other operation runners | Registered unary operations compile through `internal/operationplan` and `internal/operationregistry` ([unary operations](unary-operations.md), [Cohere v2](cohere-native-v2.md)); media, durable batch, realtime and Gemini lifecycle compile their own contracts ([media](media-strict.md), [audio translation](audio-translation.md), [durable](durable-lifecycle.md), [realtime](strict-realtime.md), [Gemini lifecycle](gemini-lifecycle-v1beta.md)). Operations without an admitted contract fail `target_capability`. |
