...
The registered-profile public semantic smoke and six runner mutation tests passed before timed capture. The v1 source and runner hashes exactly match their frozen artifact. The v2 native baseline, budgets and replacement comparisons are pending a coordinated quiet host window; host load from unrelated Rust builds was above 30 during source validation. No G6 pass claim follows from source existence or smoke timing.
