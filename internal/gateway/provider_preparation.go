...
func (x *execution) preparedProvider(provider *runtime.Provider, model string) (preparedProvider, error) {
	...
	if x.strict() {
		...
		binding := interaction.Context{Headers: x.semanticHeaders, Query: x.semanticQuery, AllowProviderState: x.authority.Policy.AllowProviderState, AllowHostedTools: x.authority.Policy.AllowHostedTools, RequiredServing: x.serving, RetainedResponses: x.providerState}
