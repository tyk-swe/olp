...
func TestStreamLifetimeCapClassifiesAsTimeExhaustion(t *testing.T) {
	...
	st.reason.Store(3)
	if class := st.classify(io.EOF, true); class != classResourceExhausted {
		t.Fatalf("stream cap classified %q", class)
	}
...
func TestProjectionExhaustionClassifiesByResource(t *testing.T) {
	...
	st := &attemptState{parent: context.Background()}
	if class := st.classify(&interaction.Exhaustion{Resource: "admitted_event_work_bytes", Category: interaction.LimitEventWork, Limit: 8}, true); class != classResourceExhausted {
		t.Fatalf("projection exhaustion classified %q", class)
	}
}
