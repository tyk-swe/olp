...
func inspectorAnyRequest(raw json.RawMessage, operation, surface, mode, dialect, slug string, strict bool) (*openai.Request, *oif.Request, *media.Request, generation.Source, error) {
	if !strict || operation == "generation" {
		request, source, err := inspectorRequest(raw, operation, surface, mode, dialect, slug, strict)
		return request, nil, nil, source, err
	}
	if len(raw) == 0 {
		return nil, nil, nil, generation.Source{}, nil
	}
	doc, err := oif.ParseJSON(raw, oif.Limits{MaxBytes: 1 << 20})
	if err != nil {
		return nil, nil, nil, generation.Source{}, access.Invalid("request", "Use one bounded unambiguous native request.")
	}
	...
	if mediacontract.IsMediaOperation(operation) {
		if surface != "openai" || mode != "unary" && mode != "streaming" || operation != "image_generation" && operation != "speech" {
			return nil, nil, nil, generation.Source{}, access.Invalid("request", "This media source requires a live multipart or lifecycle invocation; the inspector cannot stage its assets.")
		}
		...
		if failure != nil || request.Route != slug || request.Mode() != mode {
			return nil, nil, nil, generation.Source{}, access.Invalid("request", "Use a native media request naming this route and delivery mode.")
		}
		return nil, nil, request, generation.Source{}, nil
	}
	if dialect == "" {
	...
	if !ok || codec.Operation.ID != operation || mode != "unary" || surface != "native" && surface != codec.Surface {
		return nil, nil, nil, generation.Source{}, access.Invalid("dialect", "Choose a registered unary dialect for this operation and surface.")
	}
	request, err := operationplan.Parse(dialect, raw, 1<<20)
	if err != nil {
		return nil, nil, nil, generation.Source{}, access.Invalid("request", "Use a valid native operation source.")
	}
	return nil, &request, nil, generation.Source{}, nil
}
