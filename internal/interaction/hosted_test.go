...
func TestHostedWebSearchAdmissionRequiresPermissionAndProfile(t *testing.T) {
	...
	_, err := qualified.BindRequest(hostedSource(t, body), Context{})
	...
	for _, id := range []string{"compatible-responses", "azure-v1-responses", "azure-legacy-responses"} {
		t.Run(id, func(t *testing.T) {
			_, err := template(t, configuration(t, id)).BindRequest(hostedSource(t, body), Context{AllowHostedTools: true, AllowProviderState: true, RetainedResponses: true})
	...
	for _, id := range []string{"anthropic-messages", "gemini-generation", "bedrock-converse"} {
		t.Run(id, func(t *testing.T) {
			_, err := template(t, configuration(t, id)).BindRequest(hostedSource(t, body), Context{AllowHostedTools: true})
	...
	_, err = chat.BindRequest(request(t, openai.FamilyChat, `{"model":"route","messages":[{"role":"user","content":"hi"}],"tools":[{"type":"web_search"}]}`), Context{AllowHostedTools: true})
	assertReason(t, err, "state_carrier")

	// Chat web_search_options is a hosted search control without a declared
	// lifecycle; permission alone cannot admit it.
	_, err = chat.BindRequest(request(t, openai.FamilyChat, `{"model":"route","messages":[{"role":"user","content":"hi"}],"web_search_options":{"search_context_size":"low"}}`), Context{AllowHostedTools: true})
	assertReason(t, err, "state_carrier")
}
...
func TestHostedWebSearchToolSchemaIsBounded(t *testing.T) {
	...
	for name, tool := range refused {
		t.Run(name, func(t *testing.T) {
			body := `{"model":"route","input":"hello","store":false,"tools":[` + tool + `]}`
			_, err := compiled.BindRequest(hostedSource(t, body), context)
			assertReason(t, err, "target_capability")
		})
	}
...
func TestHostedSelectorsRequireAdmission(t *testing.T) {
	...
	for _, include := range []string{"web_search_call.results", "web_search_call.action.sources"} {
		_, err := compiled.BindRequest(hostedSource(t, `{`+base+`,"include":["`+include+`"]}`), context)
		assertReason(t, err, "state_carrier")
	}
	...
	for _, include := range []string{"file_search_call.results", "code_interpreter_call.outputs", "computer_call_output.output.image_url"} {
		_, err := compiled.BindRequest(hostedSource(t, `{`+base+`,"tools":[{"type":"web_search"}],"include":["`+include+`"]}`), context)
		assertReason(t, err, "state_carrier")
	}
	// Unknown selectors fail closed.
	_, err := compiled.BindRequest(hostedSource(t, `{`+base+`,"tools":[{"type":"web_search"}],"include":["future_call.details"]}`), context)
	assertReason(t, err, "target_capability")
	...
	for _, choice := range []string{"web_search", "web_search_preview", "web_search_2025_08_26"} {
		_, err := compiled.BindRequest(hostedSource(t, `{`+base+`,"tool_choice":{"type":"`+choice+`"}}`), context)
		assertReason(t, err, "state_carrier")
	}
	...
	_, err = compiled.BindRequest(hostedSource(t, `{`+base+`,"tools":[{"type":"web_search"}],"tool_choice":{"type":"file_search"}}`), context)
	assertReason(t, err, "state_carrier")
...
func TestHostedWebSearchKeepsPolicyCoverage(t *testing.T) {
	...
	_, err := compiled.BindRequest(hostedSource(t, `{"model":"route","input":"hello","store":false,"tools":[{"type":"function","name":"f","parameters":{"type":"object"}},{"type":"web_search"}]}`), context)
	assertReason(t, err, "policy_conflict")
