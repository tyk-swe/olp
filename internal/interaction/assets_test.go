...
func TestResolvedFileBindingRefusals(t *testing.T) {
	...
	for _, fixture := range []struct {
		...
		t.Run(fixture.name, func(t *testing.T) {
			...
			_, bindErr := compiled.Bind(source, Context{ResolvedAssets: fixture.bindings})
...
func TestResolvedFileBindingStaysDialectOwned(t *testing.T) {
	...
	if _, err := chat.Bind(source, Context{ResolvedAssets: map[string]AssetBinding{managedFileID: binding}}); err == nil {
...
func TestFileReferencesVisitsOnlyDialectPositions(t *testing.T) {
	...
	refs := FileReferences(document, openai.FamilyResponses)
	...
	if refs := FileReferences(document, openai.FamilyChat); len(refs) != 1 || refs[0] != "strict_file_b" {
