...
UPDATE olp.users u SET role_management='oidc'
WHERE EXISTS (SELECT 1 FROM olp.oidc_identities i WHERE i.user_id=u.id)
  AND NOT EXISTS (SELECT 1 FROM olp.invitations i WHERE i.accepted_by=u.id)
  AND NOT EXISTS (SELECT 1 FROM olp.audit a WHERE a.actor_user_id=u.id
...
ALTER TABLE olp.sessions ADD COLUMN browser_hint text NOT NULL DEFAULT 'Unknown browser';
