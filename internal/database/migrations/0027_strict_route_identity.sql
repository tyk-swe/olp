...
ALTER TABLE olp_go.routes ADD COLUMN strict_contract boolean NOT NULL DEFAULT false;
UPDATE olp_go.routes r SET strict_contract=coalesce(v.fidelity->>'mode','legacy')='strict'
FROM olp_go.route_revisions v WHERE v.id=r.latest_revision_id;

DO $$ BEGIN
    IF EXISTS (
        SELECT 1 FROM olp_go.routes r JOIN olp_go.route_revisions v ON v.route_id=r.id
...
CREATE FUNCTION olp_go.preserve_route_contract_identity() RETURNS trigger
LANGUAGE plpgsql AS $$ BEGIN
    ...
    RETURN NEW;
END $$;
CREATE TRIGGER preserve_route_contract_identity BEFORE UPDATE ON olp_go.routes
FOR EACH ROW EXECUTE FUNCTION olp_go.preserve_route_contract_identity();

CREATE FUNCTION olp_go.check_route_revision_contract() RETURNS trigger
LANGUAGE plpgsql AS $$
...
BEGIN
    ...
    FROM olp_go.routes WHERE id=NEW.route_id FOR UPDATE;
    ...
    IF NEW.fidelity IS NULL AND EXISTS (
        SELECT 1 FROM olp_go.route_revisions v
    ...
    RETURN NEW;
END $$;
CREATE TRIGGER check_route_revision_contract BEFORE INSERT OR UPDATE ON olp_go.route_revisions
FOR EACH ROW EXECUTE FUNCTION olp_go.check_route_revision_contract();
...
CREATE FUNCTION olp_go.check_runtime_route_contracts() RETURNS trigger
LANGUAGE plpgsql AS $$ BEGIN
    IF EXISTS (
        SELECT 1 FROM olp_go.routes r
        JOIN olp_go.route_revisions v ON v.id=r.latest_revision_id
    ...
    RETURN NEW;
END $$;
CREATE TRIGGER check_runtime_route_contracts BEFORE INSERT ON olp_go.runtime_releases
FOR EACH ROW EXECUTE FUNCTION olp_go.check_runtime_route_contracts();
