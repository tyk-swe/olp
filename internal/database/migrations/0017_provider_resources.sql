...
ALTER TABLE olp.attempt_usage_facts DROP CONSTRAINT attempt_usage_facts_surface_check;
ALTER TABLE olp.attempt_usage_facts ADD CONSTRAINT attempt_usage_facts_surface_check
    CHECK (surface IN ('openai','anthropic','gemini','bedrock','unknown'));
ALTER TABLE olp.attempt_usage_hourly DROP CONSTRAINT attempt_usage_hourly_surface_check;
ALTER TABLE olp.attempt_usage_hourly ADD CONSTRAINT attempt_usage_hourly_surface_check
    CHECK (surface IN ('openai','anthropic','gemini','bedrock','unknown'));
