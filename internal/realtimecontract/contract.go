...
// Refusal is a pre-dispatch incompatibility with a safe source location.
type Refusal struct {
	Field, Requirement, Message string
}

func (e *Refusal) Error() string { return e.Message }

func refuse(field, requirement, message string) error {
	return &Refusal{Field: field, Requirement: requirement, Message: message}
}
...
func Compile(c Config) (*Template, error) {
	...
	if err != nil || c.Provider.ValidateProfile() != nil {
		return nil, refuse("/profile", "explicit_profile", "Strict realtime requires a registered versioned provider profile.")
	}
	if p.OperationDialect("realtime") != "openai-realtime" ||
		...
		return nil, refuse("/profile", "native_realtime_contract", "This provider profile has no strict OpenAI Realtime WebSocket contract.")
	}
	if _, err := c.Provider.RealtimeURL(c.Model); err != nil {
		return nil, refuse("/model", "realtime_address", "This target cannot address its native realtime model.")
	}
	if c.Policy != nil && len(c.Policy.Rules) > 0 {
		return nil, refuse("/content_policy", "inspectable_policy", "Realtime frames cannot be admitted with a content policy this relay cannot enforce.")
	}
	if len(c.Provider.SemanticHeaders) > 0 || len(c.Provider.QuerySettings) > 0 {
		return nil, refuse("/provider", "unchanged_realtime_handshake", "Strict realtime has no qualified provider handshake overrides or implicit defaults.")
	}
	if defaults, ok := c.Provider.OperationDefaults["realtime"]; ok && (len(defaults.Values) > 0 || len(defaults.NativeOptions) > 0) {
		return nil, refuse("/operation_defaults/realtime", "native_realtime_defaults", "Realtime defaults cannot be applied to opaque duplex events.")
	}
	if binding, ok := c.Provider.Bindings[c.Model]; ok {
		if defaults, ok := binding.Defaults["realtime"]; ok && (len(defaults.Values) > 0 || len(defaults.NativeOptions) > 0) {
			return nil, refuse("/bindings/realtime", "native_realtime_defaults", "Realtime model defaults cannot be applied to opaque duplex events.")
		}
	}
...
func (t *Template) AdmitHandshake(rawQuery string, headers http.Header, route string) error {
	if t == nil || t.profileID == "" || t.profileRevision == "" || t.model == "" {
		return refuse("/profile", "compiled_realtime_contract", "The selected target has no compiled realtime contract.")
	}
	query, err := url.ParseQuery(rawQuery)
	if err != nil {
		return refuse("/query", "unambiguous_realtime_query", "The realtime query is malformed.")
	}
	for name, values := range query {
		switch name {
		case "model":
			if len(values) != 1 || values[0] != route {
				return refuse("/query/model", "one_published_route", "Realtime requires exactly one published route model.")
			}
		case "attribution":
			if len(values) != 1 {
				return refuse("/query/attribution", "one_attribution", "Realtime accepts at most one attribution value.")
			}
		default:
			return refuse("/query/"+name, "native_realtime_query", "This realtime query control has no qualified native binding.")
		}
	}
	...
	for name, values := range headers {
		if len(values) != 1 {
			return refuse("/headers/"+name, "single_header", "Realtime does not admit repeated handshake headers.")
		}
		...
		default:
			return refuse("/headers/"+name, "native_realtime_header", "This realtime handshake header has no qualified native binding.")
		}
	}
	if len(headers.Values("Authorization")) != 1 {
		return refuse("/headers/Authorization", "one_authorization", "Strict realtime requires one Authorization header.")
	}
	return nil
}
