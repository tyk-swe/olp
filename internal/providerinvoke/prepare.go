...
func Prepare(request *openai.Request, config connectors.Config, model string, legacyDefaults protocols.Object) (Invocation,error) {
	...
	if config.ProfileID!="" {
		...
		if err!=nil{return Invocation{},profileError(err)}
	...
	if err!=nil{return Invocation{},err}
	if config.ProfileID=="" {return Invocation{Prepared:prepared,Wire:wire},nil}
	...
	for _,entry:=range prepared.Provenance(){
		if entry.Origin!=oif.ProviderDefault{continue}
		for _,origin:=range origins {
			if origin.Pointer==entry.Pointer || strings.HasPrefix(entry.Pointer,origin.Pointer+"/") {
				origin.Pointer=entry.Pointer
				applied=append(applied,origin)
				if strings.HasPrefix(origin.Source,"binding_default") {prepared=prepared.WithProvenance(oif.Provenance{Pointer:entry.Pointer,Origin:oif.ModelDefault,Reason:origin.Source+" in "+origin.Dialect})}
				break
			}
		}
	}
	...
	if !bytes.Equal(body,wrapped){
		document,err:=oif.ParseJSON(wrapped,prepared.Document().Limits());if err!=nil{return Invocation{},err}
...
func profileError(err error) error {return &openai.RequestError{Code:"unsupported_parameter",Param:"provider_profile",Message:err.Error()}}
