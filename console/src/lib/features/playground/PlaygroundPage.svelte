<script lang="ts">
  ...
  async function submit(event: SubmitEvent) {
    ...
    try {
      ...
      validationError = error instanceof Error ? error.message : 'Check the request fields.';
      return;
    }
...
<div class="playground-grid">
  <form class="card composer" onsubmit={submit}>
    ...
    <div class="route-grid"><div class="form-field"><label for="playground-temperature">Temperature</label><input id="playground-temperature" bind:value={temperature} inputmode="decimal" autocomplete="off" placeholder="Provider default" aria-describedby="temperature-help" /><small id="temperature-help">0 through 2.</small></div><div class="form-field"><label for="playground-max-output">Max output tokens</label><input id="playground-max-output" bind:value={maxOutputTokens} inputmode="numeric" autocomplete="off" placeholder="Provider default" aria-describedby="max-output-help" /><small id="max-output-help">1 through 1000000.</small></div></div>
  ...
  <section class="card result" aria-labelledby="result-title" aria-live="polite">
    ...
    {:else if mutation.data}
      {#if mutation.data.refusal}<div class="refusal" role="alert"><strong>The model refused this request</strong><p>{mutation.data.refusal}</p></div>{/if}
      {#if !mutation.data.refusal && !mutation.data.output_text && !mutation.data.tool_calls?.length && (mutation.data.structured_output === undefined || mutation.data.structured_output === null)}<div class="empty-state"><div><strong>No content returned</strong><p>The route answered without text, a tool call, or structured output. The finish reason below says why.</p></div></div>{/if}
      ...
      <dl><div><dt>Response ID</dt><dd class="mono">{mutation.data.id}</dd></div><div><dt>Route</dt><dd>{mutation.data.model}</dd></div><div><dt>Provider model</dt><dd>{mutation.data.provider_model ?? 'Not reported'}</dd></div><div><dt>Finish reason</dt><dd>{mutation.data.finish_reason ?? 'Not reported'}</dd></div><div><dt>Input tokens</dt><dd>{formatInteger(mutation.data.usage?.input_tokens)}</dd></div><div><dt>Cached input tokens</dt><dd>{formatInteger(mutation.data.usage?.cached_input_tokens)}</dd></div><div><dt>Output tokens</dt><dd>{formatInteger(mutation.data.usage?.output_tokens)}</dd></div><div><dt>Reasoning tokens</dt><dd>{formatInteger(mutation.data.usage?.reasoning_tokens)}</dd></div><div><dt>Total tokens</dt><dd>{formatInteger(mutation.data.usage?.total_tokens)}</dd></div></dl>
...
<style>
  ...
  .form-field small { display: block; margin-top: 0.25rem; color: var(--foreground-muted); font-size: 0.7rem; }
  .refusal { margin-top: 1rem; padding: 0.85rem 1rem; border: 1px solid color-mix(in srgb, var(--warning) 45%, var(--border)); border-radius: 0.375rem; background: var(--warning-soft); color: var(--warning); }
  .refusal p { margin: 0.25rem 0 0; }
