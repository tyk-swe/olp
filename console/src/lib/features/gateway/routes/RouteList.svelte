...
{:else if drafts.isError || activeRoutes.isError}
  <div class="inline-problem" role="alert">
    {message(drafts.error ?? activeRoutes.error)}
    <button
      class="button button-secondary"
      type="button"
      onclick={() => {
        drafts.refetch();
        activeRoutes.refetch();
      }}>Retry</button
    >
  </div>
...
{:else}
  <section class="route-section" aria-labelledby="active-routes-heading">
        ...
        <table class="data-table">
          <thead
            ><tr
              ...
              ><th>Targets</th><th>Activated</th><th
          ...
          ><tbody
            >{#each activeRoutes.data.items as item (item.id)}<tr
                ...
                ><td
                  >{new Date(
                    item.latest_revision.activated_at
                  ).toLocaleString()}</td
                ><td
                  ><a
                    class="button button-secondary"
    ...
    {/if}
    <CursorPagination
      {...cursorPaginationProps(listState.route, activeRoutes.data?.nextCursor)}
        ...
        <table class="data-table">
          <thead
            ><tr
              ...
              ><th>Updated</th><th><span class="sr-only">Actions</span></th></tr
            ></thead
          ><tbody
            >{#each drafts.data.items as item (item.id)}<tr
                ...
                ><td>{new Date(item.updated_at).toLocaleString()}</td><td
                  ><a
                    class="button button-secondary"
                    href={resolve(`/routes/${item.id}`)}>Open Studio</a
                  ></td
                ></tr
              >{/each}</tbody
          >
