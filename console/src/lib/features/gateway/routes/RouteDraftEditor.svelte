...
<div class="page-header">
  ...
  <div class="page-actions">
    <a class="button button-secondary" href={resolve('/routes')}>Cancel</a
    >{#if resourceId && draft.data}<button
...
{:else if (!isNew && draft.isError) || providerModels.isError}
  <div class="inline-problem" role="alert">
    {message(draft.error ?? providerModels.error)}
    <button
...
{:else}
  ...
  >
      ...
      <section class="card editor" aria-labelledby="targets-heading">
        <div class="section-heading">
          ...
          <button
            ...
            type="button"
            onclick={addTarget}
            disabled={!modelOptions.length}>Add target</button
          >
        </div>
          ...
          {#each targets as target, index (index)}
            <li>
              ...
              <button
                ...
                onclick={() => removeTarget(index)}>×</button
              >
              <div
    ...
    <aside class="card publish-panel" aria-labelledby="publish-heading">
      ...
      <p>Saving changes invalidates prior validation.</p>
      <button
        class="button button-secondary"
        type="submit"
        disabled={Boolean(busy)}
      ...
      {#if !isNew && draft.data}
        ...
        <button
          ...
          disabled={Boolean(busy) || !validated}
