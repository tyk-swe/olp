<script lang="ts">
  ...
  let {
    ...
    busy: string;
    onRotate: (event: SubmitEvent) => void;
...
<section class="card editor" aria-labelledby="credential-heading">
  ...
  {#if providerSpec && !requiresCredential(providerSpec, current.auth_mode)}<div
    ...
    >
      ...
      >
    </div>{:else}<form class="credential-form" onsubmit={onRotate}>
  ...
  {#if credentialsPending}<p role="status">Loading versions…</p>{:else}<ul
      class="credential-list"
    >
      {#each credentials as credential (credential.id)}<li>
          <span
