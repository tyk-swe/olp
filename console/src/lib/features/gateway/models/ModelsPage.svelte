<script lang="ts">
  ...
  function message(error: unknown) {
    return error instanceof Error
      ? error.message
      : 'The model inventory could not be loaded.';
  }
...
{:else if filtered.length === 0}
  <section class="card empty-state">
    <div>
      ...
      <p>Clear the search or select another client surface.</p>
      <button
        class="button button-secondary"
        type="button"
        onclick={() => {
            ...
            <td
              ><label class="eligibility"
                ><input
                  ...
                  disabled={busyModel === entry.model.id}
                  onchange={(event) =>
                    toggle(entry, event.currentTarget.checked)}
                /><span>{entry.model.enabled ? 'Enabled' : 'Disabled'}</span
                ></label
              ></td
            >
          </tr>
