<script lang="ts">
  ...
  import { popCursor, pushCursor, type CursorHistory } from '$lib/api/pagination';
...
{:else}
  ...
  <CursorPagination page={listState.history.length + 1} hasPrevious={listState.history.length > 0} hasNext={Boolean(providers.data?.nextCursor)} onPrevious={previousProviderPage} onNext={nextProviderPage} label="Provider pages" />
{/if}
