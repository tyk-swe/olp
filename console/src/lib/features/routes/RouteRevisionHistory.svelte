<script lang="ts">
  ...
  const revisions = createQuery(() => ({
    queryKey: routeKeys.revisions(routeId),
  ...
  async function compareRevisions() {
    ...
    await run('diff', async () => {
      revisionDiff = await diffRouteRevisions(
        routeId,
        fromRevision,
        toRevision
      );
  ...
  async function restore(revision: RouteRevision) {
    ...
    if (
      !confirm(
        `Restore route revision ${revision.revision} as a new draft? The live revision keeps serving traffic until the draft is activated.`
      )
    )
      return;
...
<div class="page-header">
  <div>
    <p class="eyebrow">Gateway · Route history</p>
    <h1 class="page-title">Immutable revisions</h1>
    <p class="page-description">
      Restoring history creates a new editable draft. It never rolls back keys
      or provider credentials.
    </p>
  </div>
  <a class="button button-secondary" href={resolve('/routes')}>All routes</a>
</div>
...
{:else if !revisions.data?.length}
  <section class="card empty-state">
    <div>
      <h2>No activated revisions</h2>
      <p>Activate a validated route draft to create revision 1.</p>
    </div>
  </section>
{:else}
  <section class="card revision-compare" aria-labelledby="compare-heading">
    <div>
      <p class="eyebrow">Revision diff</p>
      <h2 id="compare-heading">Compare configuration</h2>
    </div>
    <label
      >From<select bind:value={fromRevision}
        >{#each revisions.data as revision (revision.id)}<option
            value={revision.id}>Revision {revision.revision}</option
          >{/each}</select
      ></label
    ><label
      >To<select bind:value={toRevision}
        >{#each revisions.data as revision (revision.id)}<option
            value={revision.id}>Revision {revision.revision}</option
          >{/each}</select
      ></label
    ><button
      ...
      disabled={busy === 'diff'}
      >{busy === 'diff' ? 'Comparing…' : 'Compare'}</button
    >
  </section>
  {#if revisionDiff}
    <section class="diff-grid" aria-label="Revision differences">
      ...
      <article class="card">
        ...
        {#if revisionDiff.operations_added.length}<ul>
            {#each revisionDiff.operations_added as item (item)}<li>
                <code>{item}</code>
              </li>{/each}
          </ul>{:else}<strong>None</strong>{/if}
        <p>Operations removed</p>
        {#if revisionDiff.operations_removed.length}<ul>
            {#each revisionDiff.operations_removed as item (item)}<li>
                <code>{item}</code>
              </li>{/each}
          </ul>{:else}<strong>None</strong>{/if}
      </article>
      <article class="card">
        <p>Target changes</p>
        {#if revisionDiff.targets_added.length}<strong>Added</strong>
          <ul>
            {#each revisionDiff.targets_added as item (item)}<li>
                <code>{item}</code>
              </li>{/each}
          </ul>{/if}{#if revisionDiff.targets_removed.length}<strong
            >Removed</strong
          >
          <ul>
            {#each revisionDiff.targets_removed as item (item)}<li>
                <code>{item}</code>
              </li>{/each}
          </ul>{/if}{#if revisionDiff.targets_changed.length}<strong
            >Changed</strong
          >
          <ul>
            {#each revisionDiff.targets_changed as item (item)}<li>
                <code>{item}</code>
              </li>{/each}
          </ul>{/if}{#if !revisionDiff.targets_added.length && !revisionDiff.targets_removed.length && !revisionDiff.targets_changed.length}<strong
            >None</strong
          >{/if}
      </article>
    </section>
      ...
      ><tbody
        >{#each revisions.data as revision (revision.id)}<tr
            ...
            ><td data-label="Revision"
              ><strong>Revision {revision.revision}</strong><br /><code
                >{revision.id}</code
              ><br /><small
...
<style>
  ...
  @media (max-width: 48rem) {
    ...
    .revision-table tbody .revision-action {
      grid-column: 1 / -1;
    }
    .revision-table tbody .revision-action::before {
      display: none;
    }
    .revision-table tbody .revision-action .button {
      width: 100%;
    }
  }
</style>
