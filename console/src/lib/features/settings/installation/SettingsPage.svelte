  ...
  async function addPricing(event: SubmitEvent) {
    ...
    try {
      ...
      error =
        cause instanceof Error
          ? cause.message
          : 'The pricing revision could not be created.';
    } finally {
      savingPrice = false;
    }
  }
      ...
      {#each settings.data ?? [] as setting (setting.key)}<article
        ...
        >
          <div>
            ...
            <small>Updated {formatDate(setting.updated_at)}</small>
          </div>
...
<section class="settings-section" aria-labelledby="pricing-title">
  ...
  <form class="card price-form" onsubmit={addPricing}>
    <div class="form-grid">
      <div class="form-field">
        <label for="provider-kind">Provider kind</label><select
          ...
          disabled={providerKinds.isPending || providerKinds.isError}
    ...
    <button
      ...
      type="submit"
      disabled={savingPrice || !providerKind || providerKinds.isError}
        ...
        <details class="card">
          ...
          >
            <table class="data-table">
              ...
              <thead
                ><tr
                  ><th>Provider / model</th><th>Operation</th><th
                    >Input / million</th
                  ><th>Output / million</th><th>Unit</th><th>Currency</th></tr
                ></thead
              >
              <tbody
