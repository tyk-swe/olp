<script lang="ts">
  ...
  const health = createQuery(() => ({
    ...
    queryFn: async () => {
      ...
      const [readiness, providers, generations, persistence, epochs] =
        await Promise.all([
          getReadiness(),
  ...
  function healthTone(value: string) {
    if (['healthy', 'ok', 'active', 'passing'].includes(value.toLowerCase()))
      return 'success';
    if (
      ['degraded', 'stale', 'unknown', 'unavailable_lkg'].includes(
        value.toLowerCase()
      )
    )
      return 'warning';
    ...
    try {
      ...
      epochError =
        error instanceof Error
          ? error.message
          : 'The epoch could not be acknowledged.';
    } finally {
      busyEpoch = '';
    }
  }
...
<div class="page-header">
  <div>
    ...
    <p class="page-description">
      Gateway dependencies, provider outcomes, runtime convergence, and
  ...
  <button
    ...
    disabled={health.isFetching}>Refresh</button
  >
</div>

<p class="refresh-note" aria-live="polite">
  Automatically refreshes every 15 seconds{health.dataUpdatedAt
...
{:else if health.isError}
  <div class="inline-problem" role="alert">
    <strong>Control health is unavailable.</strong> The gateway may still be
    serving its last-known-good runtime.
    <button class="text-button" onclick={() => health.refetch()}
      >Try again</button
    >
  </div>
{:else if health.data}
  <section class="metric-grid" aria-label="Dependency readiness">
    <article class="card metric-card">
      <p>Gateway</p>
      <strong
        ><span class="badge {healthTone(health.data.readiness.status)}"
          >{health.data.readiness.status}</span
        ></strong
      >
    </article>
    <article class="card metric-card">
      <p>PostgreSQL</p>
      <strong
        ><span class="badge {healthTone(health.data.readiness.database)}"
          >{health.data.readiness.database.replaceAll('_', ' ')}</span
        ></strong
      >
    </article>
    <article class="card metric-card">
      ...
      <strong
        ><span class="badge {healthTone(health.data.readiness.limits)}"
          >{health.data.readiness.limits.replaceAll('_', ' ')}</span
        ></strong
      >
    </article>
    <article class="card metric-card">
      <p>Active generation</p>
      <strong>#{health.data.readiness.generation ?? '—'}</strong>
    </article>
  </section>
  ...
  <section class="card persistence" aria-labelledby="persistence-title">
    <div
      class="health-icon"
      class:ok={health.data.persistence.complete}
      aria-hidden="true"
    >
      {health.data.persistence.complete ? '✓' : '!'}
    </div>
    <div>
      ...
      <p>
        {health.data.persistence.request_metadata_gap_events} request metadata gap-event
        lower bound · {health.data.persistence
          .uncertain_request_metadata_gap_count} uncertain request metadata epochs
        · {health.data.persistence.incomplete_count} incomplete requests · {health
          .data.persistence.unpriced_count} unpriced requests. Missing or uncertain
  ...
  <section class="section" aria-labelledby="epochs-title">
    <div class="section-heading">
      ...
      <span
        class:warning={health.data.epochs.items.length > 0}
        class:success={health.data.epochs.items.length === 0}
        class="badge">{health.data.epochs.items.length} on page</span
      >
    </div>
    ...
    {:else}
      ...
      >
        <table class="data-table">
          ...
          ><thead
            ><tr
              ><th scope="col">Gateway</th><th scope="col">Detected</th><th
                scope="col">Accepted / persisted</th
              ><th scope="col">Dropped / abandoned</th><th scope="col"
                >Uncertain lower bound</th
              ><th scope="col"><span class="sr-only">Action</span></th></tr
            ></thead
          ><tbody
            >{#each health.data.epochs.items as epoch (epoch.process_epoch)}<tr
                ><td
                  ><strong>{epoch.gateway_instance}</strong><br /><code
                    >{epoch.process_epoch}</code
                  ></td
                ><td
                  >{formatDate(epoch.stale_detected_at ?? epoch.updated_at)}</td
  ...
  <section class="section" aria-labelledby="providers-title">
    <div class="section-heading">
      <div>
        <p class="eyebrow">Rolling 15 minutes</p>
        <h2 id="providers-title">Providers</h2>
      </div>
      <span class="badge">{health.data.providers.data.length} configured</span>
    </div>
  ...
  <section class="section" aria-labelledby="runtime-title">
      ...
      <table class="data-table">
        ...
        ><thead
          ><tr
            ...
            ><th scope="col">Created</th><th scope="col">Gateway state</th></tr
          ></thead
        ><tbody
          >{#each health.data.generations.items as generation (generation.id)}<tr
              ><td><strong>#{generation.sequence}</strong></td><td class="mono"
                >{generation.sha256.slice(0, 16)}…</td
              ><td>{generation.created_by_email}</td><td
                >{formatDate(generation.created_at)}</td
              ><td
                >{#if generation.sequence === health.data.readiness.generation}<span
    ...
    <CursorPagination
      {...cursorPaginationProps(
        ...
        health.data.generations.nextCursor
      )}
