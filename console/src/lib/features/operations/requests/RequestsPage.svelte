...
{#if requestId}
  ...
  {:else if detail.isError}
    <div class="inline-problem" role="alert">
      The request timeline could not be loaded. <button
        class="text-button"
  ...
  {:else if detail.data}
    <section class="metric-grid" aria-label="Request summary">
      ...
      <article class="card metric-card">
        <p>Total latency</p>
        <strong>{detail.data.total_latency_ms ?? '—'} ms</strong>
      </article>
      <article class="card metric-card">
        <p>First byte (TTFT)</p>
        <strong>{detail.data.first_byte_ms ?? '—'} ms</strong>
      </article>
      <article class="card metric-card">
        <p>Estimated cost</p>
        <strong class:unpriced={detail.data.unpriced}
          >{formatCost(
            detail.data.estimated_cost,
            detail.data.currency ?? 'USD'
          )}</strong
        >
      </article>
    </section>
    ...
    <section class="card request-facts" aria-labelledby="decision-title">
      ...
      <dl>
        ...
        <div>
          <dt>Input tokens</dt>
          <dd>{formatCompact(detail.data.input_tokens)}</dd>
        </div>
        <div>
          <dt>Output tokens</dt>
          <dd>{formatCompact(detail.data.output_tokens)}</dd>
        </div>
        <div>
          ...
          <dd>
            <span
              class="badge"
              class:success={detail.data.usage_complete}
              class:warning={!detail.data.usage_complete}
              >{detail.data.usage_complete ? 'Complete' : 'Incomplete'}</span
            >
          </dd>
        </div>
        <div>
...
{:else}
  ...
  {:else if requests.isError}
    <div class="inline-problem" role="alert">
      Request metadata is unavailable. <button
        class="text-button"
        onclick={() => requests.refetch()}>Try again</button
      >
    </div>
        ...
        <tbody>
          {#each requests.data?.items ?? [] as request (request.id)}
            <tr>
              ...
              <td
                >{request.first_byte_ms ?? '—'} / {request.total_latency_ms ??
                  '—'} ms</td
              >
              <td
                >{formatCompact(request.input_tokens)} in<br />{formatCompact(
        ...
        <li class="card">
          ...
          <dl>
            ...
            <div>
              <dt>Tokens</dt>
              <dd>
                {formatCompact(request.input_tokens)} in · {formatCompact(
                  request.output_tokens
                )} out
              </dd>
            </div>
            <div>
              ...
              <dd class:unpriced={request.unpriced}>
                {formatCost(request.estimated_cost, request.currency ?? 'USD')}
              </dd>
            </div>
          </dl>
          <a
