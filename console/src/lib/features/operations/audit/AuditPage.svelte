...
<div class="page-header">
  <div>
    ...
    <p class="page-description">
      Security-sensitive actions and outcomes. Credentials, prompts, outputs,
      raw headers, and tool data are never recorded.
    </p>
  </div>
  <button
...
{:else if audit.isError}
  <div class="inline-problem" role="alert">
    Audit events are unavailable. <button
      class="text-button"
      onclick={() => audit.refetch()}>Try again</button
    >
  </div>
{:else if audit.data?.items.length === 0}
  <div class="card empty-state">
    <div>
      ...
      <p>Security and configuration changes will appear here.</p>
    </div>
  </div>
{:else}
      ...
      <caption class="sr-only">Audit events, newest first</caption><thead
        ><tr
          ><th scope="col">Occurred</th><th scope="col">Actor</th><th
            scope="col">Action</th
          ><th scope="col">Resource</th><th scope="col">Outcome</th></tr
        ></thead
      ><tbody
  ...
  <CursorPagination
    {...cursorPaginationProps(pagination, audit.data?.nextCursor)}
    label="Audit pages"
  />
{/if}
