<script lang="ts">
  ...
  }: { points: UsagePoint[]; title?: string } = $props();
  const bounds = {
    width: 640,
    height: 260,
    left: 52,
...
>
  <figcaption>
    <div>
      <h2 id="usage-chart-title">{title}</h2>
      <p id="usage-chart-description">
        Request count by time bucket. Exact values follow the chart.
      </p>
    </div>
    ...
    <div class="chart" aria-hidden="true">
      ...
      >
        ...
        <text class="axis-label" x={bounds.left} y={bounds.height - 8}
          >{formatDate(points[0].bucket)}</text
        >
        {#if points.length > 1}
          <text
            class="axis-label"
            x={bounds.width - bounds.right}
            y={bounds.height - 8}
            text-anchor="end">{formatDate(points.at(-1)?.bucket)}</text
          >
        {/if}
      </svg>
    </div>
      ...
      >
        <table class="data-table">
          <caption class="sr-only"
            >Exact usage values shown in the time-series chart</caption
          >
          <thead
            ><tr
          ...
          <tbody>
            {#each points as point (point.bucket)}
              <tr
                ><td>{formatDate(point.bucket)}</td><td
                  >{point.request_count}</td
                ><td>{point.input_tokens}</td><td>{point.output_tokens}</td><td
                  >{formatCost(
