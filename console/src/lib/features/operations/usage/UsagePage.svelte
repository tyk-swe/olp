<script lang="ts">
  ...
  let applied = $state<UsageFilters>({
...
{:else if usage.isError}
  <div class="inline-problem" role="alert">
    Usage could not be loaded. <button
      class="text-button"
      onclick={() => usage.refetch()}>Try again</button
    >
  </div>
{:else if usage.data}
    ...
    <article class="card pipeline-card">
      ...
      <span
        >{usage.data.completeness.request_metadata_consumer.checked_at
          ? `Checkpoint ${usage.data.completeness.request_metadata_consumer.heartbeat_age_seconds ?? 0}s ago`
          : 'No worker checkpoint recorded'}</span
      >
    </article>
    <article class="card pipeline-card">
    ...
    <article class="card metric-card">
      ...
      <strong class:unpriced={usage.data.summary.estimated_cost == null}
        >{formatCost(
          ...
          usage.data.summary.currency ?? 'USD'
        )}</strong
      >
    </article>
  </section>
  ...
  <section class="breakdown" aria-labelledby="breakdown-title">
    <div class="section-heading">
      <div>
        ...
        <h2 id="breakdown-title">By {dimension.replace('_', ' ')}</h2>
      </div>
    ...
    {:else}
      ...
      >
        <table class="data-table">
          ...
          <thead
            ><tr
              ><th scope="col">{dimension.replace('_', ' ')}</th><th scope="col"
                >Requests</th
              ><th scope="col">Input tokens</th><th scope="col"
                >Output tokens</th
          ...
          <tbody
            >{#each usage.data.breakdown as row (row.dimension)}<tr
                ...
                ><td>{formatCost(row.estimated_cost, row.currency ?? 'USD')}</td
                ><td
