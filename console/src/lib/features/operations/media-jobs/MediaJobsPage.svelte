...
{#if jobId}
  ...
  {:else if detail.isError}<div class="inline-problem" role="alert">
      {errorMessage(detail.error, 'The media job could not be loaded.')}
      <button class="text-button" type="button" onclick={() => detail.refetch()}
        >Retry</button
      >
    </div>
  {:else if detail.data}
    <section class="card job-detail" aria-labelledby="job-state-heading">
      ...
      <dl>
        ...
        <div>
          <dt>Last polled</dt>
          <dd>
            {detail.data.last_polled_at
              ? formatDate(detail.data.last_polled_at)
              : 'Never polled'}
          </dd>
        </div>
        <div>
          ...
          <dd>
            {detail.data.expires_at
              ? formatDate(detail.data.expires_at)
              : 'No retention deadline'}
          </dd>
        </div>
        <div>
          <dt>Deleted</dt>
          <dd>
            {detail.data.deleted_at
              ? formatDate(detail.data.deleted_at)
              : 'Not deleted'}
          </dd>
        </div>
        <div>
...
<style>
  ...
  @media (max-width: 48rem) {
    ...
    .job-table tbody .job-action {
      grid-column: 1 / -1;
    }
    .job-table tbody .job-action::before {
      display: none;
    }
    .job-table tbody .job-action .button {
      width: 100%;
    }
  }
</style>
