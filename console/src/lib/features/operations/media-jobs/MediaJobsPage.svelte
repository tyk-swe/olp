...
{#if jobId}
  <div class="page-header">
    <div>
      <p class="eyebrow">Operations · Media job</p>
      <h1 class="page-title">Media job detail</h1>
      <p class="page-description">
        Lifecycle and reconciliation metadata only. Uploaded and generated media
  ...
  {:else if detail.isError}<div class="inline-problem" role="alert">
      The media job could not be loaded. <button
        class="text-button"
        type="button"
        onclick={() => detail.refetch()}>Retry</button
      >
    </div>
  {:else if detail.data}
    <section class="card job-detail" aria-labelledby="job-state-heading">
      <div class="section-heading">
        <div>
          <p class="eyebrow">{detail.data.operation}</p>
          <h2 id="job-state-heading">{detail.data.route}</h2>
        </div>
        <span class={`badge ${tone(detail.data.state)}`}
          >{detail.data.state}</span
        >
      </div>
      <dl>
        <div>
        ...
        <div>
          <dt>Progress</dt>
          <dd>
            {detail.data.progress_percent == null
              ? '—'
              : `${detail.data.progress_percent}%`}
          </dd>
        </div>
        <div>
          <dt>Provider</dt>
          <dd>
            {detail.data.provider_name}<small
              >{detail.data.provider_model}</small
            >
          </dd>
        </div>
        <div>
        ...
        <div>
          ...
          <dd>
            {detail.data.content_available
              ? 'Available through the authenticated vendor API'
              : 'Not available'}
          </dd>
        </div>
        <div>
      ...
      {#if detail.data.error_class || detail.data.reconciliation_error}<div
          ...
          role="alert"
        >
          <strong>{detail.data.error_class ?? 'Reconciliation error'}</strong>
          <p>
            {detail.data.reconciliation_error ?? 'The upstream job failed.'}
          </p>
        </div>{/if}
      <div class="identifiers">
        <p><strong>OLP job</strong><code>{detail.data.id}</code></p>
        <p>
          <strong>Upstream job</strong><code
            >{detail.data.upstream_job_id ?? 'not assigned'}</code
          >
        </p>
        <p><strong>API key ID</strong><code>{detail.data.api_key_id}</code></p>
      </div>
    </section>
  {/if}
{:else}
  <div class="page-header">
    <div>
      ...
      <h1 class="page-title">Media Jobs</h1>
      <p class="page-description">
        Track asynchronous video and media lifecycles without exposing uploaded
  ...
  {:else if jobs.isError}<div class="inline-problem" role="alert">
      Media jobs are unavailable. <button
        class="text-button"
        type="button"
        onclick={() => jobs.refetch()}>Retry</button
      >
    </div>
  ...
  {:else}<div class="table-shell">
      <table class="data-table">
        <caption class="sr-only">Asynchronous media jobs</caption><thead
          ><tr
            ><th>Route / operation</th><th>Provider</th><th>State</th><th
              >Lifecycle</th
            ><th>Progress</th><th>Updated</th><th
        ...
        ><tbody
          >{#each jobs.data?.items ?? [] as job (job.id)}<tr
              ...
              ><td>{job.provider_name}<small>{job.provider_model}</small></td
              ><td
                ><span class={`badge ${tone(job.state)}`}>{job.state}</span></td
              ><td>{job.lifecycle.replaceAll('_', ' ')}</td><td
                >{job.progress_percent == null
                  ? '—'
                  : `${job.progress_percent}%`}</td
              ><td>{formatDate(job.updated_at)}</td><td
                ><a
                  class="button button-secondary"
    ...
    <CursorPagination
      {...cursorPaginationProps(listState, jobs.data?.nextCursor)}
