<script lang="ts">
  import { healthKeys } from '$lib/features/runtime/health/healthKeys';
  ...
  const epochs = createQuery(() => ({
    queryKey: healthKeys.epochs(epochPagination.cursor),
    queryFn: () =>
