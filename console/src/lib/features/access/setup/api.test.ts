...
import { createOwner, getSetupStatus } from '$lib/features/access/setup/api';
...
describe('setup API', () => {
  ...
  it('normalizes RFC 9457 field errors', async () => {
      ...
      vi.fn().mockResolvedValue(
        new Response(
          JSON.stringify({
            ...
            errors: {
              email: [
                { code: 'invalid', message: 'That address is already in use.' }
              ]
            }
          }),
          {
    ...
    expect((error as ApiProblem).problem.errors).toEqual({
      email: [{ code: 'invalid', message: 'That address is already in use.' }]
    });
  });
});
