<script lang="ts">
  ...
  async function changeActive(user: User) {
    ...
    if (!active && !confirm(`Deactivate ${user.display_name}? Every active session will be revoked. API keys are team-scoped and will remain active; after deactivation, review the API-key inventory and explicitly rotate or revoke any keys attributed to this member.`)) return;
    await run(`active-${user.id}`, async () => {
      ...
      notice = active
        ...
        : `${updated.display_name} was deactivated and existing sessions were revoked. Next: review API Keys for keys attributed to this member; team-scoped keys are not automatically revoked.`;
    });
  }
...
<svelte:head><title>Team & Access · OpenLLMProxy</title></svelte:head>
...
<div class="page-header"><div><p class="eyebrow">Access</p><h1 class="page-title">Team & Access</h1><p class="page-description">One installation, fixed roles, explicit invitations, revocable sessions, and one linked OIDC provider.</p></div><a class="button button-primary" href="/team" onclick={(event) => { event.preventDefault(); tab = 'invitations'; }}>Invite member</a></div>

<nav class="tabs" aria-label="Team settings">{#each [['members', 'Members'], ['invitations', 'Invitations'], ['sessions', 'Sessions'], ['oidc', 'OIDC']] as item (item[0])}<button class:active={tab === item[0]} type="button" aria-current={tab === item[0] ? 'page' : undefined} onclick={() => tab = item[0] as Tab}>{item[1]}</button>{/each}</nav>
