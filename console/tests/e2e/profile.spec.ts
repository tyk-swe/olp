...
test('profile updates identity, changes password, and revokes another session', async ({
  page
}) => {
  ...
  await page.route('**/api/v1/profile', async (route) => {
    if (route.request().method() === 'PATCH')
      displayName = (route.request().postDataJSON() as { display_name: string })
        .display_name;
    await route.fulfill({
      json: {
        id: userId,
        ...
        active: true,
        created_at: '2026-07-01T12:00:00Z',
        etag: '01980000-0000-7000-8000-000000000020'
      }
    });
  });
  await page.route('**/api/v1/profile/password', async (route) => {
    ...
    await route.fulfill({
      json: {
        ...
        active: true,
        created_at: '2026-07-01T12:00:00Z',
        etag: '01980000-0000-7000-8000-000000000022'
      }
    });
  });
  ...
  const linkReauthentication = page.getByRole('dialog', {
  ...
  await expect(
    page.getByRole('button', { name: 'Link an OIDC identity' })
  ).toBeEnabled();
  ...
  await unlinkReauthentication
    .getByLabel('Current password')
    .fill('old password for testing');
  ...
  await expect(
    page.getByText('No OIDC identity is linked to this account.')
  ).toBeVisible();
...
test('OIDC callback markers require explicit confirmation before beginning an identity link', async ({
  page
}) => {
  ...
  await page
    .getByRole('button', { name: 'Confirm OIDC identity link' })
    .click();
  await expect(page).toHaveURL(/\/oidc-test-redirect$/);
  expect(linkStarts).toBe(1);
});

test('OIDC-only profile enrolls a local password before unlinking', async ({
  page
}) => {
  ...
  await page.route('**/api/v1/profile/password/enroll', async (route) => {
    expect(route.request().postDataJSON()).toEqual({
      new_password: 'local password for testing'
    });
    expect(route.request().headers()['if-match']).toBe(
      '"01980000-0000-7000-8000-000000000120"'
    );
  ...
  await expect(
    page.getByRole('heading', { name: 'Add a local password' })
  ).toBeVisible();
  await expect(page.getByLabel('Current password')).toHaveCount(0);
  await page.clock.fastForward(5 * 60 * 1000);
  await expect(page.getByText('Identity verification expired.')).toBeVisible();
  await expect(
    page.getByRole('button', { name: 'Verify identity with OIDC' })
  ).toBeVisible();

  await page.goto('/settings/profile?reauthenticated=password_enrollment');
  await page
    .getByLabel('New password', { exact: true })
    .fill('local password for testing');
  await page
    .getByLabel('Confirm new password')
    .fill('local password for testing');
  await page.getByRole('button', { name: 'Add local password' }).click();
  await expect(
    page.getByText('Verify your identity again before adding a local password.')
  ).toBeVisible();
  await expect(
    page.getByRole('button', { name: 'Verify identity with OIDC' })
  ).toBeVisible();
...
test('stale profile save reloads the remote version and retries with its ETag', async ({
  page
}) => {
  ...
  await page.route('**/api/v1/profile', async (route) => {
    ...
    if (request.method() === 'PATCH') {
      ...
      current = {
        ...current,
        display_name: (request.postDataJSON() as { display_name: string })
          .display_name,
  ...
  expect(
    (await new AxeBuilder({ page }).include('.concurrent-notice').analyze())
      .violations
  ).toEqual([]);
...
test('background profile refresh announces drift without clobbering dirty input', async ({
  page
}) => {
