...
test('profile updates identity, changes password, and revokes another session', async ({ page }) => {
  ...
  await page.route(/\/api\/v1\/oidc\/identities(?:\?.*)?$/, async (route) => {
    await route.fulfill({ json: { data: identities, linking_available: true } });
  });
  await page.route('**/api/v1/oidc/identities/*', async (route) => {
    const id = new URL(route.request().url()).pathname.split('/').at(-1);
  ...
  await expect(page.getByText('Password changed. Your other sessions were revoked.')).toBeVisible();
...
test('OIDC-only profile enrolls a local password before unlinking', async ({ page }) => {
  ...
  await expect(page.getByText('Local password added. Your other sessions were revoked.')).toBeVisible();
