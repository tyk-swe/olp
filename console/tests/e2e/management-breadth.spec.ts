...
const sessionOptions = {
  userId: ids.user,
  csrfToken: 'csrf-management-breadth'
};
...
test('provider studio compares redacted history and restores non-secret configuration', async ({
  page
}) => {
  ...
  await page.route('**/api/v1/providers/**', async (route) => {
    ...
    if (url.pathname.endsWith('/revisions/diff')) {
      await route.fulfill({
        json: {
          ...
          models_added: [],
          models_changed: ['gpt-test'],
          models_removed: [],
          capabilities_added: ['gpt-test/generation/openai/streaming'],
    ...
    if (url.pathname.endsWith('/restore-as-draft')) {
      ...
      await route.fulfill({
        json: { provider: current, credential_restored: false }
      });
      return;
    }
  ...
  await expect(
    page.getByRole('heading', { name: 'Provider revisions' })
  ).toBeVisible();
  await page.getByRole('button', { name: 'Compare' }).click();
  await expect(
    page.getByRole('region', { name: 'Provider revision 1 to 2 diff' })
  ).toContainText('Endpoint changed');
  await expect(
    page.getByText('gpt-test/generation/openai/streaming')
  ).toBeVisible();
  ...
  await page
    ...
    .click();
  await expect(
    page.getByText(/Current credential selection was preserved/)
  ).toBeVisible();
...
test('media job explorer stays metadata-only through list and detail', async ({
  page
}) => {
  await mockSession(page, sessionOptions);
  const job = {
  ...
  await expect(
    page.getByText('Available through the authenticated vendor API')
  ).toHaveCount(0);
