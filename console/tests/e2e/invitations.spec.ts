...
test('one-time fragment invitation creates a session without leaking its token', async ({
  page
}) => {
  let accepted = false;
  ...
  await expect(
    page.getByRole('heading', { name: 'Finish creating your account.' })
  ).toBeVisible();
...
test('expired invitation has a recoverable public state', async ({ page }) => {
  ...
  await expect(
    page.getByRole('heading', {
      name: 'This invitation can no longer be used.'
    })
  ).toBeVisible();
