...
test(
  'protected routes never render before authentication and return after login',
  { tag: '@browser' },
  async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Providers' })).toHaveCount(
      0
    );
    await expect(
...
test(
  'failed sign out does not pretend the server session ended',
  { tag: '@browser' },
  async ({ page }) => {
    await page.route('**/api/v1/sessions/current', async (route) => {
...
test(
  'sign out treats an already-absent server session as complete',
  { tag: '@browser' },
  async ({ page }) => {
    await page.route('**/api/v1/sessions/current', async (route) => {
      if (route.request().method() === 'DELETE') {
...
test(
  'login renders only discovered authentication methods',
  { tag: '@browser' },
  async ({ page }) => {
    await page.route('**/api/v1/auth/capabilities', async (route) => {
      await route.fulfill({
...
test(
  'OIDC initiation failures remain a recoverable login-page problem',
  { tag: '@browser' },
  async ({ page }) => {
    await page.route('**/api/v1/auth/capabilities', async (route) => {
      await route.fulfill({
