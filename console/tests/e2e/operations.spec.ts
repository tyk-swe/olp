...
test(
  'request explorer loading state is accessible in forced colors at 200% zoom',
  { tag: '@browser' },
  async ({ page }) => {
...
test('usage exposes pricing gaps and exact chart data accessibly', async ({
  ...
  await expect(persistence).toContainText(
    '1 partial retained-hour boundary excluded'
  );
  await expect(
...
test(
  'health and audit remain usable with forced colors, reduced motion, and 200% zoom',
  { tag: '@browser' },
  async ({ page }) => {
    await mockSession(page);
    let epochOpen = true;
