...
async function streamReply(response, responses) {
  ...
  const events = responses
    ? [
        ...
        `event: response.output_text.delta\ndata: ${JSON.stringify({ type: 'response.output_text.delta', output_index: 0, delta: reply })}\n\n`,
        `event: response.completed\ndata: ${JSON.stringify({ type: 'response.completed', response: completion })}\n\n`
      ]
    : [
        `data: ${JSON.stringify({ id: 'chatcmpl-vertical-stream', object: 'chat.completion.chunk', created: 1, model: deployment, choices: [{ index: 0, delta: { role: 'assistant', content: reply }, finish_reason: null }] })}\n\n`,
        `data: ${JSON.stringify({ id: 'chatcmpl-vertical-stream', object: 'chat.completion.chunk', created: 1, model: deployment, choices: [{ index: 0, delta: {}, finish_reason: 'stop' }], usage: chatResponse().usage })}\n\n`,
        'data: [DONE]\n\n'
      ];
...
const server = createServer(async (request, response) => {
  ...
  if (body.stream === true)
    return streamReply(response, url.pathname === azureResponses);
