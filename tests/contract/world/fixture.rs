...
use crate::harness::GatewayProcess;
use crate::harness::Server;
use crate::harness::WorkerBoundary;
use crate::harness::WorkerProcess;
...
pub(crate) async fn bootstrap_ha() -> Result<(World, GatewayProcess), String> {
    ...
    let world = bootstrap_server_at_gateway(server, None, Some(otlp)).await?;
    await_initial_routes(&world.http, &gateway.public_origin, &world.api_key).await?;
    Ok((world, gateway))
}
...
async fn bootstrap_server_at_gateway(
    ...
    let compat_provider = configure_provider(
        &management,
        json!({"name": "e2e-compat", "credential": mock_upstream::COMPAT_CREDENTIAL, "configuration": {"kind": "openai_compatible", "endpoint": format!("{}/v1/", mock.base), "auth_mode": "api_key"}}),
        mock_upstream::MODEL,
        json!([
    ...
    let azure_provider = configure_provider(
        &management,
        json!({"name": "e2e-azure", "credential": mock_upstream::AZURE_CREDENTIAL, "configuration": {"kind": "azure_openai", "endpoint": mock.base, "deployment": mock_upstream::DEPLOYMENT, "api_version": mock_upstream::API_VERSION, "auth_mode": "api_key"}}),
...
async fn configure_route(
    ...
    // Routes are drafts-first: `/api/v3/routes` is read-only and a route comes
    ...
    if !(200..300).contains(&created.status) {
        return Err(format!(
            "POST /api/v3/route-drafts returned {}: {}",
    ...
    let validated = management
        .send(
            reqwest::Method::POST,
            &format!("/api/v3/route-drafts/{draft_id}/validate"),
            None,
            None,
            Some(&etag),
        )
    ...
    if !(200..300).contains(&validated.status) {
        return Err(format!(
            "POST /api/v3/route-drafts/{draft_id}/validate returned {}: {}",
    ...
    let activated = management
        .send(
            reqwest::Method::POST,
            &format!("/api/v3/route-drafts/{draft_id}/activate"),
            None,
            None,
            Some(&etag),
        )
    ...
    if !(200..300).contains(&activated.status) {
        return Err(format!(
            "POST /api/v3/route-drafts/{draft_id}/activate returned {}: {}",
