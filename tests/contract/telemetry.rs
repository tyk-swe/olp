...
fn every_request_is_recorded_exactly_once() {
    runtime().block_on(async {
        ...
        let summary = world
            .management
            .get(&format!(
                "/api/v3/usage/summary?start={start}&end={end}&api_key_id={}",
                key.id
            ))
            .await
        ...
        assert_eq!(
            summary.status, 200,
            "GET /api/v3/usage/summary returned {}: {}",
...
fn missing_upstream_usage_is_incomplete_and_unpriced_never_zero() {
    ...
    runtime().block_on(async {
        ...
        let completeness = world
            .management
            .get(&format!(
                "/api/v3/usage/completeness?start={start}&end={end}&api_key_id={}",
                key.id
            ))
            .await
        ...
        assert_eq!(
            ...
            "GET /api/v3/usage/completeness returned {}: {}",
...
fn trace_allowlists_match_the_engine() {
    assert_eq!(
        ...
        olp::inference::tracing::REQUEST_ATTRIBUTE_KEYS
    );
    assert_eq!(
        ATTEMPT_TRACE_ATTRIBUTE_KEYS,
        olp::inference::tracing::ATTEMPT_ATTRIBUTE_KEYS
    );
}
