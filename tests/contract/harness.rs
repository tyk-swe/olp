...
impl LaunchGuard {
    ...
    async fn prepare(
        ...
        support::write_secret(
            &master_key_file,
            &serde_json::json!({
                "active_version": 1,
                "keys": [{"version": 1, "key": support::random_base64_secret()}]
            })
            .to_string(),
        )?;
        ...
        for attempt in 1..=3 {
            ...
            if attempt == 1 {
                support::run_migrate(binary, &environment, database)?;
            }
...
impl Server {
    ...
    async fn launch_process(
        ...
        let run_token = std::env::var("OLP_E2E_RUN_TOKEN")
            .map_err(|_| "OLP_E2E_RUN_TOKEN is unset; run via scripts/integration.sh".to_owned())?;
        if run_token.len() != 10
            || !run_token
                .bytes()
...
/// exit destructor. `scripts/integration.sh` owns that case with its
