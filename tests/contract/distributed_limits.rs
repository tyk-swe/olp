...
async fn await_budget(
    ...
    loop {
        let response = world()
            .management
            .get(&format!("/api/v3/api-keys/{api_key_id}"))
            .await
...
async fn await_daily_budget_rejection(api_key_id: &str, limit: Decimal) {
    ...
    let world = world();
    let pool = olp::database::connect(&world.database_url, 1)
        .await
        .expect("budget counter PostgreSQL connection");
    let namespace = olp::limits::valkey::valkey_keyspace(&pool)
        .await
...
fn a_priced_request_exhausts_a_daily_budget_before_the_second_request() {
    runtime().block_on(async {
        ...
        let first = world
            .gateway_post("/v1/chat/completions", request(), &key.secret)
            .await
