...
fn management_requires_a_session() {
    runtime().block_on(async {
        ...
        let response = management
            .raw(reqwest::Method::GET, "/api/v3/providers", None, &[])
            .await
            .expect("unauthenticated provider listing");
        assert_problem("GET /api/v3/providers without a session", 401, &response);
    });
}
...
fn management_mutations_require_a_csrf_token() {
    ...
    runtime().block_on(async {
        ...
        assert_problem("POST /api/v3/api-keys without a CSRF token", 403, &response);
    });
}
...
fn management_mutations_reject_a_foreign_origin() {
    runtime().block_on(async {
        ...
        assert_problem(
            "POST /api/v3/api-keys from a foreign Origin",
            403,
            &response,
        );
    });
    ...
    runtime().block_on(async {
        ...
        let response = world
            ...
            .send(
                ...
                &format!("/api/v3/providers/{}", world.compat_provider),
                Some(json!({"name": "renamed by a stale writer", "configuration": {"auth_mode": "api_key", "endpoint": format!("{}/v1/", world.mock.base), "kind": "openai_compatible"}})),
                None,
                Some(stale),
            )
            .await
        ...
        assert_problem(
            "PATCH /api/v3/providers/{id} with a stale ETag",
            412,
            &response,
        );
    });
...
fn replaying_an_idempotency_key_with_a_different_body_is_refused() {
    ...
    runtime().block_on(async {
        ...
        assert_problem(
            "POST /api/v3/api-keys replaying an Idempotency-Key with a new body",
            409,
            &second,
        );
        ...
        let listing = world()
            .management
            .get("/api/v3/api-keys?limit=100")
            .await
            .expect("api key listing");
        assert_eq!(
            listing.status, 200,
            "GET /api/v3/api-keys returned {}: {}",
    ...
    runtime().block_on(async {
        let response = world()
            ...
            .send(
                reqwest::Method::POST,
                "/api/v3/providers",
                Some(json!({"name": "", "credential": "", "configuration": {"kind": "openai_compatible", "endpoint": "not-a-url", "auth_mode": "api_key"}})),
                None,
                None,
            )
            .await
        ...
        assert_problem(
            "POST /api/v3/providers with an invalid draft",
            422,
            &response,
        );
