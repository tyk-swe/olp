    ...
    runtime().block_on(async {
        ...
        let created = management
            .expect(
                reqwest::Method::POST,
                "/api/v3/providers",
                Some(json!({"name": "e2e-probe-freshness", "credential": mock_upstream::COMPAT_CREDENTIAL, "configuration": {"kind": "openai_compatible", "endpoint": format!("{}/v1/", world.mock.base), "auth_mode": "api_key"}})),
                Some(&key),
                None,
                201,
            )
        ...
        let reviewed = management
            .expect(
                ...
                &format!("/api/v3/providers/{provider_id}/models/{model_row}"),
                Some(json!({
                    "enabled": true,
                    "capabilities": [
        ...
        let certify = management
            .send(
                reqwest::Method::POST,
                &format!("/api/v3/providers/{provider_id}/models/{model_row}/certify"),
                None,
                None,
                Some(&etag),
            )
