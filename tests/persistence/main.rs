//! Persistence integration tests. The PostgreSQL and
//! Valkey suites stay `#[ignore]`d here and run via `make integration`
//! (`scripts/integration.sh`), which runs them with Cargo
//! with one database per test provisioned by `olp::test_support`.
