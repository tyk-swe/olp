...
async fn fallback_uses_current_keys_and_release_exact_provider_transport_config() {
    let db = olp::test_support::TestDb::create_migrated("runtime_fallback").await;
    ...
    assert_eq!(
        olp::providers::runtime::runtime_provider_configurations(&(pool), &historical)
            .await
            .unwrap()
            .len(),
        1
    ...
    .await
    .unwrap();
    let historical_secret =
        olp::providers::runtime::runtime_provider_configurations(&pool, &historical)
            .await
            .unwrap();
    assert_eq!(
        historical_secret[0].configuration.endpoint.as_deref(),
    ...
    let current = olp::runtime::publication::compiler::current_runtime_api_keys(&pool)
        .await
        .unwrap();
