...
async fn idle_production_consumer_processes_after_a_full_blocking_read() {
    let db = olp::test_support::TestDb::create_migrated("metadata_idle_block").await;
    let pool = db.pool(5).await;
    let fixture = fixture(&pool, "idle-production-block").await;
    let stream = stream("idle-production-block");
    let mut connection = valkey_connection().await;
    let (shutdown, receiver) = watch::channel(false);
    let consumer_store = pool.clone();
    let consumer_stream = stream.clone();
    let consumer = tokio::spawn(async move {
        olp::usage::queue::run_request_metadata_consumer(
            &consumer_store,
            &valkey_url(),
            &consumer_stream,
...
async fn three_workers_distribute_new_events_and_checkpoint_group_wide_drain() {
    let db = olp::test_support::TestDb::create_migrated("metadata_ha").await;
    let pool = db.pool(12).await;
    let fixture = fixture(&pool, "three-workers").await;
    ...
    wait_for_usage_facts(&pool, 24).await;
