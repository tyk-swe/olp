...
async fn reconnect_resumes_own_pending_before_stale_idle_and_survivor_recovers_dead_owner() {
    let db = olp::test_support::TestDb::create_migrated("metadata_reconnect").await;
    let pool = db.pool(8).await;
    let fixture = fixture(&pool, "reconnect").await;
    ...
    wait_for_usage_facts(&pool, 1).await;
    ...
    wait_for_group_drain(&pool, &mut connection, &stream).await;
    ...
    let mut persistence_lock = pool.begin().await.unwrap();
    ...
    wait_for_usage_facts(&pool, 3).await;
    wait_for_group_drain(&pool, &mut connection, &stream).await;
    stop_consumers(shutdown, vec![survivor]).await;
    ...
    persistence_lock.rollback().await.unwrap();
    wait_for_usage_facts(&pool, 4).await;
...
async fn commit_before_ack_replays_as_duplicate_under_two_concurrent_claimants() {
    let db = olp::test_support::TestDb::create_migrated("metadata_commit").await;
    let pool = db.pool(10).await;
    let fixture = fixture(&pool, "commit-replay").await;
    ...
    let persisted = olp::usage::ingestion::persistence::persist_request_metadata_stream_event(
        &pool, &event, &payload,
    )
    .await
    .unwrap();
    ...
    let duplicate = olp::usage::ingestion::persistence::persist_request_metadata_stream_event(
        &pool, &event, &payload,
    )
    .await
    .unwrap();
    ...
    let second = olp::usage::ingestion::persistence::persist_request_metadata_stream_event(
        &pool,
        &second_event,
        &second_payload,
    )
...
async fn slow_active_entry_is_not_stolen_below_idle_threshold() {
    let db = olp::test_support::TestDb::create_migrated("metadata_idle").await;
    let pool = db.pool(8).await;
    let fixture = fixture(&pool, "idle-threshold").await;
...
async fn concurrent_recovery_records_malformed_and_invalid_entries_once_then_drains() {
    let db = olp::test_support::TestDb::create_migrated("metadata_invalid").await;
    let pool = db.pool(10).await;
    let fixture = fixture(&pool, "invalid-recovery").await;
