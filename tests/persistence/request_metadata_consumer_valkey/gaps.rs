...
async fn a_vanished_own_pending_payload_is_recorded_as_a_missing_stream_event() {
    let db = olp::test_support::TestDb::create_migrated("metadata_vanished").await;
    let pool = db.pool(5).await;
    let fixture = fixture(&pool, "vanished").await;
    ...
    wait_for_gap(&pool, "missing_stream_event").await;
...
async fn reclaim_gaps_are_recorded_even_when_the_activity_counter_write_fails() {
    let db = olp::test_support::TestDb::create_migrated("metadata_reclaim").await;
    let pool = db.pool(5).await;
    let fixture = fixture(&pool, "reclaim-gap").await;
...
async fn reclaim_gap_survives_a_postgres_reporting_failure() {
    let db = olp::test_support::TestDb::create_migrated("metadata_reclaim_retry").await;
    let pool = db.pool(5).await;
    let fixture = fixture(&pool, "reclaim-retry").await;
