...
async fn route_targets_survive_a_provider_revision_that_dropped_their_model() {
    let db = olp::test_support::TestDb::create_migrated("route_history").await;
    let pool = db.pool(5).await;
    let actor = owner_id(&pool, "route-history").await;
    let (provider_id, primary, secondary, _) =
        insert_two_model_provider(&pool, actor, "history").await;
    let (route_id, revision_id, draft_id) =
        insert_route(&pool, actor, "history", &[primary, secondary]).await;

    let before = olp::routes::revisions::get_route_revision(&pool, route_id, revision_id)
        .await
        .unwrap();
    ...
    let narrowed = insert_provider_revision(&pool, actor, provider_id, 2, &[primary]).await;
    ...
    let after = olp::routes::revisions::get_route_revision(&pool, route_id, revision_id)
        .await
        .unwrap();
    assert_eq!(
        after.targets.len(),
    ...
    let draft = olp::routes::repository::get_route_draft(&pool, draft_id)
        .await
        .unwrap();
    assert_eq!(
        draft.targets.len(),
    ...
    .await
    .unwrap();
    let disabled = olp::routes::revisions::get_route_revision(&pool, route_id, revision_id)
        .await
        .unwrap();
    assert_eq!(disabled.targets.len(), 2);
    assert!(disabled.targets.iter().all(|target| !target.available));
    assert_eq!(
        olp::routes::repository::get_route_draft(&(pool), draft_id)
            .await
            .unwrap()
            .targets
            .len(),
...
async fn activating_a_provider_that_orphans_a_live_route_target_names_it() {
    let db = olp::test_support::TestDb::create_migrated("orphan_target").await;
    let pool = db.pool(5).await;
    let actor = owner_id(&pool, "orphan-target").await;
    let (provider_id, primary, secondary, etag) =
        insert_two_model_provider(&pool, actor, "orphan").await;
    insert_route(&pool, actor, "orphan", &[primary, secondary]).await;
    ...
    let error = olp::providers::lifecycle::activate_provider(
        &pool,
