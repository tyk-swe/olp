...
async fn a_rejected_public_auth_attempt_still_consumes_the_wider_buckets() {
    let db = olp::test_support::TestDb::create_migrated("auth_admit").await;
...
async fn invitations_do_not_outlive_the_inviter_and_expiry_is_not_revocation() {
    let db = olp::test_support::TestDb::create_migrated("invitations").await;
    let pool = db.pool(5).await;
    let master_key = MasterKey::new(1, [17; 32]);
    let founder = owner_id(&pool, "invitations").await;
    ...
    let Outcome::Executed { value: granted, .. } =
        olp::access::identity::invitations::create_invitation(
            &pool,
            &olp::database::RequestProvenance::default(),
            NewInvitation {
    ...
    olp::access::identity::accounts::update_user_access(
        &pool,
        &olp::database::RequestProvenance::default(),
        co_owner.id,
        Some(Role::Viewer),
    ...
    .await
    .unwrap();
    let error = olp::access::identity::invitations::accept_invitation(
        &pool,
        &olp::database::RequestProvenance::default(),
        AcceptInvitation {
