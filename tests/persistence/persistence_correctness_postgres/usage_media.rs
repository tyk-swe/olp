...
async fn usage_hour_buckets_stay_utc_under_a_half_hour_session_timezone() {
    let db = olp::test_support::TestDb::create_migrated("usage_tz").await;
    {
    ...
    let actor = owner_id(&pool, "usage-tz").await;
    ...
    sqlx::query(
        "INSERT INTO api_keys (id, lookup_id, secret_digest, name, created_by) \
         VALUES ($1, 'olpv3usagetz', $2, 'usage tz', $3)",
    )
    .bind(api_key_id)
    ...
    let report = olp::usage::retention::run_maintenance(&skewed, Utc::now())
        .await
        .unwrap();
    ...
    let coverage = olp::usage::reports::series::usage_series(
        &pool,
        &Filters {
    ...
    let mut transaction = skewed.begin().await.unwrap();
...
async fn a_polled_media_job_is_neither_stale_nor_backed_off() {
    let db = olp::test_support::TestDb::create_migrated("media_recon").await;
    let pool = db.pool(5).await;
    let actor = owner_id(&pool, "media-recon").await;
    let provider_id = Uuid::now_v7();
    ...
    sqlx::query(
        "INSERT INTO api_keys (id, lookup_id, secret_digest, name, created_by) \
         VALUES ($1, 'olpv3recon01', $2, 'recon', $3)",
    )
    .bind(api_key_id)
    ...
    .await
    .unwrap();
    let attached = olp::media::jobs::lifecycle::attach_media_job_upstream(
        &pool,
        job_id,
        "upstream-1",
        MediaJobUpdate {
    ...
    let refreshed = olp::media::jobs::lifecycle::refresh_media_job(
        &pool,
        job_id,
        MediaJobUpdate {
    ...
    .unwrap();
    assert_eq!(
        olp::media::jobs::reconciliation::media_reconciliation_summary(&(pool), Utc::now())
            .await
            .unwrap()
            .stale,
        0,
    ...
    for _ in 0..3 {
        let claimed =
            olp::media::jobs::reconciliation::claim_media_reconciliation_jobs(&pool, Utc::now(), 4)
                .await
                .unwrap();
    ...
    assert_eq!(
        olp::media::jobs::queries::media_job(&(pool), job_id)
            .await
            .unwrap()
