...
pub(super) async fn exercise(
    ...
    sqlx::query(
        "INSERT INTO request_metadata_event_receipts \
         ...
         VALUES ($1, $2, decode(repeat('00', 32), 'hex'), 'pending', $3)",
    )
    ...
    sqlx::query(
        "INSERT INTO request_metadata_event_receipts \
         ...
         VALUES ($1, $2, decode(repeat('00', 32), 'hex'), 'pending', now() - interval '8 days', \
                 now() - interval '8 days')",
    )
    ...
    sqlx::query(
        "INSERT INTO request_metadata_event_receipts \
         (event_id, request_id, event_sha256, status, observed_at, recorded_at) \
         VALUES ($1, $2, decode(repeat('00', 32), 'hex'), 'pending', now() + interval '100 years', \
                 now() - interval '8 days')",
    )
    ...
    .await
    .unwrap();
    let mut concurrent_usage = pool.begin().await.unwrap();
    sqlx::query(
        "INSERT INTO attempt_usage_facts \
         (attempt_id, event_id, request_id, request_started_at, api_key_id, provider_id, route_slug, \
          upstream_model, operation, surface, observed_at, unpriced, usage_complete, attempt_ordinal, charge_status, usage_observed, \
          ...
          request_incomplete_counted, provider_incomplete_counted, model_incomplete_counted, target_incomplete_counted) \
         VALUES ($1, $1, $2, $3, $4, $5, 'retention-race', 'mock-model', 'generation', \
                 'gemini', $3, true, true, 1, 'billable', true, \
    ...
    .await
    .unwrap();
    let maintenance_store = pool.clone();
    let maintenance = tokio::time::timeout(
        std::time::Duration::from_secs(5),
        tokio::spawn(async move {
            olp::usage::retention::run_maintenance(&maintenance_store, Utc::now()).await
        }),
    )
    .await
    ...
    let archived_summary = olp::usage::reports::summary::usage_summary(pool, &archived_filters)
        .await
        .unwrap();
    ...
    let archived_by_key = olp::usage::reports::breakdown::usage_breakdown(
        pool,
        &archived_filters,
        Dimension::ApiKey,
        50,
    ...
    let partial_archived =
        olp::usage::reports::summary::usage_summary(pool, &partial_archived_filters)
            .await
            .unwrap();
    ...
    olp::usage::ingestion::persistence::persist_request_metadata_event(pool, &late_event)
        .await
        .unwrap();
    sqlx::query(
        "INSERT INTO request_metadata_ingestion_gaps \
    ...
    .await
    .unwrap();
    let late_maintenance = olp::usage::retention::run_maintenance(pool, Utc::now())
        .await
        .unwrap();
    ...
    let additive_usage: (i64, String, String) = sqlx::query_as(
        "SELECT provider_request_count, input_tokens::text, output_tokens::text \
    ...
    olp::usage::ingestion::persistence::persist_request_metadata_event(pool, &late_event)
        .await
        .unwrap();
    let replay_maintenance = olp::usage::retention::run_maintenance(pool, Utc::now())
        .await
        .unwrap();
    ...
    assert!(matches!(
        olp::usage::ingestion::persistence::persist_request_metadata_event(
            pool,
            &conflicting_late_event
        )
        .await,
        Err(olp::database::error::Error::InvalidRequestMetadataEvent)
    ));
    ...
    sqlx::query(
        "INSERT INTO request_metadata_event_receipts \
         (event_id, request_id, event_sha256, status, observed_at, recorded_at) \
         VALUES ($1, $2, decode(repeat('00', 32), 'hex'), 'fact_persisted', now() - interval '7 days 2 minutes', \
    ...
    .await
    .unwrap();
    olp::usage::retention::run_maintenance(pool, Utc::now() + Duration::minutes(10))
        .await
        .unwrap();
