use crate::support::route_fixtures::DraftFixture;
use crate::support::route_fixtures::LIFECYCLE_OPERATIONS;
use crate::support::route_fixtures::ProviderFixture;
use crate::support::route_fixtures::insert_provider;
use crate::support::route_fixtures::insert_provider_revision;
...
use olp::runtime::publication::compiler::RuntimeCompileError;
use sqlx::PgPool;
use sqlx::Row;
use uuid::Uuid;
...
async fn activation_revalidates_current_revisions_and_preserves_live_media_targets() {
    let db = olp::test_support::TestDb::create_migrated("route_activation_revalidation").await;
    ...
    let first = insert_provider(&pool, actor, "video-primary").await;
    let second = insert_provider(&pool, actor, "video-fallback").await;
    let (route_id, active_revision_id) = insert_active_route(&pool, actor, first.model_id).await;
    ...
    let missing_target =
        insert_route_draft(&pool, actor, active_revision_id, &[second.model_id]).await;
    let (missing_target_etag, _) = olp::routes::drafts::validate_route_draft(
        &pool,
        &olp::database::RequestProvenance::default(),
        missing_target.id,
        missing_target.etag,
    ...
    .await
    .unwrap();
    let media_job_id = insert_media_job(&pool, actor, first, route_id).await;
    let error = olp::routes::drafts::activate_route_draft(
        &pool,
        &olp::database::RequestProvenance::default(),
        missing_target.id,
        missing_target_etag,
    ...
    .await;
    let (stale_capability_etag, _) = olp::routes::drafts::validate_route_draft(
        &pool,
        &olp::database::RequestProvenance::default(),
        stale_capability.id,
        stale_capability.etag,
        actor,
    )
    .await
    .unwrap();
    ...
    .await;
    let mut preceding_publication = pool.begin().await.unwrap();
    ...
    sqlx::query("UPDATE providers SET active_revision_id = $1 WHERE id = $2")
        ...
        .await
        .unwrap();
    let activation_store = pool.clone();
    ...
    let error = olp::runtime::publication::compiler::compile_and_publish_runtime(&pool, actor)
        .await
        .unwrap_err();
    assert!(matches!(
        error,
    ...
    // the release.
    let disabled_revision = insert_provider_revision(&pool, actor, first, 3, false, &[]).await;
    sqlx::query("UPDATE providers SET active_revision_id = $1 WHERE id = $2")
    ...
    assert_eq!(
        generations, 1,
        "only the original valid media authority may be published"
    );
}
...
async fn insert_media_job(
    ...
    let upstream_model: String =
        sqlx::query("SELECT upstream_model FROM provider_models WHERE id = $1")
            ...
            .unwrap()
            .get("upstream_model");
    let generation = olp::runtime::publication::compiler::compile_and_publish_runtime(pool, actor)
        .await
        .unwrap();
    ...
    sqlx::query(
        "INSERT INTO async_media_jobs \
         (id, upstream_job_id, api_key_id, provider_id, provider_model, route_slug, \
          operation, surface, state, lifecycle_state, runtime_generation_id, provider_revision_id) \
         SELECT $1, 'upstream-video-1', $2, $3, $4, r.slug, 'video_create', \
                'openai', 'queued', 'active', $6, p.active_revision_id \
         FROM routes r JOIN providers p ON p.id = $3 WHERE r.id = $5",
    )
    .bind(job_id)
    .bind(api_key_id)
