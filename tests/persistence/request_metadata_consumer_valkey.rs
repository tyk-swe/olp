...
use olp::usage::ingestion::persistence::Outcome;
use olp::usage::queue::test_support::RequestMetadataConsumerTestPolicy;
use olp::usage::queue::test_support::run_request_metadata_consumer;
...
async fn fixture(pool: &PgPool, label: &str) -> Fixture {
    ...
    .bind(api_key_id)
    .bind(format!("olpv3{}", Uuid::now_v7().simple()))
...
) -> JoinHandle<Result<(), olp::limits::valkey::Error>> {
...
async fn stop_consumers(
    ...
    consumers: Vec<JoinHandle<Result<(), olp::limits::valkey::Error>>>,
) {
...
async fn wait_for_usage_facts(pool: &PgPool, expected: i64) {
    tokio::time::timeout(WAIT_TIMEOUT, async {
        let mut interval = tokio::time::interval(Duration::from_millis(10));
        loop {
            interval.tick().await;
...
async fn wait_for_group_drain(pool: &PgPool, connection: &mut MultiplexedConnection, stream: &str) {
    tokio::time::timeout(WAIT_TIMEOUT, async {
        let mut interval = tokio::time::interval(Duration::from_millis(10));
        loop {
            interval.tick().await;
...
async fn wait_for_gap(pool: &PgPool, reason: &str) {
    tokio::time::timeout(WAIT_TIMEOUT, async {
        let mut interval = tokio::time::interval(Duration::from_millis(10));
        loop {
            interval.tick().await;
