...
use olp::database::cursor::Error as OperationsError;
...
async fn remaining_management_mutations_exactly_replay_without_double_execution() {
    let db = olp::test_support::TestDb::create_migrated("management_idempotency_replay").await;
    ...
    let provider_secret = master_key
        ...
        .unwrap();
    let first_provider = olp::providers::lifecycle::create_provider_draft(
        &pool,
        &olp::database::RequestProvenance::default(),
        provider_input(
            provider_id,
    ...
    let replay_provider_secret = master_key
        ...
        .unwrap();
    let replayed_provider = olp::providers::lifecycle::create_provider_draft(
        &pool,
        &olp::database::RequestProvenance::default(),
        provider_input(
            replay_provider_id,
    ...
    .await
    .unwrap();
    let activated_provider = olp::providers::lifecycle::activate_provider(
        &pool,
        &olp::database::RequestProvenance::default(),
        provider_id,
        created_provider.etag,
    ...
    .unwrap();
    let first_route = olp::routes::drafts::create_route_draft(
        &pool,
        &olp::database::RequestProvenance::default(),
    ...
    let replayed_route = olp::routes::drafts::create_route_draft(
        &pool,
        &olp::database::RequestProvenance::default(),
        route_input(actor, provider_id, route_key, "replay-route"),
        Replayable::new(route_fingerprint, &master_key),
    ...
    assert!(matches!(
        olp::routes::drafts::create_route_draft(
            &(pool),
            &olp::database::RequestProvenance::default(),
    ...
    let rotation_secret = master_key
        ...
        .unwrap();
    let first_rotation = olp::providers::credentials::rotate_provider_credential(
        &pool,
        &olp::database::RequestProvenance::default(),
        provider_id,
    ...
    let discarded_secret = master_key
        ...
        .unwrap();
    let replayed_rotation = olp::providers::credentials::rotate_provider_credential(
        &pool,
        &olp::database::RequestProvenance::default(),
        provider_id,
        RotateCredentialInput {
    ...
    .unwrap();
    let first_pricing = olp::usage::pricing::create_pricing_revision(
        &pool,
        &olp::database::RequestProvenance::default(),
        actor,
        pricing_key,
    ...
    let replayed_pricing = olp::usage::pricing::create_pricing_revision(
        &pool,
        &olp::database::RequestProvenance::default(),
        actor,
        pricing_key,
    ...
    let concurrent_scope =
        format!("olp:3:idempotency:{actor}:pricing_revision.create:{concurrent_key}");
...
fn provider_input(
    ...
    credential_id: Uuid,
    model_id: Uuid,
    credential: olp::crypto::envelope::EncryptedSecret,
    actor: Uuid,
    idempotency_key: &str,
    name: &str,
) -> NewProviderDraft {
