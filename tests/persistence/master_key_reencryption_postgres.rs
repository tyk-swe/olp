...
async fn master_key_reencryption_is_authenticated_resumable_and_retirement_safe() {
    let db = olp::test_support::TestDb::create_migrated("master_key_reencryption").await;
    ...
    let initial = olp::crypto::rotation::master_key_encryption_status(&pool, keyring.version())
        .await
        .unwrap();
    ...
    assert!(matches!(
        olp::crypto::rotation::verify_master_key_retirement(&(pool), &keyring, 1, 2).await,
    ...
    sqlx::query("UPDATE provider_credential_versions SET ciphertext = $1 WHERE id = $2")
        ...
        .unwrap();
    assert!(matches!(
        olp::crypto::rotation::reencrypt_master_key_batch(&(pool), &keyring, 2).await,
    ...
    let first_batch = olp::crypto::rotation::reencrypt_master_key_batch(&pool, &keyring, 2)
        .await
        .unwrap();
    assert_eq!(first_batch.rows_reencrypted, 2);
    assert_eq!(
        olp::crypto::rotation::master_key_encryption_status(&(pool), keyring.version())
            .await
            .unwrap()
            .non_active_references(),
        2
    );
    let resumed_batch = olp::crypto::rotation::reencrypt_master_key_batch(&pool, &keyring, 2)
        .await
        .unwrap();
    assert_eq!(resumed_batch.rows_reencrypted, 2);
    assert_eq!(
        olp::crypto::rotation::reencrypt_master_key_batch(&(pool), &keyring, 2)
            .await
            .unwrap()
            .rows_reencrypted,
        0
    ...
    assert_eq!(
        olp::crypto::rotation::verify_master_key_retirement(&(pool), &keyring, 1, 2)
            .await
            .unwrap()
            .rows_verified,
        4
