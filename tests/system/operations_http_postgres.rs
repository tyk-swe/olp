...
use olp::observability::cache::refresh_observability_cache;
use olp::observability::router as observability_router;
use olp::observability::workers::RequestMetadataConsumerActivity;
use olp::observability::workers::WorkerTask;
use olp::observability::workers::WorkerTaskCheckpointOutcome;
...
async fn operations_http_contract_is_authorized_paginated_exact_and_metadata_only() {
    let db = olp::test_support::TestDb::create_migrated("operations_http").await;
    ...
    sqlx::query(
        "INSERT INTO api_keys
         ...
         VALUES ($1, 'olpv3oper002', $2, 'operations HTTP test', $3)",
    )
    .bind(api_key_id)
    ...
    .await
    .unwrap();
    olp::runtime::publication::outbox::acquire_runtime_outbox_leader(&pool)
        .await
        .unwrap()
        .release()
        .await
    ...
    let requests = get(&app, "/api/v3/requests?limit=1", &cookie).await;
    ...
    let detail = get(&app, &format!("/api/v3/requests/{request_id}"), &cookie).await;
    ...
    let summary = get(
        &app,
        &format!("/api/v3/usage/summary?start={start}&end={end}"),
        &cookie,
    )
    .await;
    ...
    let series = get(
        &app,
        &format!("/api/v3/usage/time-series?start={start}&end={end}&granularity=hour"),
        &cookie,
    )
    .await;
    ...
    let breakdown = get(
        &app,
        &format!("/api/v3/usage/breakdown?start={start}&end={end}&dimension=provider"),
        &cookie,
    )
    .await;
    ...
    let completeness = get(
        &app,
        &format!("/api/v3/usage/completeness?start={start}&end={end}"),
        &cookie,
    )
    .await;
    ...
    let management_ready = get(&app, "/api/v3/health/ready", &cookie).await;
    ...
    let archived_bucket = (observed_at - Duration::days(3))
        ...
        .with_nanosecond(0)
        .unwrap();
    let mut aggregate_fixture = pool.begin().await.unwrap();
    sqlx::query(
        "INSERT INTO attempt_usage_hourly (
            ...
            estimated_cost, request_unpriced_count, provider_unpriced_count,
            model_unpriced_count, target_unpriced_count, request_incomplete_count,
    ...
    let archived_full = get(
        &app,
        &format!(
            "/api/v3/usage/summary?start={archived_full_start}&end={archived_full_end}&route=archived-http"
        ),
        &cookie,
    )
    .await;
    ...
    let archived_partial = get(
        &app,
        &format!(
            "/api/v3/usage/summary?start={archived_partial_start}&end={archived_partial_end}&route=archived-http"
        ),
        &cookie,
    )
    .await;
    ...
    let archived_partial_series = get(
        &app,
        &format!(
            "/api/v3/usage/time-series?start={archived_partial_start}&end={archived_partial_end}&route=archived-http&granularity=hour"
        ),
        &cookie,
    )
    .await;
    ...
    let invalid_range = get(
        &app,
        &format!("/api/v3/usage/summary?start={end}&end={start}"),
        &cookie,
    )
    .await;
    ...
    let invalid_limit = get(&app, "/api/v3/requests?limit=0", &cookie).await;
    ...
    for endpoint in [
        "/api/v3/provider-health?window_minutes=180".to_owned(),
        "/api/v3/audit?limit=50".to_owned(),
        "/api/v3/runtime-generations?limit=50".to_owned(),
        "/api/v3/settings".to_owned(),
    ] {
        let response = get(&app, &endpoint, &cookie).await;
        assert_eq!(response.status(), StatusCode::OK, "{endpoint}");
    }

    let pricing_page = get(&app, "/api/v3/pricing/revisions?limit=1", &cookie).await;
    ...
    let older_pricing = get(
        &app,
        &format!(
            "/api/v3/pricing/revisions?limit=1&cursor={}",
    ...
    let setting = get(&app, "/api/v3/settings/retention.requests_days", &cookie).await;
    assert_eq!(setting.status(), StatusCode::OK);
    let etag = setting.headers()[header::ETAG].to_str().unwrap().to_owned();
    let updated = send(
        &app,
        Method::PUT,
        "/api/v3/settings/retention.requests_days",
        Some(json!({ "value": "31" })),
        RequestHeaders {
            cookie: Some(&cookie),
            csrf: Some(&csrf),
    ...
    let outage_uri = "/api/v3/settings/limits.valkey_unavailable";
    ...
    let audit = get(
        &app,
        "/api/v3/audit?action=setting.update&resource_id=limits.valkey_unavailable",
        &cookie,
    )
    .await;
    ...
    let unauthenticated_readiness = app
        .clone()
        .oneshot(
            Request::get("/api/v3/health/ready")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    attempts::exercise(&pool, &app, &cookie, generation_id, provider_id, api_key_id).await;
}
