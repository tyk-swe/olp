...
use chrono::Utc;
use olp::limits::valkey::Error;
use olp::runtime::publication::outbox::RuntimeOutboxState;
use olp::runtime::worker::OUTBOX_BATCH_SIZE;
use olp::runtime::worker::OutboxBatchOutcome;
use olp::runtime::worker::RuntimeHintPublication;
use olp::runtime::worker::publish_outbox_batch;
...
async fn insert_outbox_rows(pool: &PgPool, count: usize) -> Vec<TestOutboxRow> {
...
async fn three_workers_publish_each_outbox_row_once_during_ordinary_operation() {
    ...
    let pool = db.pool(8).await;
    let rows = insert_outbox_rows(&pool, 12).await;
...
async fn runtime_outbox_owner_death_before_reading_a_batch_allows_takeover() {
    ...
    let mut leader = olp::runtime::publication::outbox::acquire_runtime_outbox_leader(&pool)
        .await
        .unwrap();
    ...
    olp::database::ping(&pool).await.unwrap();
...
async fn runtime_outbox_owner_death_after_read_before_publish_leaves_the_row_available() {
    ...
    let rows = insert_outbox_rows(&pool, 1).await;
    let mut old_owner = olp::runtime::publication::outbox::acquire_runtime_outbox_leader(&pool)
        .await
        .unwrap();
...
async fn runtime_outbox_ambiguous_valkey_error_keeps_the_row_pending_for_retry() {
    ...
    let (_shutdown_sender, mut shutdown) = watch::channel(false);
    let mut first_owner = olp::runtime::publication::outbox::acquire_runtime_outbox_leader(&pool)
        .await
        .unwrap();
    ...
    let pending = olp::runtime::publication::outbox::runtime_outbox_status(&pool)
        .await
        .unwrap();
...
async fn stale_outbox_owner_failed_takeover_and_abandoned_claim_are_durable() {
    ...
    let rows = insert_outbox_rows(&pool, 1).await;
    let mut owner = olp::runtime::publication::outbox::acquire_runtime_outbox_leader(&pool)
        .await
        .unwrap();
    ...
    assert!(
        olp::runtime::publication::outbox::try_acquire_runtime_outbox_leader(&(pool),)
            .await
            .unwrap()
            .is_none()
    );
    let stale = olp::runtime::publication::outbox::runtime_outbox_status(&pool)
        .await
        .unwrap();
    ...
    let handoff = olp::runtime::publication::outbox::runtime_outbox_status(&pool)
        .await
        .unwrap();
    assert!(!handoff.owner_active);
    assert!(!handoff.ownership_abandoned());
    let clean_owner = olp::runtime::publication::outbox::try_acquire_runtime_outbox_leader(&pool)
        .await
        .unwrap()
        .expect("cleanly released leadership must be immediately available");
    let after_clean_handoff = olp::observability::workers::worker_recovery_counters(&pool)
        .await
        .unwrap();
...
async fn runtime_outbox_backlog_drains_in_bounded_oldest_first_batches() {
    ...
    let pool = db.pool(4).await;
    let rows = insert_outbox_rows(&pool, 205).await;
    let mut leader = olp::runtime::publication::outbox::acquire_runtime_outbox_leader(&pool)
        .await
        .unwrap();
    let mut publisher = RecordingPublisher::default();
...
async fn runtime_outbox_shutdown_during_publish_releases_ownership_without_marking_the_row() {
    ...
    let pool = db.pool(4).await;
    insert_outbox_rows(&pool, 1).await;
...
async fn runtime_outbox_non_shutdown_watch_change_retries_the_same_row() {
    ...
    let pool = db.pool(4).await;
    let rows = insert_outbox_rows(&pool, 2).await;
    let (shutdown_sender, mut shutdown) = watch::channel(false);
    let (started, started_receiver) = oneshot::channel();
    let mut leader = olp::runtime::publication::outbox::acquire_runtime_outbox_leader(&pool)
        .await
