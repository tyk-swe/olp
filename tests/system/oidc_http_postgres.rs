...
async fn oidc_code_flow_is_bound_validated_mapped_linked_and_session_backed() {
    let db = olp::test_support::TestDb::create_migrated("oidc_http").await;
    ...
    let initial_capabilities =
        send_empty(&app, Method::GET, "/api/v3/auth/capabilities", None, None).await;
    ...
    let tab_a = begin_login_with_request(
        &app,
        "/api/v3/oidc/login?return_to=%2Fsettings%3Ftab%3Dsecurity%23sessions",
        None,
    )
    .await;
    ...
    let tab_b = begin_login_with_request(
        &app,
        "/api/v3/oidc/login?return_to=%2Fproviders%3Fview%3Dall%23page-2",
        Some(&tab_a_cookie),
    )
    .await;
    ...
    let wrong_cookie = format!("v3.{}", "0".repeat(43));
    ...
    let blocked_unlink = send_empty_with_origin(
        &app,
        Method::DELETE,
        &format!("/api/v3/oidc/identities/{developer_identity_id}"),
        Some(&developer_cookies),
        Some(&developer_csrf),
    )
    .await;
    ...
    let unlinked = send_empty_with_origin(
        &app,
        Method::DELETE,
        &format!("/api/v3/oidc/identities/{owner_identity_id}"),
        Some(&owner_cookies),
        Some(&owner_csrf),
    )
    .await;
    ...
    for _ in 0..2 {
        ...
        assert!(flow.flow_cookie.starts_with("v3."));
    }
    let login_flow_rows: i64 =
        sqlx::query_scalar("SELECT count(*) FROM oidc_authorization_flows WHERE purpose = 'login'")
            .fetch_one(&pool)
