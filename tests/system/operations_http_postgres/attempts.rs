...
pub(super) async fn exercise(
    ...
    olp::usage::ingestion::persistence::persist_request_metadata_event(pool, &event)
        .await
        .unwrap();
    let path = format!("/api/v3/requests/{}", event.request_id);
    ...
    assert_retained_attempt_without_facts(pool, app, cookie, &event).await;
...
async fn assert_retained_attempt_without_facts(
    ...
    let response = get(
        app,
        &format!("/api/v3/requests/{}", event.request_id),
        cookie,
    )
    .await;
    assert_eq!(response.status(), StatusCode::OK);
