...
async fn update_key(
    ...
    let duplicate_policy = send(
        app,
        Method::PATCH,
        &format!("/api/v3/api-keys/{api_key_id}"),
        Some(json!({
...
async fn rotate_key(
    ...
    let rotated_key = send(
        app,
        Method::POST,
        &format!("/api/v3/api-keys/{api_key_id}/rotate"),
        None,
        Some(cookie),
        Some(csrf),
...
fn assert_secret_and_generation(body: &Value) {
    assert!(body["secret"].as_str().unwrap().starts_with("olp_v3_"));
...
async fn assert_budget_audits(app: &Router, cookie: &str, api_key_id: &str) {
    for (action, expected_count) in [
        ...
        let audit = send(
            ...
            Method::GET,
            &format!(
                "/api/v3/audit?action={action}&resource_type=api_key&resource_id={api_key_id}&outcome=success"
            ),
            None,
            Some(cookie),
            None,
...
async fn revoke_key(
    ...
    let stale_revoke = send(
        app,
        Method::POST,
        &format!("/api/v3/api-keys/{api_key_id}/revoke"),
        None,
        Some(cookie),
        Some(csrf),
