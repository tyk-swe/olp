...
pub(super) async fn exercise(
    ...
    let provider = send(
        ...
        Method::POST,
        "/api/v3/providers",
        Some(json!({"name": "openai-primary", "credential": "sk-openai-test-secret", "model": "compatible-model", "display_name": "Compatible Model", "configuration": {"kind": "openai", "auth_mode": "api_key"}})),
        Some(cookie),
        Some(csrf),
    ...
    let provider_mismatch = send(
        ...
        Method::POST,
        "/api/v3/providers",
        Some(json!({"name": "openai-changed", "credential": "sk-openai-test-secret", "model": "compatible-model", "display_name": "Compatible Model", "configuration": {"kind": "openai", "auth_mode": "api_key"}})),
        Some(cookie),
        Some(csrf),
        Some("provider-http-create-0001"),
        None,
    ...
    let provider_update = send(
        app,
        Method::PATCH,
        &format!("/api/v3/providers/{provider_id}"),
        Some(json!({"name": "openai-primary-updated", "configuration": {"auth_mode": "api_key", "kind": "openai"}})),
        Some(cookie),
        Some(csrf),
        None,
        Some(&provider_etag),
    ...
    let missing_probe_precondition = send(
        app,
        Method::POST,
        &format!("/api/v3/providers/{provider_id}/probe"),
        None,
        Some(cookie),
        Some(csrf),
        None,
    ...
    let discovery = send(
        app,
        Method::POST,
        &format!("/api/v3/providers/{provider_id}/discovery"),
        Some(json!({
            "models": [
                {
    ...
    let first_model_page = send(
        app,
        Method::GET,
        &format!("/api/v3/providers/{provider_id}/models?limit=1"),
        None,
        Some(cookie),
        None,
        None,
    ...
    let second_model_page = send(
        app,
        Method::GET,
        &format!("/api/v3/providers/{provider_id}/models?limit=1&cursor={model_cursor}"),
        None,
        Some(cookie),
        None,
        None,
    ...
    let reviewed_model = send(
        app,
        Method::PATCH,
        &format!("/api/v3/providers/{provider_id}/models/{model_id}"),
        Some(json!({
            "enabled": true,
            "capabilities": [
    ...
    let reviewed_models = send(
        app,
        Method::GET,
        &format!("/api/v3/providers/{provider_id}/models?limit=100"),
        None,
        Some(cookie),
        None,
        None,
    ...
    let eligible_inventory = send(
        app,
        Method::GET,
        "/api/v3/provider-models?enabled=true&limit=100",
        None,
        Some(cookie),
        None,
        None,
    ...
    let native_certification = send(
        app,
        Method::POST,
        &format!("/api/v3/providers/{provider_id}/models/{model_id}/certify"),
        None,
        Some(cookie),
        Some(csrf),
        None,
    ...
    let renamed_provider = send(
        ...
        Method::PATCH,
        &format!("/api/v3/providers/{provider_id}"),
        Some(json!({"name": "openai-primary-renamed", "configuration": {"auth_mode": "api_key", "kind": "openai"}})),
        Some(cookie),
        Some(csrf),
        None,
        Some(&certified_etag),
    ...
    let missing_activation_key = send(
        app,
        Method::POST,
        &format!("/api/v3/providers/{provider_id}/activate"),
        None,
        Some(cookie),
        Some(csrf),
        None,
    ...
    let rotated_provider = send(
        app,
        Method::POST,
        &format!("/api/v3/providers/{provider_id}/credentials"),
    ...
    let credential_page = send(
        app,
        Method::GET,
        &format!("/api/v3/providers/{provider_id}/credentials?limit=1"),
        None,
        Some(cookie),
        None,
        None,
    ...
    let older_credentials = send(
        app,
        Method::GET,
        &format!("/api/v3/providers/{provider_id}/credentials?limit=1&cursor={credential_cursor}"),
        None,
        Some(cookie),
        None,
        None,
    ...
    let cannot_revoke_runtime_credential = send(
        app,
        Method::POST,
        &format!("/api/v3/providers/{provider_id}/credentials/{runtime_credential_id}/revoke"),
        None,
        Some(cookie),
        Some(csrf),
    ...
    let provider_revisions = send(
        app,
        Method::GET,
        &format!("/api/v3/providers/{provider_id}/revisions?limit=1"),
        None,
        Some(cookie),
        None,
        None,
    ...
    let provider_revision_detail = send(
        app,
        Method::GET,
        &format!("/api/v3/providers/{provider_id}/revisions/{latest_provider_revision_id}"),
        None,
        Some(cookie),
        None,
        None,
    ...
    let revision_models = send(
        ...
        Method::GET,
        &format!(
            "/api/v3/providers/{provider_id}/revisions/{latest_provider_revision_id}/models?limit=1"
        ),
        None,
        Some(cookie),
        None,
    ...
    let active_credentials = send(
        app,
        Method::GET,
        &format!("/api/v3/providers/{provider_id}/credentials?limit=100"),
        None,
        Some(cookie),
        None,
        None,
...
async fn reserve_media_activation_blocker(
    ...
    sqlx::query(
        "INSERT INTO api_keys (id, lookup_id, secret_digest, name, created_by)          SELECT $1, 'olpv3httpmediaguard', $2, 'media activation guard', created_by          FROM providers WHERE id = $3",
    )
    .bind(api_key_id)
...
async fn assert_media_activation_blocked(
    ...
    let message = body["errors"]["provider"][0]["message"].as_str().unwrap();
    ...
    let provider = olp::providers::repository::get_provider(
        &state.pool,
        Uuid::parse_str(provider_id).unwrap(),
    )
    .await
    ...
    assert_eq!(
        olp::media::jobs::queries::media_job(&(state.pool), job_id)
            .await
            .unwrap()
            .lifecycle,
        MediaJobLifecycle::Active
    );
    olp::media::jobs::lifecycle::begin_media_job_deletion(&state.pool, job_id)
        .await
        .unwrap();
    assert!(
        olp::media::jobs::lifecycle::finalize_media_job_deletion(&(state.pool), job_id)
            .await
            .unwrap()
    );
}
