...
pub(super) async fn begin_login(app: &Router) -> BrowserFlow {
    begin_login_with_request(app, "/api/v3/oidc/login", None).await
}
...
pub(super) async fn callback_request(
    ...
    let cookie_name = if flow_cookie.starts_with("v3.") {
    ...
    let uri = format!("/api/v3/oidc/callback?code=mock-code&state={state}");
