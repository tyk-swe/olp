...
pub(crate) async fn await_metadata_recovery(
    ...
    loop {
        let (attempt_usage_facts, recovered, pending, lag): (i64, i64, Option<i64>, Option<i64>) =
            sqlx::query_as(
                "SELECT \
        ...
        if attempt_usage_facts == 1 && recovered >= 1 && pending == Some(0) && lag == Some(0) {
            return Ok(());
        }
        if Instant::now() >= deadline {
            return Err(format!(
                "metadata recovery did not converge: usage={attempt_usage_facts}, recovered={recovered}, pending={pending:?}, lag={lag:?}"
            ));
        }
...
/// stream (`report_processing_activity` in `usage::ingestion`), so an empty stream alone
