...
use aws_smithy_types::event_stream::Message as EventMessage;
use bytes::Bytes;
...
use olp::inference::transport::MediaSpoolError;
use olp::inference::transport::MediaUpload;
use olp::inference::transport::OpenedMedia;
...
use olp::protocols::canonical::results::MediaArtifact;
use olp::providers::runtime_model::ProviderKind;
use olp::providers::test_support::local_provider;
...
impl MediaSpool for InlineMediaSpool {
    fn put<'a>(
        &'a self,
        _upload: MediaUpload,
    ) -> olp::inference::transport::BoxFuture<'a, Result<MediaArtifact, MediaSpoolError>> {
    ...
    ) -> olp::inference::transport::BoxFuture<'a, Result<OpenedMedia, MediaSpoolError>> {
    ...
    ) -> olp::inference::transport::BoxFuture<'a, Result<(), MediaSpoolError>> {
        Box::pin(async { Err(MediaSpoolError::Unavailable) })
    }
}
