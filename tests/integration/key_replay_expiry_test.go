...
func TestExpiredKeyCreationStillReplaysItsOriginalResult(t *testing.T) {
	...
	status, created, originalHeaders := h.request(owner, "POST", "/api/v1/api-keys", input, headers)
	if status != 201 {
	...
	status, replayed, replayHeaders := h.request(owner, "POST", "/api/v1/api-keys", input, headers)
	...
	h.want(owner, "POST", "/api/v1/api-keys", input, map[string]string{"Idempotency-Key": "new-expired-create"}, 422)
