...
func TestOIDCClientChangeRequiresAnIndependentOwnerSignIn(t *testing.T) {
	...
	link := func(b *browser, claims map[string]any) {
		t.Helper()
		h.want(b, "POST", "/api/v3/profile/reauthenticate", map[string]any{"current_password": accessPassword, "purpose": "oidc_link"}, nil, 204)
		authorization := h.want(b, "POST", "/api/v3/oidc/link", nil, nil, 200)["authorization_url"].(string)
	...
	owner = pending
	if current := h.want(owner, "GET", "/api/v3/profile", nil, nil, 200); current["id"] != profile["id"] || current["role"] != "owner" {
	...
	if current := h.want(login, "GET", "/api/v3/profile", nil, nil, 200); current["id"] != profile["id"] || current["role"] != "owner" {
