...
func TestOIDCUnlinkPreservesAnIdentityAuthorizedByCurrentMappings(t *testing.T) {
	for _, source := range []string{"email", "groups"} {
		t.Run(source, func(t *testing.T) {
			...
			member := &browser{}
			authorize(member, "/api/v1/oidc/login", map[string]any{}, mapped, 303)
			authorize(member, "/api/v1/oidc/reauthenticate", map[string]any{"purpose": "oidc_link"}, mapped, 303)
			authorize(member, "/api/v1/oidc/link", nil, unmapped, 303)

			identities := h.want(member, "GET", "/api/v1/oidc/identities", nil, nil, 200)["items"].([]any)
			if len(identities) != 2 {
			...
			authorize(member, "/api/v1/oidc/reauthenticate", map[string]any{"purpose": "oidc_unlink", "resource_id": mappedID}, mapped, 303)
			problem := h.want(member, "DELETE", "/api/v1/oidc/identities/"+mappedID, nil, nil, 409)
			...
			authorize(&browser{}, "/api/v1/oidc/login", map[string]any{}, unmapped, 403)
			h.want(member, "GET", "/api/v1/sessions/current", nil, nil, 401)
			authorize(member, "/api/v1/oidc/login", map[string]any{}, mapped, 303)
			authorize(member, "/api/v1/oidc/reauthenticate", map[string]any{"purpose": "oidc_unlink", "resource_id": mappedID}, mapped, 303)
			h.want(member, "DELETE", "/api/v1/oidc/identities/"+mappedID, nil, nil, 409)
			...
			h.want(member, "DELETE", "/api/v1/oidc/identities/"+mappedID, nil, nil, 204)
			login := &browser{}
			authorize(login, "/api/v1/oidc/login", map[string]any{}, unmapped, 303)
			if profile := h.want(login, "GET", "/api/v1/profile", nil, nil, 200); profile["role"] != "viewer" {
