...
func TestConfigurationPromotion(t *testing.T) {
	...
	draft := source.want(sourceOwner, "POST", "/api/v3/route-drafts", map[string]any{
	...
	currentPolicy := source.want(sourceOwner, "GET", "/api/v3/routing-policies/route-draft/"+draftID, nil, nil, 200)
	source.want(sourceOwner, "PUT", "/api/v3/routing-policies/route-draft/"+draftID, policy, withMatch(currentPolicy, idem("policy-promoted")), 200)
	draft = source.want(sourceOwner, "GET", "/api/v3/route-drafts/"+draftID, nil, nil, 200)
	activated := source.want(sourceOwner, "POST", "/api/v3/route-drafts/"+draftID+"/activate", nil, withMatch(draft, idem("activate-promoted")), 200)
	...
	if status, _ := source.browserOn(sourceUsage.URL, sourceOwner, "POST", "/api/v3/pricing/revisions", pricingBody, map[string]string{"Idempotency-Key": "pricing-promote"}); status != 201 {
		t.Fatal("pricing revision must be created", status)
	}

	exported := source.want(sourceOwner, "GET", "/api/v3/configuration/export", nil, nil, 200)
	...
	if status, out, _ := destination.request(destinationOwner, "POST", "/api/v3/configuration/plan", map[string]any{"document": keyed}, nil); status != 422 || problemCode(t, out) != "non_portable_reference" {
		t.Fatal("imported API-key restrictions must be rejected", status, out)
	}

	planBody := map[string]any{"document": document}
	planned := destination.want(destinationOwner, "POST", "/api/v3/configuration/plan", planBody, nil, 200)
	...
	if status, out, _ := destination.request(destinationOwner, "POST", "/api/v3/configuration/apply", planBody, idem("apply-no-secret")); status != 409 || problemCode(t, out) != "configuration_not_applicable" {
	...
	applied := destination.want(destinationOwner, "POST", "/api/v3/configuration/apply", applyBody, idem("apply-promote"), 200)
	...
	projects := destination.want(destinationOwner, "GET", "/api/v3/projects", nil, nil, 200)["items"].([]any)
	...
	models := destination.want(destinationOwner, "GET", "/api/v3/providers/"+providerID+"/models", nil, nil, 200)["items"].([]any)
	if len(models) == 0 {
	...
	for _, item := range destination.want(destinationOwner, "GET", "/api/v3/routes", nil, nil, 200)["items"].([]any) {
		if item.(map[string]any)["slug"] == "promoted" {
			t.Fatal("apply must never activate routes", item)
		}
	}
	stagedPolicy := destination.want(destinationOwner, "GET", "/api/v3/routing-policies/route-draft/"+stagedDraft["id"].(string), nil, nil, 200)
	...
	stagedDraftDetail := destination.want(destinationOwner, "GET", "/api/v3/route-drafts/"+stagedDraft["id"].(string), nil, nil, 200)
	...
	status, revisions := destination.browserOn(destinationUsage.URL, destinationOwner, "GET", "/api/v3/pricing/revisions", nil, nil)
	...
	audits := destination.want(destinationOwner, "GET", "/api/v3/audit", nil, nil, 200)["items"].([]any)
	found := false
	...
	replayed := destination.want(destinationOwner, "POST", "/api/v3/configuration/apply", applyBody, idem("apply-promote"), 200)
	if replayed["digest"] != applied["digest"] {
		t.Fatal("replay must return the recorded result", replayed)
	}
	providerBefore := destination.want(destinationOwner, "GET", "/api/v3/providers/"+providerID, nil, nil, 200)
	draftBefore := destination.want(destinationOwner, "GET", "/api/v3/route-drafts/"+stagedDraft["id"].(string), nil, nil, 200)
	...
	again := destination.want(destinationOwner, "POST", "/api/v3/configuration/apply", applyBody, idem("apply-again"), 200)
	...
	providerAfter := destination.want(destinationOwner, "GET", "/api/v3/providers/"+providerID, nil, nil, 200)
	draftAfter := destination.want(destinationOwner, "GET", "/api/v3/route-drafts/"+stagedDraft["id"].(string), nil, nil, 200)
	...
	stale := destination.want(destinationOwner, "GET", "/api/v3/configuration/export", nil, nil, 200)["digest"].(string)
	...
	if status, out, _ := destination.request(destinationOwner, "POST", "/api/v3/configuration/apply", conflictBody, idem("apply-stale")); status != 409 || problemCode(t, out) != "configuration_not_applicable" {
		t.Fatal("a stale expected digest must refuse", status, out)
	}
	conflictPlan := destination.want(destinationOwner, "POST", "/api/v3/configuration/plan", conflictBody, nil, 200)
	found = false
	...
	destination.want(destinationOwner, "POST", "/api/v3/configuration/apply", current, idem("apply-current"), 200)
	...
	if status, out, _ := destination.request(destinationOwner, "POST", "/api/v3/configuration/apply", kindConflict, idem("apply-kind")); status != 409 || problemCode(t, out) != "configuration_not_applicable" {
	...
	mismatchPlan := destination.want(destinationOwner, "POST", "/api/v3/configuration/plan", mismatchBody, nil, 200)
	found = false
	...
	if status, out, _ := destination.request(destinationOwner, "POST", "/api/v3/configuration/apply", mismatchBody, idem("apply-mismatch")); status != 409 || problemCode(t, out) != "configuration_not_applicable" {
		t.Fatal("a provider project change must refuse", status, out)
	}

	allToken := destination.want(destinationOwner, "POST", "/api/v3/management-tokens", map[string]any{
		"name": "promotion", "scopes": []any{"read", "configure"},
		"expires_at": time.Now().Add(time.Hour).UTC().Format(time.RFC3339),
	}, idem("token-promotion"), 201)["secret"].(string)
	if status, _ := destination.machine(allToken, "GET", "/api/v3/configuration/export", nil, nil); status != 200 {
		t.Fatal("all-project tokens must export", status)
	}
	if status, _ := destination.machine(allToken, "POST", "/api/v3/configuration/plan", planBody, nil); status != 200 {
		t.Fatal("all-project tokens must plan", status)
	}
	if status, _ := destination.machine(allToken, "POST", "/api/v3/configuration/apply", applyBody, idem("machine-apply")); status != 200 {
		t.Fatal("all-project tokens must apply", status)
	}
	scopedToken := destination.want(destinationOwner, "POST", "/api/v3/management-tokens", map[string]any{
	...
	for _, attempt := range []struct{ method, path string }{
		{"GET", "/api/v3/configuration/export"},
		{"POST", "/api/v3/configuration/plan"},
		{"POST", "/api/v3/configuration/apply"},
	} {
	...
	promoted := destination.want(destinationOwner, "GET", "/api/v3/providers/"+providerID, nil, nil, 200)
	activateScopedProvider(destination, destinationOwner, promoted)
	stagedDetail := destination.want(destinationOwner, "GET", "/api/v3/route-drafts/"+stagedDraft["id"].(string), nil, nil, 200)
	stagedActivation := destination.want(destinationOwner, "POST", "/api/v3/route-drafts/"+stagedDraft["id"].(string)+"/activate", nil, withMatch(stagedDetail, idem("activate-staged")), 200)
	...
	updatedApply := destination.want(destinationOwner, "POST", "/api/v3/configuration/apply", map[string]any{"document": updated, "secret_bindings": map[string]any{bindingKey: vendorSecret}}, idem("apply-update"), 200)
	...
	destination.want(destinationOwner, "GET", "/api/v3/routes/"+stagedRouteID, nil, nil, 200)
