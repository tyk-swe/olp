...
func TestGeminiAcceptedResourceCommitsOutliveClientDisconnect(t *testing.T) {
	...
	var ownerID string
	if err := h.Pool.QueryRow(t.Context(), `SELECT id::text FROM olp.api_keys WHERE name='gemini-interactions key'`).Scan(&ownerID); err != nil {
		t.Fatal(err)
	}
 ...
 WHERE pid<>pg_backend_pid() AND datname=current_database() AND wait_event_type='Lock' AND query LIKE $1)`, "%"+fragment+"%").Scan(&waiting)
	...
	gateCall := func(method, providerSuffix, publicPath string, body []byte, lock func(pgx.Tx)) {
		...
		if method == http.MethodPost && providerSuffix == "/v1beta/interactions" {
			waitForLock("olp.api_keys")
		} else {
			waitForLock("UPDATE olp.provider_resources SET state")
		}
		cancel()
	...
	lockOwner := func(tx pgx.Tx) {
		var id string
		if err := tx.QueryRow(t.Context(), `SELECT id::text FROM olp.api_keys WHERE id=$1 FOR UPDATE`, ownerID).Scan(&id); err != nil {
			t.Fatal(err)
		}
	}
	lockResource := func(tx pgx.Tx) {
		var id string
		if err := tx.QueryRow(t.Context(), `SELECT id::text FROM olp.provider_resources WHERE api_key_id=$1 AND kind='interaction' AND state<>'deleted' FOR UPDATE`, ownerID).Scan(&id); err != nil {
			t.Fatal(err)
		}
	}
	...
	deadline := time.Now().Add(3 * time.Second)
	for time.Now().Before(deadline) {
		err := h.Pool.QueryRow(t.Context(), `SELECT id::text,state FROM olp.provider_resources WHERE api_key_id=$1 AND kind='interaction'`, ownerID).Scan(&resourceID, &state)
	...
	defer stopRead()
	if err := h.Pool.QueryRow(readCtx, `SELECT state FROM olp.provider_resources WHERE id=$1 FOR UPDATE`, resourceID).Scan(&state); err != nil || state != "completed" {
	...
	defer stopDelete()
	if err := h.Pool.QueryRow(deleteCtx, `SELECT state FROM olp.provider_resources WHERE id=$1 FOR UPDATE`, resourceID).Scan(&state); err != nil || state != "deleted" {
		t.Fatalf("accepted delete was not tombstoned after disconnect: state=%q err=%v", state, err)
	}
	if err := h.Pool.QueryRow(t.Context(), `SELECT EXISTS(SELECT 1 FROM olp.secrets WHERE id=$1)`, resourceID).Scan(&ciphertext); err != nil || ciphertext {
