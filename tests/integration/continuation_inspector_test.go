...
func TestPlanInspectorShowsNegotiatedActionabilityWithoutCreatingWork(t *testing.T) {
	...
	allowed := h.want(owner, "POST", "/api/v3/api-keys", map[string]any{"name": "negotiated inspector", "scopes": []string{"inference"}, "allowed_routes": []string{slug}, "allow_provider_state": true}, idem(uuid.NewString()), 201)
	denied := h.want(owner, "POST", "/api/v3/api-keys", map[string]any{"name": "disabled inspector", "scopes": []string{"inference"}, "allowed_routes": []string{slug}, "allow_provider_state": false}, idem(uuid.NewString()), 201)
	...
	rejected := h.list(owner, "POST", "/api/v3/routing/simulate", input, nil, 200)[0].(map[string]any)
	...
	h.want(owner, "POST", "/api/v3/routing/simulate", input, nil, 422)
	var claims int
