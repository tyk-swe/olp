...
func TestNativeConfigurationForwardMigrationPreservesHistoryAndDigests(t *testing.T) {
	...
	h := newAccessHarnessAtInstallation(t, pool, databaseURL, installation)
	owner := h.owner()
	vendor := newVendor(t)
	created := nativeWant(t, h, owner, "POST", "/api/v3/providers", map[string]any{
		"name": "Historical native source", "model": vendorModel, "credential": vendorSecret,
		"configuration": map[string]any{"kind": "openai_compatible", "auth_mode": "api_key", "endpoint": vendor.URL + "/v1", "profile_id": "compatible-chat", "profile_revision": "1",
			"options": map[string]any{"operation_defaults": map[string]any{"generation": map[string]any{"dialect": "openai-chat", "native_options": map[string]any{"fixture_negative_zero": json.RawMessage("-0")}}}}},
	}, idem("historical-native-create"), 201)
	providerID := nativeJSONString(t, created, "id")
	path := "/api/v3/providers/" + providerID
	certifyProfileNetworkProvider(t, h, owner, providerID)
	slug, key := publishProfileNetworkRoute(t, h, owner, providerID)
	before := nativeWant(t, h, owner, "GET", path, nil, nil, 200)
	nativePath := []string{"configuration", "options", "operation_defaults", "generation", "native_options", "fixture_negative_zero"}
	if string(nativeJSONAt(t, before, nativePath...)) != "0" {
		t.Fatal("historical jsonb fixture did not establish its already-normalized value")
	}
	type stored struct{ digest, source string }
	history := map[string]stored{}
	rows, err := pool.Query(t.Context(), "SELECT id::text,sha256,snapshot::text FROM olp_go.runtime_releases")
	if err != nil {
		t.Fatal(err)
	}
	for rows.Next() {
		var id string
		var record stored
		if err := rows.Scan(&id, &record.digest, &record.source); err != nil {
			t.Fatal(err)
		}
		history[id] = record
	}
	...
	// Process restart replaces prepared-statement caches after column-type DDL.
	pool.Reset()
	if err := database.Migrate(t.Context(), pool); err != nil {
		t.Fatal("forward migration was not repeatable", err)
	}
	for id, previous := range history {
		var after stored
		if err := pool.QueryRow(t.Context(), "SELECT sha256,snapshot::text FROM olp_go.runtime_releases WHERE id=$1", id).Scan(&after.digest, &after.source); err != nil || after != previous {
			t.Fatal("forward migration rewrote a historical snapshot or digest", err)
		}
	}
	for _, column := range []struct{ table, name string }{{"providers", "configuration"}, {"provider_revisions", "configuration"}, {"runtime_releases", "snapshot"}} {
		var kind string
		if err := pool.QueryRow(t.Context(), "SELECT data_type FROM information_schema.columns WHERE table_schema='olp_go' AND table_name=$1 AND column_name=$2", column.table, column.name).Scan(&kind); err != nil || kind != "json" {
			t.Fatal("native source column did not migrate to JSON", column, err)
		}
	}
	reloaded := newAccessHarnessOn(t, pool, databaseURL)
	reloaded.refresh()
	if reloaded.Runtime.Release().Digest != previousDigest {
		t.Fatal("historical serving digest changed across native-storage migration")
	}
	after := nativeWant(t, reloaded, owner, "GET", path, nil, nil, 200)
	if string(nativeJSONAt(t, after, nativePath...)) != "0" {
		t.Fatal("migration fabricated original bytes that historical jsonb had already lost")
	}
	status, _, _ := reloaded.gateway("POST", "/v1/chat/completions", key, map[string]any{"model": slug, "messages": []any{map[string]any{"role": "user", "content": "after forward migration"}}})
	if status != 200 {
		t.Fatal("historical published route stopped serving after migration")
	}
	configuration := nativeJSONAt(t, after, "configuration")
	changed := bytes.Replace(configuration, []byte(`"fixture_negative_zero":0`), []byte(`"fixture_negative_zero":-0`), 1)
	if bytes.Equal(changed, configuration) {
		t.Fatal("new negative-zero edit was not constructed")
	}
	updated := nativeWant(t, reloaded, owner, "PATCH", path, map[string]any{"name": "Historical native source", "configuration": json.RawMessage(changed)}, nativeETag(t, after), 200)
	if string(nativeJSONAt(t, updated, nativePath...)) != "-0" {
		t.Fatal("new native values were normalized after forward migration")
	}
}
