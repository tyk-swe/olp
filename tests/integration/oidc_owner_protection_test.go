...
func TestOIDCConfigurationPreservesTheLastMappedOwner(t *testing.T) {
	for _, source := range []string{"default", "email", "groups"} {
		t.Run(source, func(t *testing.T) {
			...
			saved := h.want(bootstrap, "PUT", "/api/v3/oidc/configuration", configuration, nil, 200)
			login := func() *browser {
				b := &browser{}
				...
				if h.want(b, "GET", "/api/v3/profile", nil, nil, 200)["role"] != "owner" {
					t.Fatal("the remaining owner could not regain owner authority")
				}
				return b
			}
			owner := login()
			local := h.want(bootstrap, "GET", "/api/v3/profile", nil, nil, 200)
			h.want(owner, "PATCH", "/api/v3/users/"+local["id"].(string), map[string]any{"role": "viewer"}, etagHeader(local), 200)
			withoutSecret := maps.Clone(configuration)
			withoutSecret["client_secret"] = ""
			problem := h.want(owner, "PUT", "/api/v3/oidc/configuration", withoutSecret, etagHeader(saved), 409)
			...
			for _, role := range []string{"viewer", ""} {
				...
				h.want(owner, "PUT", "/api/v3/oidc/configuration", candidate, etagHeader(saved), 409)
			}
			...
			for _, query := range []string{
				...
				h.want(owner, "PUT", "/api/v3/oidc/configuration", configuration, etagHeader(saved), 409)
				owner = login()
				saved = h.want(owner, "PUT", "/api/v3/oidc/configuration", configuration, etagHeader(saved), 200)
			}
...
func TestOIDCSecretRemovalRequiresAnIndependentOwnerSignIn(t *testing.T) {
	...
	saved = h.want(owner, "PUT", "/api/v3/oidc/configuration", configuration, etagHeader(saved), 200)
	if saved["has_client_secret"] != false || h.want(owner, "GET", "/api/v3/oidc/configuration", nil, nil, 200)["has_client_secret"] != false {
		t.Fatal("secret removal did not delete the stored credential")
	}
	h.want(owner, "PUT", "/api/v3/settings/auth.local_login_enabled", map[string]any{"value": "false"}, etagHeader(localSetting), 409)
	localOwner := &browser{}
	...
	h.want(localOwner, "PUT", "/api/v3/oidc/configuration", configuration, etagHeader(saved), 200)
