...
use olp::access::policy::ApiKeyAuthorizationError;
...
use olp::access::policy::GatewayCapability;
use olp::access::policy::authorize_api_key;
...
use olp::protocols::canonical::events::EventSequenceError;
use olp::protocols::canonical::events::EventSequenceValidator;
...
use olp::protocols::canonical::identity::RequestMetadata;
...
use olp::protocols::canonical::requests::ExtensionError;
...
use olp::protocols::canonical::requests::OPENAI_ENDPOINT_EXTENSION;
...
use olp::routes::model::Error as RouteValidationError;
use olp::routes::model::Route;
use olp::routes::model::Target;
use olp::routes::selection::AttemptPlan;
use olp::routes::selection::RoutingError;
use olp::routes::selection::select_attempts;
use olp::routes::selection::weighted_rendezvous_score;
use olp::runtime::snapshot::Error as SnapshotValidationError;
...
fn provider(
    ...
    let target = Target {
        ...
        routing_id: TargetId::from_uuid(id(target_number)),
        provider_id,
...
fn snapshot(targets: Vec<(Provider, Target)>, max_attempts: u16) -> Snapshot {
    ...
    let route = Route {
        ...
        routing_id: RouteId::from_uuid(id(500)),
        slug: slug.clone(),
...
#[test]
fn provider_revision_is_required_and_reaches_attempts() {
    let mut runtime = snapshot(vec![provider(1, 11, 0, 1)], 1);
    let mut missing_revision = serde_json::to_value(&runtime).unwrap();
    missing_revision["providers"]
        .as_object_mut()
        .unwrap()
        .values_mut()
        .next()
        ...
        .unwrap()
        .remove("revision_id");
    assert!(serde_json::from_value::<Snapshot>(missing_revision).is_err());

    let revision_id = id(91);
    runtime.providers.values_mut().next().unwrap().revision_id = revision_id;
...
#[test]
fn persisted_routing_ids_drive_rendezvous() {
    ...
    let route = runtime
        ...
        .unwrap();
    route.routing_id = RouteId::from_uuid(id(700));
    for (target, routing_id) in route.targets.iter_mut().zip([701, 702, 703, 704]) {
        target.routing_id = TargetId::from_uuid(id(routing_id));
    }
