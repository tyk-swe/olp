...
export const SCHEMA = 'openllmproxy.dev/fidelity-source-paired-r9';
...
export const C_UPPER_INDEX = 23; // C primary d_(24), zero-based.
export const C_LOWER_INDEX = 8; // C relay d_(9), zero-based.
export const C_SLOW_UPPER_INDEX = 79; // C primary d_(80), zero-based.
export const C_SLOW_LOWER_INDEX = 48; // C relay d_(49), zero-based.
...
export const cBoundForGroup = (group) => group === SLOW_GROUP
  ? { blocks: SLOW_BLOCKS, upper_index: C_SLOW_UPPER_INDEX, lower_index: C_SLOW_LOWER_INDEX, upper_statistic: 80, lower_statistic: 49 }
  : { blocks: BLOCKS, upper_index: C_UPPER_INDEX, lower_index: C_LOWER_INDEX, upper_statistic: 24, lower_statistic: 9 };
const oldFiles = {
  ...
  pairedRunner: 'scripts/fidelity-paired-r9.mjs',
...
const PAIRED_EVIDENCE_PATH = 'docs/evidence/fidelity-performance/source-paired-v2/paired-r9.json';
...
export function makeManifest(root) {
  ...
  'r9 changed old workload, metric, margin or exact outcome');
  ...
  'r9 changed the sealed B8 seed, order or block count');
  return {
    schema: `${SCHEMA}-manifest`, method_version: 5, manifest_revision: 'profiled-C-only-one-shot-r9', attempt_id: 'source-paired-r9',
    supersedes_manifest_sha256: R8_MANIFEST_SHA256,
    previous_attempts: {
    ...
    sequential_rule: 'R6 C failed before complete numeric blocks; R7 had no C; R8 C failed during semantic gate before timed blocks. R9 is one prospective write-once C opportunity with the exact sealed B8 baseline reused. Only C9 paired primary tightens to n32 d24 one-sided alpha 0.0035001833 and slow/c1 n128 d80 alpha 0.0029626033, relay lower d9/d49; B8 and paired B signed sham retain r8 d23/d78 and lower d10/d51. Conservative four-opportunity per-metric alpha sum 0.0250512299 + 0.0100308035 + 0.0100308035 + 0.0035001833 = 0.0486130202 < 0.05, counting unused numeric looks. No across-metric familywise claim, no r9 retry. Hard C gateway absolute L is separate from sign CI.',
    ...
    reused_B8: { manifest_path: oldFiles.r8Manifest, manifest_sha256: R8_MANIFEST_SHA256,
      ...
      decision: 'pass all 254 signed B controls under original r8 manifest; no B9 capture or seed choice' },
    chronology: 'B stays at sealed M8 with exact untracked B8 reservations; E8 created B8 JSON+journal directly from M8 and strictly precedes C9; C9 descends from failed-C8 evidence PR head and bc325 with byte-identical production Go blobs; no new product change',
    C_route_contract: strictRouteContracts, C_provider_contract: strictProviderContracts,
    B_product_revision: B_PRODUCT, C_product_revision: C_PRODUCT_REVISION,
    C8_failed_evidence_head: C8_FAILED_EVIDENCE_HEAD,
    ...
    bound: {
      ...
      C_primary_ordinary: { blocks: 32, lower_order_statistic: 9, upper_order_statistic: 24,
        upper_coverage: 0.99649981665425 },
      C_primary_slow_c1: { group: SLOW_GROUP, blocks: 128, lower_order_statistic: 49,
        upper_order_statistic: 80, upper_coverage: 0.9970373966966284 },
      primary: 'paired C-minus-B d_(24)<M at n32 and d_(80)<M at slow/c1 n128; relay lower d_(9)>-M or d_(49)>-M',
    ...
    conditions: { ...r8Manifest.conditions,
      baseline: 'reuse complete B8 under its own frozen r8 manifest, exact M8 binary hash, seed, schedule and B sham; no B9',
      candidate_provider: 'exact explicit compatible-chat/1 for native and anthropic-messages/1 for translated/rejected',
      timing: 'same B8 480-block schedule and 64 requests/subrun; C9 paired capture one-shot after full fixed gate and host preflight' }
  };
}
...
export function validateOutputReservation(mode, output, root, manifest) {
  invariant(mode === 'paired', 'r9 reserves only the fixed one-shot C9 paired output');
...
export function resolvePairedProvenance(root, baselinePath, CRevision, manifest) {
  ...
  invariant(evidenceCommit === manifest.reused_B8.evidence_revision, 'C9 must reuse the single sealed E8 evidence commit');
  ...
  invariant(gitSucceeds(['merge-base', '--is-ancestor', manifest.C8_failed_evidence_head, CRevision], root), 'C9 must descend from the retained failed-C8 evidence');
...
export function verifyPairedProvenance(capture, baseline, manifest, root, baselinePath) {
  ...
  invariant(baseline.B?.revision === manifest.reused_B8.method_revision &&
    baseline.B?.binary_sha256 === capture.B?.binary_sha256, 'C9 changed sealed B8 method or binary');
...
function sourceIdentity(root, manifest, B, allowedUntracked = []) {
  ...
  invariant(unexpected.length === 0, `${B ? 'B8' : 'C9'} source tree is dirty: ${unexpected.join(', ')}`);
  ...
  invariant(command('git', ['merge-base', manifest.B_product_revision, 'HEAD'], root) === manifest.B_product_revision,
    `${B ? 'B8' : 'C9'} does not descend from historical product`);
  if (B) {
    ...
    invariant(digest(resolve(root, oldFiles.pairedRunner)) === manifest.paired_runner_sha256, 'C9 runner differs');
    invariant(gitSucceeds(['merge-base', '--is-ancestor', manifest.C8_failed_evidence_head, revision]),
      'C9 must descend from failed-C8 evidence commit');
  }
...
function validateCapture(capture, manifest, baselineOnly) {
  invariant(!baselineOnly, 'r9 is C-only and cannot record a new B baseline');
  ...
  if (!baselineOnly) {
    ...
    invariant(isDeepStrictEqual(capture.C_provider_contract, manifest.C_provider_contract), 'C provider profile overlay differs from frozen r9 manifest');
...
export function analyzePaired(capture, baseline, manifest) {
  ...
  'B8 and C9 measurement hardware, toolchain or runtime differ');
...
async function record(mode, output, manifestPath, baselinePath, Bbinary, Broot, Cbinary, Croot) {
  invariant(mode === 'paired', 'r9 is C-only; B8 must be reused without another B capture');
  ...
  if (mode === 'paired') {
    ...
    invariant(isDeepStrictEqual(providerContract, manifest.C_provider_contract), 'C provider profiles differ from frozen r9 manifest');
  }
...
function main() {
  ...
  if (action === 'freeze-manifest' && args.length === 1) {
    ...
    return console.log(`Frozen source r9 manifest: ${args[0]}`);
  }
  ...
  throw new Error('Usage: fidelity-paired-r9.mjs freeze-manifest OUT | record-paired OUT MANIFEST B8_BASELINE B8_BINARY B8_METHOD_ROOT C9_BINARY C9_ROOT | compare-paired PAIRED_CAPTURE B8_CAPTURE MANIFEST C9_REPO B8_BASELINE');
}
