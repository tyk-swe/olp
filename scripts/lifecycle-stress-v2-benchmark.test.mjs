...
import { compare, freeze, names, parse, validateNegatives, validateProfiles, validateRuns, writeFailedCapture } from './lifecycle-stress-v2-benchmark.mjs';
...
function evidence() {
  ...
  return {
    schema: 'openllmproxy.dev/lifecycle-stress-performance/v2', contract: null,
    working_tree: '', harness_sha256: 'harness', shared_harness_sha256: 'v1-shared', fixture_sha256: 'fixture',
    ...
    storage: { postgresql_server_version: '180004' },
    profiles: { video: { id: 'openai-responses', revision: '1' }, realtime: { id: 'azure-v1-responses', revision: '1' } },
    conditions: { network: 'fixture' },
...
test('complete native inventory, controls, and direct/gateway comparison pass', () => {
  ...
  assert.equal(validateProfiles(artifact.profiles), true);
  assert.deepEqual(compare(artifact, freeze(artifact)), []);
  assert.deepEqual(parse(artifact.runs.map((run) => `prefix LIFECYCLE_STRESS_V2_MEASUREMENT ${JSON.stringify(run)}`).join('\n'), 'LIFECYCLE_STRESS_V2_MEASUREMENT '), artifact.runs);
});
