#!/usr/bin/env node
// One write-once final-product verification repeat. The complete successful
// attempt-2 method, B baseline, criteria and C evidence remain immutable.
...
import { committedOnce, fileSHA256, helperPath, verifyMethod, verifyProductLock } from './fidelity-final-source-lock.mjs';

export const anchor = '29e18268ac23f7290d1881cbda3af2cc2ccab018';
export const schema = 'openllmproxy.dev/continuation-barrier-paired/v2-attempt3';
export const seed = 'OLP-BARRIER-V2-A2-2026-09-23-32x4';
// This ID belongs to the frozen criteria. The new write-once evidence has its
// own repeatAttemptId, path and method revision.
export const attemptId = 'barrier-v2-attempt2-authority-poll';
export const repeatAttemptId = 'barrier-v2-attempt3-final-product-repeat';
...
export const runnerPath = 'scripts/continuation-barrier-paired-v2-attempt3.mjs';
export const runnerTestPath = 'scripts/continuation-barrier-paired-v2-attempt3.test.mjs';
export const priorRunnerPath = 'scripts/continuation-barrier-paired-v2-attempt2.mjs';
export const priorCandidatePath = 'docs/evidence/fidelity-performance/paired-barrier-v2-attempt2/candidate.json';
export const priorCandidateJournalPath = `${priorCandidatePath}.journal.jsonl`;
export const candidatePath = 'docs/evidence/fidelity-performance/paired-barrier-v2-attempt3/candidate.json';
export const priorCandidateSHA256 = '7fe2b9a6e1f1faabcc4c779b2dd8b052c7bb2fcbb8ad1ff87719135f275b7e8c';
export const priorCandidateJournalSHA256 = '515f55e17261bd13ec33b64dc33be4447e9c2c0ab9b09dcbfb0778c43eceaceb';
...
function flagMaterialInterference(output, reason, pidText, processName) {
  check(output === candidatePath, 'Unknown attempt-3 artifact for interference flag');
...
export function reservationForArtifact(artifact) {
  return {
    schema: artifact.schema, mode: artifact.mode, attempt_id: artifact.attempt_id,
    runner_sha256: artifact.runner_sha256, criteria_sha256: artifact.criteria_sha256,
    reference_revision: artifact.reference_overlay_revision, candidate_revision: artifact.candidate_revision,
    reference_executable_sha256: artifact.builds.reference.executable_sha256,
    candidate_executable_sha256: artifact.builds.translated?.executable_sha256 ?? null,
    host_preflight_sha256: digest(JSON.stringify(artifact.host_preflight)),
    host_guard_sha256: digest(JSON.stringify(artifact.host_guard)),
    runner_pid: process.pid, runner_start_ticks: artifact.runner_start_ticks,
...
async function capture(mode, output, baselinePath) {
  check(mode === 'paired', 'Attempt 3 is a C-only verification of the passing B2 reference');
  const prescribedOutput = resolve(candidatePath);
  ...
  const candidateRevision = sourceRevision(process.cwd());
  const methodRevision = verifyMethod(runnerPath, candidateRevision);
  check(methodRevision === verifyMethod(runnerTestPath, candidateRevision) &&
    methodRevision === verifyMethod(helperPath, candidateRevision),
  'Attempt-3 method, test and product lock helper were not registered together');
  const productLock = verifyProductLock(candidateRevision);
  const priorCandidateCommit = committedOnce(priorCandidatePath, candidateRevision);
  check(priorCandidateCommit === committedOnce(priorCandidateJournalPath, candidateRevision) &&
    fileSHA256(priorCandidatePath) === priorCandidateSHA256 &&
    fileSHA256(priorCandidateJournalPath) === priorCandidateJournalSHA256,
  'The passing attempt-2 C artifact and journal changed');
  const priorCandidate = readJSON(priorCandidatePath);
  check(priorCandidate.candidate_revision === productLock.prior_product_revision &&
    validatePriorArtifact(priorCandidate, 'paired', criteria, priorCandidatePath).passed,
  'The retained bc325 candidate did not pass the frozen attempt-2 method');
  ...
  if (mode === 'paired') {
    ...
    check(baseline.mode === 'baseline' && baseline.analysis?.passed && baseline.criteria_sha256 === fileHash(criteriaPath) && baseline.runner_sha256 === fileHash(priorRunnerPath) && baseline.reference_harness_sha256 === fileHash(referenceHarness), 'Frozen B-only baseline or method is unavailable/failed');
    validatePriorArtifact(baseline, 'baseline', criteria, baselinePath);
  ...
  const buildDir = mkdtempSync(join(tmpdir(), 'olp-paired-barrier-attempt3-'));
  ...
  const artifact = {
    schema, mode, attempt_id: repeatAttemptId, started_at: new Date().toISOString(), completed_at: null,
    prior_failed_sha256: fileHash(priorFailedPath), prior_journal_sha256: fileHash(priorJournalPath),
    prior_candidate_sha256: priorCandidateSHA256, prior_candidate_journal_sha256: priorCandidateJournalSHA256,
    prior_candidate_commit: priorCandidateCommit, product_lock_commit: productLock.lock_commit,
    product_lock_sha256: productLock.lock_sha256, product_revision: productLock.product_revision,
    production_go_sha256: productLock.production_go_sha256,
    criteria_sha256: fileHash(criteriaPath), runner_sha256: fileHash(runnerPath),
    runner_test_sha256: fileHash(runnerTestPath),
    lock_helper_sha256: fileHash(helperPath),
    ...
    method_revision: methodRevision, prior_method_revision: baseline.method_revision,
  ...
  const journal = reserveCaptureJournal(output, reservationForArtifact(artifact));
...
export function validateArtifact(artifact, mode, criteria = verifyCriteria(), artifactPath = candidatePath) {
  check(mode === 'paired' && artifactPath === candidatePath, 'Attempt 3 accepts only its registered C artifact');
  ...
  check(artifact.attempt_id === repeatAttemptId && artifact.prior_failed_sha256 === fileHash(priorFailedPath) && artifact.prior_journal_sha256 === fileHash(priorJournalPath), 'Attempt 3 lost the retained first-attempt failure');
  const priorCommit = committedOnce(priorCandidatePath, artifact.candidate_revision);
  check(priorCommit === committedOnce(priorCandidateJournalPath, artifact.candidate_revision) &&
    artifact.prior_candidate_commit === priorCommit && artifact.prior_candidate_sha256 === priorCandidateSHA256 &&
    artifact.prior_candidate_journal_sha256 === priorCandidateJournalSHA256 &&
    fileSHA256(priorCandidatePath) === priorCandidateSHA256 && fileSHA256(priorCandidateJournalPath) === priorCandidateJournalSHA256,
  'Passing attempt-2 C artifact/journal changed');
  const priorCandidate = readJSON(priorCandidatePath);
  check(priorCandidate.candidate_revision === 'bc325da50c575803c771531dc4e3aab1ac203a53' &&
    validatePriorArtifact(priorCandidate, 'paired', criteria, priorCandidatePath).passed,
  'Retained attempt-2 C no longer passes its frozen method');
  const productLock = verifyProductLock(artifact.candidate_revision);
  check(artifact.product_lock_commit === productLock.lock_commit && artifact.product_lock_sha256 === productLock.lock_sha256 &&
    artifact.product_revision === productLock.product_revision && artifact.production_go_sha256 === productLock.production_go_sha256,
  'Candidate source differs from locked P10 production Go');
  ...
  check(artifact.method_revision === verifyMethod(runnerPath, artifact.candidate_revision) &&
    artifact.method_revision === verifyMethod(runnerTestPath, artifact.candidate_revision) &&
    artifact.method_revision === verifyMethod(helperPath, artifact.candidate_revision) &&
    artifact.runner_test_sha256 === fileHash(runnerTestPath) &&
    artifact.lock_helper_sha256 === fileHash(helperPath) &&
    gitBlobHash(artifact.method_revision, runnerPath) === artifact.runner_sha256 &&
    gitBlobHash(artifact.method_revision, criteriaPath) === artifact.criteria_sha256,
  'Pre-candidate attempt-3 method/test commit changed');
  ...
  if (mode === 'paired') {
    ...
    const bOnlyResult = validatePriorArtifact(bOnly, 'baseline', criteria, artifact.b_only_path);
    check(bOnlyResult.passed && bOnly.method_revision === artifact.prior_method_revision &&
      artifact.prior_method_revision !== artifact.method_revision && bOnly.reference_overlay_revision === artifact.reference_overlay_revision,
    'Committed B-only artifact is inconclusive or from a different method/reference');
  } else {
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    if (action === 'record-paired' && path === candidatePath && extra === bOnlyPath && !more) await capture('paired', path, extra);
    else if (action === 'flag-interference' && path && extra && more && detail) flagMaterialInterference(path, extra, more, detail);
    else if (action === 'compare' && path === candidatePath && !extra) {
    ...
    } else throw new Error(`Usage: record-paired ${candidatePath} ${bOnlyPath} | flag-interference ${candidatePath} <unrelated-build|unrelated-test|unrelated-compute> <pid> <process-name> | compare ${candidatePath}`);
