...
auth_wait=$(psql "$OLP_CONSOLE_E2E_DATABASE_URL" -XAt -v ON_ERROR_STOP=1 -c \
  "SELECT GREATEST(0, COALESCE(ceil(extract(epoch FROM max(window_started_at) + interval '1 minute' - clock_timestamp())), 0))::int FROM olp.auth_admission WHERE action = 'login' AND attempts >= 5")
...
before=$(psql "$OLP_CONSOLE_E2E_DATABASE_URL" -XAt -v ON_ERROR_STOP=1 -c 'SELECT count(*) FROM olp.requests')
pnpm --dir console exec playwright test --config playwright.journeys.config.ts --grep 'restored installation'
after=$(psql "$OLP_CONSOLE_E2E_DATABASE_URL" -XAt -v ON_ERROR_STOP=1 -c 'SELECT count(*) FROM olp.requests')
