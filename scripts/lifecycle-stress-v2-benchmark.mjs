...
const schema = 'openllmproxy.dev/lifecycle-stress-performance/v2';
const budgetSchema = 'openllmproxy.dev/lifecycle-stress-budget/v2';
...
const harness = 'tests/integration/lifecycle_stress_v2_performance_test.go';
const sharedHarness = 'tests/integration/lifecycle_stress_performance_test.go';
const fixture = 'tests/integration/fidelity_lifecycle_performance_test.go';
const runner = 'scripts/lifecycle-stress-v2-benchmark.mjs';
const commandArgs = [
  'test', '-mod=readonly', '-tags=integration', '-run', '^TestLifecycleStressV2Performance$',
...
const conditions = {
  ...
  authority: 'Publicly configured versioned direct OpenAI openai-responses rev1 video jobs and Azure-v1 Responses rev1 realtime; real isolated PostgreSQL, local scripted provider and bounded media spool. Concurrent 4MiB parsers use distinct equally authorized API keys because one API key may hold only one parser lease.',
...
export function validateProfiles(profiles) {
  const expected = {
    video: { id: 'openai-responses', revision: '1' },
    realtime: { id: 'azure-v1-responses', revision: '1' }
  };
  if (!isDeepStrictEqual(profiles, expected)) throw new Error('Registered serving-profile identity changed');
  return true;
}
...
export function writeFailedCapture(path, reason, result, started, loadBefore) {
  ...
  writeFileSync(failurePath, JSON.stringify({
    schema: 'openllmproxy.dev/lifecycle-stress-failed-capture/v2',
...
function identity(artifact) {
  return {
    ...
    hardware: artifact.hardware, storage: artifact.storage, profiles: artifact.profiles, conditions: artifact.conditions,
...
export function freeze(artifact) {
  ...
  validateProfiles(artifact.profiles);
  ...
  return {
    ...
    harness_sha256: artifact.harness_sha256, shared_harness_sha256: artifact.shared_harness_sha256, fixture_sha256: artifact.fixture_sha256,
...
export function compare(artifact, budget, mode = 'legacy') {
  ...
  if (artifact.working_tree !== '') throw new Error('Dirty candidate capture cannot qualify');
  validateProfiles(artifact.profiles);
  if (artifact.harness_sha256 !== budget.harness_sha256 || artifact.shared_harness_sha256 !== budget.shared_harness_sha256 || artifact.fixture_sha256 !== budget.fixture_sha256 || artifact.runner_sha256 !== budget.runner_sha256) throw new Error('V2 harness, V1 shared fixture, native duplex fixture or runner changed');
...
function record(path) {
  ...
  if (git('status', '--short') !== '') throw new Error('Clean source required before timed capture');
  const contract = process.env.OLP_LIFECYCLE_STRESS_V2_ROUTE_FIDELITY ? JSON.parse(process.env.OLP_LIFECYCLE_STRESS_V2_ROUTE_FIDELITY) : null;
  ...
  let runs, negatives, setup, profiles, summary;
  try {
    runs = parse(result.stdout, 'LIFECYCLE_STRESS_V2_MEASUREMENT ');
    // V2 reuses the frozen V1 negative-control helper verbatim. Its marker is
    // intentionally unchanged and its source hash is pinned above.
    negatives = parse(result.stdout, 'LIFECYCLE_STRESS_NEGATIVE ');
    setup = parse(result.stdout, 'LIFECYCLE_STRESS_V2_SETUP ');
    profiles = parse(result.stdout, 'LIFECYCLE_STRESS_V2_PROFILES ');
    ...
    if (profiles.length !== 1) throw new Error('Exactly one serving-profile identity record expected');
    validateProfiles(profiles[0]);
  } catch (error) {
  ...
  const artifact = {
    ...
    source_revision: git('rev-parse', 'HEAD'), working_tree: git('status', '--short'),
    harness_sha256: sha256(harness), shared_harness_sha256: sha256(sharedHarness), fixture_sha256: sha256(fixture), runner_sha256: sha256(runner),
    ...
    system_load: { before: loadBefore, after: optional('/proc/loadavg') }, storage: setup[0], profiles: profiles[0], conditions,
