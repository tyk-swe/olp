...
usage() {
  cat >&2 <<'EOF'
usage: OLP_DATABASE_URL=postgres://... OLP_CONFIRM_FRESH_VALKEY_LOSS=record-explicit-gap \
  scripts/checkpoint-lost-usage-stream.sh INCIDENT_ID EVENT_COUNT exact|lower-bound FIRST_AT LAST_AT

Records a content-free PostgreSQL ingestion gap before a fresh Valkey replaces
an unrecoverable usage Stream. Timestamps must be whole-second RFC 3339 UTC.
EOF
}
...
: "${OLP_CONFIRM_FRESH_VALKEY_LOSS:?set OLP_CONFIRM_FRESH_VALKEY_LOSS=record-explicit-gap}"
[[ $OLP_CONFIRM_FRESH_VALKEY_LOSS == record-explicit-gap ]] || {
  echo "fresh-Valkey loss confirmation did not match" >&2
  exit 2
}
...
reason="fresh_valkey_stream_loss_${precision}:${incident_id}"
...
WITH inserted AS (
  ...
  SELECT uuidv7(), 'disaster-recovery', :'event_count'::bigint,
         :'reason', :'certainty'::usage_gap_certainty,
         :'first_at'::timestamptz, :'last_at'::timestamptz
  WHERE NOT EXISTS (SELECT 1 FROM usage_ingestion_gaps WHERE reason = :'reason')
  RETURNING id
)
INSERT INTO audit_events
  (id, actor_user_id, action, resource_type, resource_id, outcome, occurred_at)
SELECT uuidv7(), NULL, 'usage.stream_loss_checkpoint', 'incident',
       :'incident_id', 'success', now()
FROM inserted;
DELETE FROM usage_consumer_health WHERE singleton;
SELECT event_count::text || ':' || certainty::text
FROM usage_ingestion_gaps WHERE reason = :'reason';
COMMIT;
SQL
)
recorded=$(printf '%s\n' "$result" | awk '/^[1-9][0-9]*:(exact|lower_bound)$/ { value=$0 } END { print value }')
[[ $recorded == "$event_count:$certainty" ]] || {
  echo "existing incident checkpoint has a different count/certainty or was not recorded" >&2
  exit 1
}
echo "usage stream loss checkpoint recorded: incident=$incident_id count=$event_count precision=$precision"
