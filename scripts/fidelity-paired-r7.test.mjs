...
import { analyzeBaseline, analyzePaired, evaluateCaptureHostInterval, evaluateHostPreflight, failureDisposition, hostInterval, makeManifest, resolvePairedProvenance, semanticGateOrder, checkedSemanticReply, SemanticFailure, sourceOrder, sourceSchedule, validateOutputReservation, validateStrictRouteContracts, validateSubrun, validateUntrackedBReservation, verifyPairedProvenance } from './fidelity-paired-r7.mjs';
...
function gitFixture(t) {
  ...
  const baseline = { schema: 'openllmproxy.dev/fidelity-source-paired-r7', mode: 'B-only', status: 'complete',
...
function capture(baselineOnly) {
  ...
  return { schema: 'openllmproxy.dev/fidelity-source-paired-r7', mode: baselineOnly ? 'B-only' : 'paired', status: 'complete',
...
test('manifest derives exactly 224 path and 30 added-latency margins from immutable B data', () => {
  ...
  assert.equal(manifest.manifest_revision, 'diagnostic-one-shot-r7');
  assert.equal(manifest.B_only_evidence_path, 'docs/evidence/fidelity-performance/source-paired-v2/baseline-r7.json');
  assert.equal(manifest.B_only_journal_path, 'docs/evidence/fidelity-performance/source-paired-v2/baseline-r7.json.journal.jsonl');
  assert.equal(manifest.paired_evidence_path, 'docs/evidence/fidelity-performance/source-paired-v2/paired-r7.json');
  ...
  assert.deepEqual([manifest.bound.lower_order_statistic, manifest.bound.upper_order_statistic], [10, 23]);
  assert.equal(manifest.bound.upper_coverage, 0.9899691964965314);
  assert.match(manifest.sequential_rule, /0\.0250512299.*0\.0350820334.*0\.05/);
...
test('r7 d10 rejects a relay lower control that old d11 would accept', () => {
  const baseline = capture(true), candidate = capture(false);
  candidate.B_only_sha256 = digest(JSON.stringify(baseline, null, 2) + '\n');
...
test('descriptive C absolute v1 failures stay visible without changing paired acceptance', () => {
  const baseline = capture(true), candidate = capture(false);
  candidate.B_only_sha256 = digest(JSON.stringify(baseline, null, 2) + '\n');
  const name = 'native_unary/c1/gateway';
  ...
  assert.equal(result.descriptive_absolute_C_vs_v1.metrics[name]['ns/op'].diagnostic_only, true);
});

test('r7 host preflight and per-block external CPU rules fail without trimming', () => {
...
test('fresh E7 evidence precedes locked C7 without inventing a product change', (t) => {
...
test('locked C7 rejects any production Go blob drift, including unrelated packages', (t) => {
  const fixture = gitFixture(t);
  fixture.addBaseline();
...
test('same commit cannot create E7 and lock C7', (t) => {
  const fixture = gitFixture(t);
...
test('missing, separately committed, or later edited E7 journal fails', (t) => {
...
test('no-ff E7 merge carries unchanged evidence and permits method-only C', (t) => {
  const fixture = gitFixture(t);
  ...
  fixture.git('merge', '-q', '--no-ff', '-m', 'Merge E7', 'evidence');
  fixture.put('internal/gateway/measure_test.go', 'package gateway\nfunc TestMeasure() {}\n');
...
test('fixed r7 output paths reject alternate attempts and existing journals', (t) => {
  const fixture = gitFixture(t);
...
test('paired B7 keeps exact reserved files untracked at measured M7', (t) => {
  const fixture = gitFixture(t);
...
test('complete synthetic B-only and paired captures pass every predeclared metric and control', () => {
  ...
  assert.deepEqual(B.count, { blocks: 384, B_subruns: 1408, B_requests: 90112, B_semantic_gate_requests: 1408 });
