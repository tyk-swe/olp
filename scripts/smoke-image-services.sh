...
args=("${olp_image_args[@]}"
  ...
  --env OLP_LISTEN_ADDR=127.0.0.1:4185 --env OLP_OBSERVABILITY_LISTEN_ADDR=127.0.0.1:4187
  --env OLP_PUBLIC_ORIGIN=http://localhost:4185)
...
for mode in all gateway control worker; do
  ...
  for _ in {1..30}; do
    ...
    elif curl --fail --silent --max-time 1 http://127.0.0.1:4187/health/live >/dev/null; then
      ready=true
      break
    fi
    sleep 1
  ...
  case "$mode" in
    all|control)
      curl --fail --silent http://127.0.0.1:4185/ | grep -qi '<html\|<!doctype html'
      [[ $(curl --silent -o /dev/null -w '%{http_code}' http://127.0.0.1:4185/_app/missing.js) == 404 ]]
      ;;
    gateway)
      [[ $(curl --silent -o /dev/null -w '%{http_code}' http://127.0.0.1:4185/api/v3/setup) == 404 ]]
      [[ $(curl --silent -o /dev/null -w '%{http_code}' http://127.0.0.1:4187/health/ready) == 503 ]]
      ;;
    worker)
      if curl --fail --silent --max-time 1 http://127.0.0.1:4187/health/live >/dev/null; then
