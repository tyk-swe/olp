...
export const SCHEMA = 'openllmproxy.dev/fidelity-source-paired-r8';
...
export const SLOW_UPPER_INDEX = 77; // d_(78), zero-based.
export const SLOW_LOWER_INDEX = 50; // d_(51), zero-based.
...
const oldFiles = {
  ...
  pairedRunner: 'scripts/fidelity-paired-r8.mjs',
...
const PAIRED_EVIDENCE_PATH = 'docs/evidence/fidelity-performance/source-paired-v2/paired-r8.json';
...
export function makeManifest(root) {
  ...
  const r7Sham = signedBValues(r7B.blocks.filter((block) => block.group === SLOW_GROUP), 'relay', 'max-inter-event-gap-p99-us');
  const r7Margin = comparisons[`${SLOW_GROUP}/relay`]['max-inter-event-gap-p99-us'].margin;
  const r7LowerSuccesses = r7Sham.filter((value) => value > -r7Margin).length;
  invariant(r7Sham.length === 32 && r7LowerSuccesses === 22, 'r7 reference-only sham power input changed');
  const schedule = sourceSchedule();
  return {
    schema: `${SCHEMA}-manifest`, method_version: 4, manifest_revision: 'signed-sham-one-shot-r8', attempt_id: 'source-paired-r8',
    supersedes_manifest_sha256: '5fcaae31ab22fb0f89c8cf43906d052e6d38b229efb5ec5a3f363933d33394aa',
    previous_invalid_attempts: {
      ...
      r7: { artifact_path: oldFiles.r7BArtifact, artifact_sha256: R7_B_ARTIFACT_SHA256,
        ...
        completed_blocks: 384, C_observations: 0, B_only_passed: false, failure: r7Failure }
    },
    sequential_rule: 'R6 remains adverse/incomplete with zero complete numeric blocks; r7 B-only failed its old absolute envelope and no r7 C numeric look occurred. R8 is one prospective write-once C opportunity. Its maximum one-sided per-metric sign alpha for the paired C-minus-B primary is 0.0100308035 (n32 d23); slow/c1 n128 d78 alpha is 0.0083353671. Conservatively counting r6 d22 alpha 0.0250512299, hypothetical unused r7 d23 alpha 0.0100308035, and r8 max alpha gives 0.0451128369 below a prospectively declared 0.05 target for one metric across three opportunities. Signed B and relay lower controls only restrict acceptance; the hard C gateway absolute median gate is separate from the sign CI. No across-metric familywise claim; no r8 retry.',
    ...
    chronology: 'paired B stays at measured r8 method commit M8 with fixed untracked reservations; a separate branch creates exact B8 JSON and journal together in normal commit E8 directly after M8; E8 is a strict ancestor of C8; C8 descends from bc325 and has byte-identical production Go blobs, with no invented product change',
    ...
    seed: SEED, blocks_per_group: Object.fromEntries(groups.map((group) => [group, blockCount(group)])), samples_per_subrun: SAMPLES,
    ...
    order: { ABBA: 'B-C-C-B per relay path; C-B-B-C per gateway path', BAAB: 'C-B-B-C per relay path; B-C-C-B per gateway path',
      outer: 'within each treatment order, 8/8 relay/gateway outer at n32 and 32/32 at n128; each paired block is an eight-subrun palindrome',
    ...
    bound: { ordinary: { blocks: 32, lower_order_statistic: 10, upper_order_statistic: 23, upper_coverage: 0.9899691964965314 },
      slow_c1: { group: SLOW_GROUP, blocks: 128, lower_order_statistic: 51, upper_order_statistic: 78, upper_coverage: 0.9916646328651513 },
      primary: 'for paired C-minus-B block means, d_(23)<M at n32 or d_(78)<M at slow/c1 n128; relay lower controls use d_(10)>-M or d_(51)>-M',
      B_sham: 'signed (B2-B1), ABBA positive and BAAB negative, for every path and added-latency metric in B-only and paired B; same group-specific upper and lower M bounds; narrow two-B order/stability control, not a four-arm placebo',
      B_and_relay_absolute: 'original v1 L comparisons are descriptive; all misses remain visible',
      C_gateway_absolute: 'every contemporary C gateway median must be <= its original immutable v1 L; separate hard gate from paired sign intervals' },
    power_estimate: { source: 'r7 B-only reference sham only, fixed artifact SHA-256', group: SLOW_GROUP,
      metric: 'relay max-inter-event-gap-p99-us lower sham side', B7_successes: r7LowerSuccesses, B7_blocks: 32,
      assumed_independent_success_probability: r7LowerSuccesses / 32, R8_blocks: 128, minimum_successes: 78,
      illustrative_one_side_probability: binomialTailAtLeast(128, 78, r7LowerSuccesses / 32),
      reported_B7_stratified_bootstrap_all_224_sham_pass: { n96: 0.931, n128: 0.971 },
      limitation: 'one-sided plug-in concerns one sham metric; reported stratified bootstrap covers only 224 path shams, not all 254 controls or paired B/C; neither predicts or guarantees B8, C8, or old-L pass' },
    ...
    conditions: {
      ...
      timing: 'after a fixed all-22 semantic gate: 64 requests/subrun, two subruns/arm/block, 128 slow/c1 blocks and 32 elsewhere; gate durations excluded; no omissions or selected retries',
      semantics: 'frozen v1 provider-bound request, complete response/SSE event oracle, exact asset, authentication and zero-dispatch rejection; C gate/subrun failures are adverse candidate results with structured details; paired B signed sham is formal'
    }
  };
}
...
function sourceIdentity(root, manifest, B, allowedUntracked = []) {
  ...
  if (B) {
    ...
    invariant(changed.every((name) => name === oldFiles.pairedHarness || name === oldFiles.pairedRunner || name === oldFiles.baseline || name === oldFiles.budgets ||
      name === oldFiles.r6Artifact || name === oldFiles.r6Journal || name === oldFiles.r7BArtifact || name === oldFiles.r7BJournal ||
      name === 'scripts/fidelity-paired-v2.mjs' || name === 'scripts/fidelity-paired-v2.test.mjs' ||
      name === 'scripts/fidelity-paired-r7.mjs' || name === 'scripts/fidelity-paired-r7.test.mjs' ||
      name === 'scripts/fidelity-paired-r8.test.mjs' || name === 'docs/evidence/fidelity-performance/source-paired-v2/manifest.json' ||
      ...
      name === oldFiles.r6Manifest || name === oldFiles.r7Manifest || name === 'docs/evidence/fidelity-performance/source-paired-v2/manifest-r8.json' ||
      name === 'docs/evidence/fidelity-performance/source-paired-v2/README.md' ||
      name === 'docs/evidence/fidelity-performance/source-paired-v2/README-r7.md' ||
      name === 'docs/evidence/fidelity-performance/source-paired-v2/README-r8.md'), `B product differs: ${changed}`);
  }
...
export function analyzeBaseline(capture, manifest) {
  ...
  const envelope = {}, absoluteVariability = {}, absoluteFailures = [];
  for (const [name, metrics] of Object.entries(manifest.comparisons)) {
    ...
    invariant(runs.length === 2 * blockCount(group), `${name}: B-only sample floor changed`);
    envelope[name] = {};
    ...
    for (const [metric, old] of Object.entries(metrics)) {
      ...
      const within = observed <= old.old_limit;
      envelope[name][metric] = { B_median: observed, old_limit: old.old_limit, within_old_limit: within, diagnostic_only: true };
      if (!within) absoluteFailures.push(`${name} ${metric}: B-only median ${observed} > frozen ${old.old_limit}`);
      ...
      const sorted = [...values].sort((a, b) => a - b);
      absoluteVariability[name][metric] = { values, median: median(values), upper: sorted[boundForGroup(group).upper_index], old_margin: old.margin, diagnostic_only: true };
    }
  }
  const sham = analyzeSignedBSham(capture, manifest);
  const subruns = capture.blocks.reduce((n, block) => n + block.subruns.length, 0);
  return { passed: sham.passed, failures: sham.failures, signed_B_sham: sham,
    descriptive_absolute_B_vs_v1: { metrics: envelope, within_old_limit: 224 - absoluteFailures.length,
      above_old_limit: absoluteFailures.length, above_old_limit_details: absoluteFailures, diagnostic_only: true },
    absolute_within_block_variability: absoluteVariability,
    count: { blocks: capture.blocks.length, B_subruns: subruns, B_requests: subruns * SAMPLES,
      B_semantic_gate_requests: 22 * SAMPLES,
      B_dispatches: capture.blocks.reduce((n, block) => n + block.subruns.reduce((sum, run) => sum + run.reply.metrics.dispatches, 0), 0),
      B_rejections: capture.blocks.reduce((n, block) => n + block.subruns.reduce((sum, run) => sum + run.reply.metrics.rejected, 0), 0) } };
}
...
export function analyzePaired(capture, baseline, manifest) {
  ...
  for (const group of groups) {
    ...
    for (const path of groupPaths(group)) {
      ...
      for (const [metric, old] of Object.entries(manifest.comparisons[name])) {
        ...
        if (!result.passed) primaries.push(`${name} ${metric}: paired d_(${boundForGroup(group).upper_statistic}) ${result.upper} >= frozen margin ${old.margin}`);
        if (path === 'relay' && !result.control_passed) controls.push(`${name} ${metric}: relay paired interval [d_(${boundForGroup(group).lower_statistic})=${result.lower}, d_(${boundForGroup(group).upper_statistic})=${result.upper}] outside (-${old.margin}, ${old.margin})`);
      }
    }
  }
  const addedLatency = {};
  for (const group of groups.filter((name) => !name.startsWith('rejected_extension/'))) {
    ...
    for (const [metric, old] of Object.entries(manifest.added_latency[group])) {
      ...
      addedLatency[group][metric] = result;
      if (!result.passed) primaries.push(`${group} added ${metric}: paired d_(${boundForGroup(group).upper_statistic}) ${result.upper} >= frozen gateway margin ${old.margin}`);
    }
  }
...
async function record(mode, output, manifestPath, baselinePath, Bbinary, Broot, Cbinary, Croot) {
  ...
  if (frozenBaseline) {
    ...
    invariant(baselineAnalysis.passed, 'frozen B-only signed sham failed; C capture is forbidden');
...
function main() {
  ...
  if (action === 'freeze-manifest' && args.length === 1) {
    ...
    return console.log(`Frozen source r8 manifest: ${args[0]}`);
  }
  ...
  throw new Error('Usage: fidelity-paired-r8.mjs freeze-manifest OUT | record-B OUT MANIFEST B_BINARY B_ROOT | record-paired OUT MANIFEST B_BASELINE B_BINARY B_ROOT C_BINARY C_ROOT | compare-B B_CAPTURE MANIFEST | compare-paired PAIRED_CAPTURE B_CAPTURE MANIFEST C_REPO B_BASELINE');
}
