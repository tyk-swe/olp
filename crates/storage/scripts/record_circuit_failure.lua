...
if active_probe or failures >= threshold then
  ...
  redis.call('HSET', KEYS[1], 'failures', threshold, 'open_until_ms', open_until_ms, 'state_revision', ARGV[5])
  redis.call('HDEL', KEYS[1], 'probe_until_ms', 'probe_token')
else
  -- Opening clamps failures to threshold, so failures during an active window re-enter the branch above.
  redis.call('HSET', KEYS[1], 'failures', failures, 'open_until_ms', 0, 'state_revision', ARGV[5])
end
