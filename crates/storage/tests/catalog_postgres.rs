...
use olp_storage::{
    CapabilityCertificationOutcome, CapabilityRecord, CatalogError, ConfigurationError,
    DiscoveredModelInput, IdempotencyOutcome, IdempotencyResponse, KeyHasher, MasterKey,
    NewApiKeyRecord, NewOwner, NewProviderDraft, NewRouteDraft, NewRouteTarget, PersistenceError,
    PgStore, ProviderModelRecord, ReplaceRouteDraftCatalogInput, ReplayableIdempotency,
    RotateApiKeyCatalogInput, RotateCredentialInput, SessionMaterial, UpdateApiKeyCatalogInput,
    UpdateProviderCatalog, credential_aad, hash_password, idempotency_fingerprint,
};
use uuid::Uuid;
...
async fn catalog_lifecycle_is_versioned_audited_and_publishes_runtime() {
    ...
    let (owner, _) = store
        .setup_owner_with_session(
            NewOwner {
                organization_name: "Catalog integration".to_owned(),
                email: "owner@catalog.test".to_owned(),
    ...
    let provider = store
        .create_provider_draft(
            NewProviderDraft {
                ...
                actor,
                idempotency_key: "provider-catalog-0001".to_owned(),
            },
            test_replay(&master_key, "provider-catalog-0001"),
    ...
    let first = store.get_provider_catalog(provider_id).await.unwrap();
    ...
    let vertex = store
        ...
        .unwrap()
        .expect_executed();
    let vertex_record = store.get_provider_catalog(vertex_id).await.unwrap();
    ...
    let route = store
        .create_route_draft(
            NewRouteDraft {
                ...
                actor,
                idempotency_key: "route-catalog-create-01".to_owned(),
            },
            test_replay(&master_key, "route-catalog-create-01"),
            empty_created_response,
        )
        .await
        .unwrap()
    ...
    let first_revision = store
        .activate_route_draft(route.id, validated_etag, actor, "route-catalog-activate-01")
        .await
        .unwrap();
    assert_eq!(first_revision.release.sequence, 3);
    let draft = store.get_route_draft_catalog(route.id).await.unwrap();
    ...
    let second_revision = store
        .activate_route_draft(
            ...
            actor,
            "route-catalog-activate-02",
        )
        .await
        .unwrap();
    let diff = store
        .diff_route_revisions_catalog(
    ...
    let first_revision_record = store
        .get_route_revision_catalog(first_revision.route_id, first_revision.revision_id)
        .await
        .unwrap();
    let restored = store
    ...
    assert!(matches!(
        store
            ...
            .await,
        Err(CatalogError::Invalid(message)) if message.contains("stable slug")
    ));
    assert_eq!(
        store
            .get_route_draft_catalog(restored.id)
            .await
            .unwrap()
            .slug,
        "default"
    ...
    let key_fingerprint = idempotency_fingerprint(&"api-key-catalog-create-01").unwrap();
    let key = store
        .create_api_key_record(
            &NewApiKeyRecord {
                ...
                expires_at: None,
                actor,
                idempotency_key: "api-key-catalog-create-01".to_owned(),
            },
    ...
    let initial_key_record = store.get_api_key_catalog(key.id).await.unwrap();
    assert!(matches!(
        store
            ...
            .await,
        Err(olp_storage::CatalogError::Invalid(_))
    ));
    ...
    let key_record = store.get_api_key_catalog(key.id).await.unwrap();
    ...
    let replacement = hasher.generate_api_key();
    let rotation_fingerprint = idempotency_fingerprint(&"api-key-catalog-rotate-01").unwrap();
    let key_rotation = store
        .rotate_api_key_catalog(
            RotateApiKeyCatalogInput {
                ...
                actor,
                idempotency_key: "api-key-catalog-rotate-01",
            },
    ...
    assert_eq!(
        store.get_provider_catalog(provider_id).await.unwrap().state,
    ...
    let restored_provider_etag = store
        ...
        .await
        .unwrap();
    let restored_provider = store.get_provider_catalog(provider_id).await.unwrap();
    ...
    store
        ...
        .await
        .unwrap();
    let pre_patch = store.get_provider_catalog(compatible_id).await.unwrap();
    ...
    let patched_etag = store
        ...
        .await
        .unwrap();
    let patched = store.get_provider_catalog(compatible_id).await.unwrap();
