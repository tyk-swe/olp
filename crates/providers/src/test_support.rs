...
use olp_domain::{BoxFuture, ProviderKind};
    ...
    let facade = match kind {
        ...
        ProviderKind::Bedrock => {
            let config = BedrockConnectorConfig::new("us-east-1")
                .and_then(|config| {
                    config.with_timeouts(crate::bedrock::ConnectorTimeouts::default())
                })
...
impl LocalProvider {
    ...
    #[must_use]
    pub fn into_transport(self) -> Arc<dyn olp_domain::ProviderTransport> {
