...
pub(crate) struct MockResponse {
    pub(crate) chunks: Vec<(Duration, Vec<u8>)>,
}

impl MockResponse {
    pub(crate) fn immediate(response: Vec<u8>) -> Self {
        Self {
...
pub(crate) async fn spawn_sequence(
    base_path: &str,
...
pub(crate) fn response(content_type: &str, body: impl AsRef<[u8]>) -> Vec<u8> {
    status_response("200 OK", content_type, body)
}

pub(crate) fn status_response(status: &str, content_type: &str, body: impl AsRef<[u8]>) -> Vec<u8> {
