use crate::domain::{
    canonical::{
        identity::Surface,
        requests::{
            MEDIA_DELETE_MISSING_IS_SUCCESS_EXTENSION, Operation, VideoCreateRequest,
            VideoJobRequest, VideoListRequest,
        },
...
async fn execute_delete(
    ...
    if response.status() == StatusCode::NOT_FOUND && reconcile_missing {
        return Ok(ProviderOutput::Result(Box::new(
            CanonicalResult::VideoDelete(crate::domain::canonical::results::VideoDeleteResult {
