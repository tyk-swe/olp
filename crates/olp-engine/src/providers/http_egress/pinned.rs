...
pub(in crate::providers) struct PinnedClientConfig {
...
#[derive(Debug, Error)]
pub(in crate::providers) enum PinnedClientError {
...
#[derive(Default)]
pub(in crate::providers) struct PinnedClientPool {
...
impl PinnedClientPool {
    pub(in crate::providers) async fn client(
        &self,
        url: &Url,
...
pub(in crate::providers) async fn one_shot_client(
    url: &Url,
...
pub(in crate::providers) fn literal_ip(url: &Url) -> Option<IpAddr> {
    url.host_str()?
        .trim_start_matches('[')
        .trim_end_matches(']')
        .parse()
