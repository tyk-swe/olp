...
use olp_storage::{ProviderRevisionCatalogRecord, ProviderRevisionDiff};
use serde::Serialize;
use utoipa::ToSchema;
use uuid::Uuid;
...
use super::{
    common::{DiffQuery, PageQuery, map_catalog, page, with_etag},
    models::ProviderModelListResponse,
    providers::ProviderCatalogResponse,
};

#[derive(Clone, Debug, Serialize, ToSchema)]
pub(super) struct ProviderRevisionResponse {
    pub id: Uuid,
...
#[derive(Clone, Debug, Serialize, ToSchema)]
pub(super) struct ProviderRevisionSummaryResponse {
    pub id: Uuid,
    pub provider_id: Uuid,
    pub revision: i32,
    pub name: String,
...
#[derive(Clone, Debug, Serialize, ToSchema)]
pub(super) struct ProviderRevisionListResponse {
    pub items: Vec<ProviderRevisionSummaryResponse>,
    pub next_cursor: Option<String>,
}

#[derive(Clone, Debug, Serialize, ToSchema)]
pub(super) struct ProviderRevisionDiffResponse {
...
#[derive(Clone, Debug, Serialize, ToSchema)]
pub(super) struct ProviderRevisionRestoreResponse {
...
pub(super) async fn list_provider_revisions(
...
pub(super) async fn get_provider_revision(
    State(state): State<ApiState>,
...
pub(super) async fn list_provider_revision_models(
    State(state): State<ApiState>,
    Path((provider_id, revision_id)): Path<(Uuid, Uuid)>,
    headers: HeaderMap,
    Query(query): Query<PageQuery>,
...
pub(super) async fn diff_provider_revisions(
    State(state): State<ApiState>,
    Path(provider_id): Path<Uuid>,
    headers: HeaderMap,
...
pub(super) async fn restore_provider_revision(
    State(state): State<ApiState>,
    Path((provider_id, revision_id)): Path<(Uuid, Uuid)>,
    headers: HeaderMap,
