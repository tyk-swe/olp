...
use olp_storage::{
    CapabilityCertificationOutcome, CapabilityRecord, CatalogError, DiscoveredModelInput,
...
use super::{
    common::{PageQuery, json, map_catalog, page, validation, with_etag},
    providers::{ProviderCatalogResponse, catalog_connector},
};

#[derive(Clone, Debug, Serialize, ToSchema)]
pub(super) struct ProviderCapabilityOptionsResponse {
...
pub(super) async fn list_provider_kind_capabilities(
...
pub(super) struct ProviderModelInventoryQuery {
...
#[derive(Clone, Debug, Serialize, ToSchema)]
pub(super) struct ProviderModelResponse {
    pub id: Uuid,
...
#[derive(Clone, Debug, Serialize, ToSchema)]
pub(super) struct ProviderModelListResponse {
    pub items: Vec<ProviderModelResponse>,
    pub next_cursor: Option<String>,
}

#[derive(Clone, Debug, Serialize, ToSchema)]
pub(super) struct ProviderModelInventoryResponse {
...
#[derive(Clone, Debug, Serialize, ToSchema)]
pub(super) struct ProviderModelInventoryListResponse {
...
pub(super) async fn list_provider_model_inventory(
    State(state): State<ApiState>,
    headers: HeaderMap,
...
pub(super) struct DiscoveredModelRequest {
    pub upstream_model: String,
    pub display_name: String,
}

#[derive(Debug, Deserialize, ToSchema)]
pub(super) struct DiscoverModelsRequest {
...
pub(super) async fn set_provider_model(
    State(state): State<ApiState>,
...
#[derive(Clone, Debug, Serialize, ToSchema)]
pub(super) struct CapabilityCertificationItemResponse {
    pub operation: String,
    pub surface: String,
    pub mode: String,
    pub succeeded: bool,
...
#[derive(Clone, Debug, Serialize, ToSchema)]
pub(super) struct CapabilityCertificationResponse {
    pub provider_id: Uuid,
    pub model_id: Uuid,
    pub status: String,
...
pub(super) async fn certify_provider_model(
    State(state): State<ApiState>,
    Path((provider_id, model_id)): Path<(Uuid, Uuid)>,
    headers: HeaderMap,
) -> Result<Response, Problem> {
...
pub(super) fn certification_item(
