...
pub(super) struct CreateProviderRequest {
    pub name: String,
    ...
    pub(super) credential: Option<WriteOnlySecret>,
    #[serde(rename = "api_key")]
    #[schema(ignore)]
    pub(super) legacy_api_key: Option<WriteOnlySecret>,
...
pub(super) fn reject_create_field(
...
pub(super) fn reject_create_cloud_fields(
    errors: &mut FieldErrors,
...
pub(super) fn require_create_auth_mode(errors: &mut FieldErrors, actual: &str, expected: &str) {
    if actual != expected {
        errors
...
#[derive(Debug, Serialize, ToSchema)]
pub(super) struct ProviderActivationResponse {
    #[schema(value_type = String, format = Uuid)]
    pub id: Uuid,
    pub state: String,
    #[schema(value_type = String, format = Uuid)]
