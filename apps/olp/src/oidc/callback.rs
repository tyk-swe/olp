...
use super::error::{
    invalid_callback, is_link_session_changed, map_oidc, map_oidc_flow_completion,
    map_token_network,
};
...
use super::session::{
    ...
    OidcCallbackState, append_cookie, authenticated_redirect, clear_flow_cookie,
    consume_login_flow_cookie, successful_redirect,
};
use crate::{
    ApiState, Problem,
...
#[utoipa::path(
    ...
    responses(
        (status = 303, description = "OIDC login or identity link completed"),
        (status = 400, description = "Invalid or rejected callback", body = Problem),
        (status = 401, description = "Session or ID token validation failed", body = Problem),
        (status = 403, description = "The link callback belongs to another authenticated session", body = Problem),
...
pub(super) async fn callback(
    ...
    let preserve_flow_cookie = result.as_ref().err().is_some_and(is_link_session_changed);
...
async fn callback_inner(
    ...
    match purpose {
        OidcFlowPurpose::Login => {
            ...
            authenticated_redirect(&material, &return_to)
        }
        OidcFlowPurpose::Link => {
            let actor_user_id = actor_user_id.ok_or_else(Problem::internal)?;
            let actor_session_id = actor_session_id.ok_or_else(Problem::internal)?;
            store
                ...
                .await
                .map_err(map_oidc_flow_completion)?;
            // Linking changes no browser session material. The exact session
            // that initiated the flow remains active and no ghost session is
            // inserted into the database.
            successful_redirect(&return_to)
        }
    }
}
...
async fn consume_callback_flow(
    ...
    let link_cookie_name = callback_state.link_cookie_name();
    let browser_binding = Zeroizing::new(
        cookies
            .get(&link_cookie_name)
            .ok_or_else(|| {
                Problem::bad_request(
    ...
    // New persisted flows are links and must match the protected flow ID and
    // exact current session in one row-locked transaction. A mismatch rejects
...
fn matching_login_callback_flow_from_cookies(
    ...
    match consume_login_flow_cookie(state, value, callback_state) {
        ...
        Err(problem) if cookies.get(&link_cookie_name).is_none() => Err(problem),
        // An abandoned login can coexist with a link flow. The exact flow ID,
        // state secret, and purpose-specific cookie select the valid flow.
        Err(_) => Ok(None),
    }
}
