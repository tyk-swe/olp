...
use crate::{
    ...
    request_cookies::{
        CSRF_COOKIE as SHARED_CSRF_COOKIE, LEGACY_OIDC_FLOW_COOKIE, LEGACY_OIDC_LOGIN_FLOW_COOKIE,
        OIDC_LINK_FLOW_COOKIE_PREFIX, OIDC_LOGIN_FLOW_COOKIE_PREFIX, RequestCookies,
        SESSION_COOKIE as SHARED_SESSION_COOKIE,
    },
};

pub(super) const SESSION_COOKIE: &str = SHARED_SESSION_COOKIE;
pub(super) const CSRF_COOKIE: &str = SHARED_CSRF_COOKIE;
...
impl OidcCallbackState {
    ...
    #[must_use]
    pub(super) fn link_cookie_name(&self) -> String {
        self.flow_id.map_or_else(
...
pub(super) fn flow_cookie_name(purpose: OidcFlowPurpose, flow_id: OidcFlowId) -> String {
    let prefix = match purpose {
        ...
        OidcFlowPurpose::Link => OIDC_LINK_FLOW_COOKIE_PREFIX,
    };
...
mod flow_tests {
    ...
    fn scoped_state_round_trips_and_selects_distinct_cookie_names() {
        ...
        assert_ne!(state.login_cookie_name(), state.link_cookie_name());
