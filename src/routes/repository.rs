...
use crate::providers::record_validation::validate_route_input;
...
pub async fn list_route_drafts(
    ...
    let rows = sqlx::query_as::<_, ListRouteDraftsRow>(
    ...
    let items = crate::routes::repository::get_route_drafts(pool, &ids).await?;
    Ok(ConfigurationPage { items, next_cursor })
}

/// Reads drafts in the order of `ids` with three queries for the whole
/// set, mirroring [`get_routes`]. Any missing id is `NotFound`.
pub async fn get_route_drafts(
    pool: &sqlx::PgPool,
    ids: &[Uuid],
) -> Result<Vec<RouteDraftRecord>, Error> {
    if ids.is_empty() {
        return Ok(Vec::new());
    }
    let header_rows = sqlx::query_as::<_, GetRouteDraftsRow>(
    ...
    for row in draft_targets(pool, ids).await? {
...
pub async fn get_route_draft(
    ...
    crate::routes::repository::get_route_drafts(pool, &[draft_id])
        .await?
        .into_iter()
        .next()
...
pub async fn replace_route_draft(
    ...
    let lineage_slug: Option<String> = sqlx::query_scalar::<_, String>(
...
pub async fn list_routes(
    ...
    let items = crate::routes::repository::get_routes(pool, &ids).await?;
    Ok(ConfigurationPage { items, next_cursor })
}

pub async fn get_routes(pool: &sqlx::PgPool, ids: &[Uuid]) -> Result<Vec<RouteRecord>, Error> {
    if ids.is_empty() {
        return Ok(Vec::new());
    }

    let route_rows = sqlx::query_as::<_, GetRoutesRow>(
    ...
    let mut revisions_map =
        crate::routes::revisions::get_route_revisions(pool, &revision_ids).await?;
...
pub async fn get_route(pool: &sqlx::PgPool, id: Uuid) -> Result<RouteRecord, Error> {
    crate::routes::repository::get_routes(pool, &[id])
        .await?
        .into_iter()
        .next()
        .ok_or(Error::NotFound)
...
async fn draft_targets(
    ...
    Ok(sqlx::query_as::<_, RouteDraftTargetRow>(
...
#[derive(Debug, sqlx::FromRow)]
pub(crate) struct RouteTargetRow {
    pub(crate) id: Uuid,
    pub(crate) routing_id: Uuid,
    pub(crate) provider_model_id: Uuid,
...
async fn replace_draft_targets(
    ...
    let previous_targets = sqlx::query_as::<_, ReplaceDraftTargetsRow>(
    ...
    let active: BTreeSet<Uuid> = sqlx::query_scalar::<_, uuid::Uuid>(
    ...
    .into_iter()
    .collect();
    let mut rows = crate::routes::queries::DraftTargetRows::default();
...
#[derive(sqlx::FromRow)]
struct ListRouteDraftsRow {
    id: uuid::Uuid,
}

#[derive(sqlx::FromRow)]
struct GetRouteDraftsRow {
    id: uuid::Uuid,
    ...
    max_attempts: i16,
    etag: uuid::Uuid,
    based_on_revision_id: Option<uuid::Uuid>,
