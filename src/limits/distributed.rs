...
use crate::limits::admission::LimitBackend;
use crate::limits::admission::LimitError;
use crate::limits::admission::LimitLease as LimitLeasePort;
...
use crate::limits::distributed::script_results::ReservationScriptResult;

const RESERVE_SCRIPT: &str = include_str!("reserve_limits.lua");
const RESERVE_COST_SCRIPT: &str = include_str!("reserve_cost.lua");
...
mod tests {
    ...
    fn stable_keys_share_one_cluster_hash_tag() {
        ...
        let first = limit_keys("olp:3:limits", "lookup_01", api_key_id);
        let second = limit_keys("olp:3:limits", "lookup_02", api_key_id);
    ...
    fn namespaces_cannot_override_the_redis_cluster_hash_tag() {
        assert!(validate_namespace("olp:3:limits").is_ok());
