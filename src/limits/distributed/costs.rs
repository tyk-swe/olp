...
use crate::limits::distributed::LimitKeys;
use crate::limits::distributed::MAX_LUA_INTEGER;
use crate::limits::distributed::RECONCILE_COST_SCRIPT;
use crate::limits::distributed::RESERVE_COST_SCRIPT;
use crate::limits::distributed::SCRIPT_RESPONSE_VERSION;
use crate::limits::distributed::script_results::CostReservationScriptResult;
#[cfg(any(test, feature = "test-util"))]
use crate::limits::distributed::supported_timestamp_ms;
#[cfg(any(test, feature = "test-util"))]
use crate::limits::distributed::validate_cost_limits;
use chrono::DateTime;
use chrono::Utc;
use redis::Script;
...
pub enum CostReconciliationError {
    #[error(transparent)]
    Persistence(#[from] crate::database::error::Error),
    #[error(transparent)]
...
impl DistributedLimiter {
    ...
    pub async fn reconcile_costs(
        ...
        self.reconcile_costs_inner(pool, now, 0).await
    }
    ...
    pub async fn reconcile_costs_at(
        ...
        self.reconcile_costs_inner(pool, now, now_override_ms).await
    }
    ...
    async fn reconcile_costs_inner(
        ...
        let Some(mut leader) = crate::limits::distributed::cost_reconciliation::try_acquire_cost_reconciliation_leader(pool).await? else {
            return Ok(CostReconciliationReport::default());
        };
        leader.reconcile_at(self, now, now_override_ms).await
    }

    pub(crate) async fn apply_cost_snapshots(
        &self,
