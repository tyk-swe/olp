...
pub async fn api_key_budget_status(
    ...
    crate::limits::budgets::api_key_budget_statuses(pool, &[api_key_id], now)
        .await?
        .remove(&api_key_id)
...
pub async fn api_key_budget_statuses(
    ...
    let rows = sqlx::query_as::<_, ApiKeyBudgetStatusesRow>(
        "WITH usage AS ( \
    ...
    .bind(api_key_ids)
    .bind(windows.daily_start)
    .bind(windows.monthly_start)
    .bind(windows.monthly_end)
    .bind(windows.daily_end)
    .fetch_all(pool)
    .await?;
    rows.into_iter()
        .map(|row| {
...
pub(crate) async fn cost_reconciliation_snapshots_on(
    ...
    let windows = budget_windows(now)?;
    let rows = sqlx::query_as::<_, CostCounterRow>(
        "WITH active_keys AS ( \
    ...
    .bind(windows.monthly_start)
    .bind(windows.monthly_end)
    .bind(windows.daily_id)
    .bind(windows.monthly_id)
    .bind(windows.daily_end)
...
pub(crate) async fn add_cost_delta_on(
    ...
    let row = sqlx::query_as::<_, CostCounterRow>(
...
#[derive(sqlx::FromRow)]
struct ApiKeyBudgetStatusesRow {
    api_key_id: uuid::Uuid,
    daily_accrued: String,
    monthly_accrued: String,
    unpriced_attempts: i64,
}
