...
use crate::crypto::Error as SecurityError;
use crate::crypto::aad::credential;
use crate::crypto::aad::idempotency_replay;
use crate::crypto::aad::oidc_client_secret;
use crate::crypto::aad::oidc_flow_payload;
...
pub async fn master_key_encryption_status(
    ...
    let rows = sqlx::query_as::<_, MasterKeyEncryptionStatusRow>(
    ...
    for row in rows {
        ...
        let table = parse_table(&table_name).ok_or_else(|| ReencryptionError::CorruptEnvelope {
    ...
    for table in EncryptedTable::ALL {
        ...
        let updated = match table {
            EncryptedTable::ProviderCredentialVersions => {
                crate::crypto::rotation::reencrypt_provider_credentials(pool, master_key, remaining)
                    .await?
            }
            EncryptedTable::OidcConfigurations => {
                crate::crypto::rotation::reencrypt_oidc_configurations(pool, master_key, remaining)
                    .await?
            }
            EncryptedTable::OidcAuthorizationFlows => {
                crate::crypto::rotation::reencrypt_oidc_flows(pool, master_key, remaining).await?
            }
            EncryptedTable::IdempotencyRecords => {
                crate::crypto::rotation::reencrypt_idempotency_records(pool, master_key, remaining)
                    .await?
            }
        };
        if updated > 0 {
...
pub async fn verify_master_key_envelopes(
    ...
    for table in EncryptedTable::ALL {
        let verified =
            crate::crypto::rotation::verify_encrypted_table(pool, master_key, table, batch_size)
                .await?;
...
pub async fn verify_master_key_retirement(
    ...
    let status =
        crate::crypto::rotation::master_key_encryption_status(pool, master_key.version()).await?;
    ...
    let verified =
        crate::crypto::rotation::verify_master_key_envelopes(pool, master_key, batch_size).await?;
    ...
    let final_references =
        crate::crypto::rotation::master_key_encryption_status(pool, master_key.version())
            .await?
...
pub(crate) async fn reencrypt_provider_credentials(
    pool: &sqlx::PgPool,
    master_key: &MasterKey,
    limit: u64,
) -> Result<u64, ReencryptionError> {
    let mut transaction = pool.begin().await?;
    let rows = sqlx::query_as::<_, ReencryptProviderCredentialsRow>(
...
pub(crate) async fn reencrypt_oidc_configurations(
    pool: &sqlx::PgPool,
    master_key: &MasterKey,
    limit: u64,
) -> Result<u64, ReencryptionError> {
    let mut transaction = pool.begin().await?;
    let rows = sqlx::query_as::<_, ReencryptOidcConfigurationsRow>(
...
pub(crate) async fn reencrypt_oidc_flows(
    pool: &sqlx::PgPool,
    master_key: &MasterKey,
    limit: u64,
) -> Result<u64, ReencryptionError> {
    let mut transaction = pool.begin().await?;
    let rows = sqlx::query_as::<_, ReencryptOidcFlowsRow>(
...
pub(crate) async fn reencrypt_idempotency_records(
    pool: &sqlx::PgPool,
    master_key: &MasterKey,
    limit: u64,
) -> Result<u64, ReencryptionError> {
    let mut transaction = pool.begin().await?;
    let rows = sqlx::query_as::<_, ReencryptIdempotencyRecordsRow>(
...
pub(crate) async fn verify_encrypted_table(
    pool: &sqlx::PgPool,
    master_key: &MasterKey,
    table: EncryptedTable,
    batch_size: u16,
    ...
    loop {
        let rows =
            crate::crypto::rotation::encrypted_verification_rows(pool, table, cursor, batch_size)
                .await?;
        if rows.is_empty() {
            break;
        }
...
#[derive(sqlx::FromRow)]
struct MasterKeyEncryptionStatusRow {
...
#[derive(sqlx::FromRow)]
struct ReencryptProviderCredentialsRow {
    id: uuid::Uuid,
...
#[derive(sqlx::FromRow)]
struct ReencryptOidcConfigurationsRow {
    id: uuid::Uuid,
...
#[derive(sqlx::FromRow)]
struct ReencryptIdempotencyRecordsRow {
    id: uuid::Uuid,
