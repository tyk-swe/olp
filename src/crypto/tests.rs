...
fn durable_encryption_scopes_have_stable_domain_separated_encodings() {
    ...
    assert_eq!(
        credential(first, second, 3).as_slice(),
        b"olp:3:provider:018f47f2-d154-7c52-8170-7a16f378bb29:credential:018f47f2-d154-7c52-8170-7a16f378bb2a:v3"
    );
    assert_eq!(
        oidc_client_secret(first).as_slice(),
        b"olp:3:oidc:018f47f2-d154-7c52-8170-7a16f378bb29:client-secret"
    );
    assert_eq!(
        oidc_flow_payload(second).as_slice(),
        b"olp:3:oidc-flow:018f47f2-d154-7c52-8170-7a16f378bb2a"
    );

    let scope = idempotency_replay_scope(first, "create-route", "request-key");
    assert_eq!(
        scope,
        format!("olp:3:idempotency:{first}:create-route:request-key")
    );
    assert_eq!(
        idempotency_replay(first, "create-route", "request-key"),
        scope.into_bytes()
    );
}
...
fn passwords_use_argon2id_and_verify() {
    ...
    assert!(!verify("incorrect", &encoded));
}
