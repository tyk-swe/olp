...
const API_KEY_PREFIX: &str = "olp_v3_";
const LOOKUP_BYTES: usize = 6;
const SECRET_BYTES: usize = 32;
const BOOTSTRAP_TOKEN_DOMAIN: &[u8] = b"olp:3:bootstrap-setup-token:v1";
...
impl AuthHmacKey {
    ...
    pub fn public_auth_source_digest(&self, source: &str) -> [u8; 32] {
        self.scoped_digest(b"olp:3:public-auth:source:v1", &[source.as_bytes()])
    }
    ...
    pub fn public_auth_source_target_digest(&self, source: &str, target: &str) -> [u8; 32] {
        self.scoped_digest(
            b"olp:3:public-auth:source-target:v1",
...
pub(crate) fn decode_key(encoded: &str) -> Result<[u8; 32], Error> {
