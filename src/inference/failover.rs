...
use crate::inference::circuit::CircuitPermit;
use crate::inference::error::Error as InferenceError;
use crate::inference::selection::operation_for_provider;
...
use crate::inference::failover::streams::canonical_error_circuit_class;
use crate::inference::failover::streams::canonical_event_protocol_error;
use crate::inference::failover::streams::circuit_accounted_event_stream_with_permit;
use crate::inference::failover::streams::validated_event_stream;

pub(crate) struct ExecutionSuccess {
    pub output: ExecutionOutput,
    pub deadline: tokio::time::Instant,
    pub attempts: Vec<RequestAttemptMetadata>,
    pub attempt_started: tokio::time::Instant,
    pub(crate) attempt_trace: Option<AttemptTrace>,
}
...
pub(crate) type AttemptStartedObserver<'a> = dyn FnMut(&[RequestAttemptMetadata], &AttemptPlan, u16, chrono::DateTime<Utc>, tokio::time::Instant)
    + Send
    + 'a;
...
async fn execute_provider_output(
    transport: &Arc<dyn crate::inference::transport::ProviderTransport>,
...
pub(crate) fn reclassify_ambiguous_transport_failure(
    ...
    if operation_is_side_effecting(operation)
        ...
        && !matches!(
            error.phase,
            crate::inference::transport::TransportPhase::Connect
        )
    {
...
pub(crate) const fn attempt_failure_name(class: AttemptFailureClass) -> &'static str {
    match class {
