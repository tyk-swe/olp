...
use crate::inference::lifecycle::RequestLifecycle;
use crate::inference::lifecycle::RequestMetadataFinalizer;
...
use crate::limits::admission::release;
...
impl ExecutionContext {
    fn accounting_input(&self) -> RequestContext {
        RequestContext {
...
pub struct RoutedEvents {
    pub first: Event,
    pub events: crate::inference::transport::ProviderEventStream,
...
impl RoutedEvents {
    #[must_use]
    pub fn into_parts(self) -> (RoutedStream, RequestLifecycle) {
        (
            RoutedStream {
                first: self.first,
                events: self.events,
...
impl Executor {
    ...
    async fn execute_reconciliation_attempts(
        ...
        context: &ExecutionContext,
        operation: Operation,
        attempts: Vec<crate::routes::selection::AttemptPlan>,
        accounting: &mut RequestLifecycle,
    ) -> Result<ExecutionSuccess, crate::inference::failover::ExecutionFailure> {
        let route = runtime
            .routes
            ...
            .expect("attempt selection returned a known route");
        {
            let mut record_attempt_started =
                |completed: &[RequestAttemptMetadata],
                 attempt: &crate::routes::selection::AttemptPlan,
                 ordinal: u16,
    ...
    async fn run_operation_attempts(
        ...
        mode: TransportMode,
        attempts: Vec<crate::routes::selection::AttemptPlan>,
        mut accounting: RequestLifecycle,
    ...
    async fn execute_operation(
        ...
        let accounting = RequestLifecycle::new(
            self.request_metadata.clone(),
            context.accounting_input(),
            lease,
            admission.reservation.take(),
    ...
    pub async fn reserve_model_limits(
        ...
        let operation = Operation::Models(
            crate::protocols::canonical::requests::ModelOperation::List {
