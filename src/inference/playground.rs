...
use crate::protocols::canonical::requests::ToolCall;
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 403, description = "CSRF, origin, or role check failed", body = Problem, content_type = "application/problem+json"),
        (status = 422, description = "Request or route is invalid", body = Problem, content_type = "application/problem+json"),
        (status = 502, description = "Provider response is invalid", body = Problem, content_type = "application/problem+json"),
        (status = 503, description = "No healthy eligible target", body = Problem, content_type = "application/problem+json")
    ),
    security(("sessionCookie" = [], "csrfToken" = [])))]
async fn execute_playground(
    State(state): State<ManagementState>,
    trace: Option<Extension<crate::inference::tracing::RequestTrace>>,
...
fn validate_playground_request(
    ...
    let route = match RouteSlug::parse(request.model.trim().to_owned()) {
        Ok(route) => Some(route),
        Err(error) => {
            fields.insert("model".to_owned(), vec![error.to_string().into()]);
            None
        }
    };
    if request.input.trim().is_empty() || request.input.len() > 128 * 1024 {
        fields.insert(
            "input".to_owned(),
            vec!["Input must contain 1-131072 bytes.".to_owned().into()],
        );
    }
    if request.tools.len() > 64 {
        fields.insert(
            "tools".to_owned(),
            vec!["At most 64 tools may be tested at once.".to_owned().into()],
        );
    }
    if request
        .temperature
        .is_some_and(|value| !value.is_finite() || !(0.0..=2.0).contains(&value))
    {
        fields.insert(
            ...
            vec!["Temperature must be from 0 through 2.".to_owned().into()],
        );
    }
    if request
        .max_output_tokens
        .is_some_and(|value| value == 0 || value > 1_000_000)
    {
        fields.insert(
            ...
            vec![
                "Maximum output tokens must be from 1 through 1000000."
                    .to_owned()
                    .into(),
            ],
        );
    ...
    let surface = match request.surface.parse::<Surface>() {
        ...
        Err(_) => {
            fields.insert(
                "surface".to_owned(),
                vec![
                    "Surface must be openai, anthropic, or gemini."
                        .to_owned()
                        .into(),
                ],
            );
...
-> utoipa_axum::router::OpenApiRouter<crate::http::control::state::ManagementState> {
    utoipa_axum::router::OpenApiRouter::new().routes(utoipa_axum::routes!(
        crate::inference::playground::execute_playground
    ))
}

#[cfg(test)]
mod tests {
    use crate::inference::playground::*;
