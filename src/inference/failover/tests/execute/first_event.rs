...
use crate::inference::failover::tests::execute::*;
...
) -> (Arc<Bundle>, Vec<crate::routes::selection::AttemptPlan>) {
    let manager = Manager::empty();
    let mut providers = BTreeMap::new();
    let mut transports = BTreeMap::new();
    let mut attempts = Vec::new();
    for (transport, timeout) in [(first, 20), (second, 100)] {
        let mut attempt = crate::inference::failover::tests::plan(TargetId::new());
