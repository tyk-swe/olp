...
use crate::protocols::openai::media::BoundedMediaPart;
use axum::extract::Multipart;
use serde_json::Value;

use crate::http::request_admission::multipart::MultipartRequestAdmission;
use crate::http::request_admission::multipart::MultipartRouteAdmission;
use crate::inference::http::state::GatewayState;

use crate::http::streaming_response::channel_stream;
...
pub(crate) struct MultipartFormData {
    ...
    pub(crate) cleanup_handles: Vec<MediaHandle>,
    cleanup_armed: bool,
...
impl MultipartFormData {
    ...
    async fn cleanup(&mut self) {
        ...
        while let Some(handle) = self.cleanup_handles.last().cloned() {
            match self.cleanup_spool.remove(&handle).await {
                Ok(()) | Err(crate::inference::transport::MediaSpoolError::NotFound) => {
    ...
    pub(crate) fn required(&mut self, name: &str) -> Result<String, InferenceError> {
        self.optional(name)?.ok_or_else(|| {
            InferenceError::invalid_request(format!("The {name} field is required."))
        })
    }

    pub(crate) fn optional(&mut self, name: &str) -> Result<Option<String>, InferenceError> {
    ...
    pub(crate) fn optional_parse<T>(&mut self, name: &str) -> Result<Option<T>, InferenceError>
    where
        T: std::str::FromStr,
    {
        self.optional(name)?
    ...
    pub(crate) fn take_repeated(&mut self, name: &str) -> Result<Vec<String>, InferenceError> {
    ...
    pub(crate) fn take_files_with_prefix(
        &mut self,
        prefix: &str,
    ...
    pub(crate) fn take_extensions(&mut self) -> Result<BTreeMap<String, Value>, InferenceError> {
...
pub(crate) async fn parse_multipart(
    ...
    let mut output = MultipartFormData::new(
        state.request_boundary.inference.media_spool.clone(),
        admission,
    );
...
async fn store_multipart_file(
    ...
    let put = state.request_boundary.inference.media_spool.put(
        crate::inference::transport::MediaUpload {
        ...
        while let Some(chunk) = field.chunk().await.transpose() {
            match chunk {
                ...
                Err(error) => {
                    let _ = sender
                        .send(Err(
                            crate::inference::transport::MediaSpoolError::Unavailable,
                        ))
                        .await;
                    return Err(InferenceError::invalid_request(format!(
    ...
    if name == "model" {
        match admission {
            ...
            MultipartRouteAdmission::RequireAuthorizedModel(allowed_routes) => {
                let route = crate::ids::RouteSlug::parse(text.as_str()).map_err(|_| {
                    InferenceError::invalid_request(
...
pub(crate) fn media_spool_error(
    error: crate::inference::transport::MediaSpoolError,
) -> InferenceError {
    match error {
        crate::inference::transport::MediaSpoolError::TooLarge { .. } => {
            InferenceError::payload_too_large("media_too_large")
        }
        crate::inference::transport::MediaSpoolError::InvalidFilename
        | crate::inference::transport::MediaSpoolError::InvalidHandle
        | crate::inference::transport::MediaSpoolError::ZeroLimit => {
            InferenceError::invalid_request(error.to_string())
        }
        crate::inference::transport::MediaSpoolError::NotFound
        | crate::inference::transport::MediaSpoolError::Unavailable => {
...
#[cfg(test)]
mod tests {
    use crate::inference::http::multipart::*;
