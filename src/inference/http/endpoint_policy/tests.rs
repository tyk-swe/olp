...
fn unsupported_gemini_actions_are_explicit_and_metadata_free() {
    ...
    let unknown = InferenceEndpoint::classify(&Method::POST, "/v1/future-action")
        .expect("public gateway paths are classified for admission");
    assert_eq!(unknown.capability(), None);
}

#[test]
fn retired_openai_prefix_has_no_endpoint_policy() {
    assert!(
        InferenceEndpoint::classify(&axum::http::Method::POST, "/openai/v1/chat/completions")
            .is_none()
    );
}
