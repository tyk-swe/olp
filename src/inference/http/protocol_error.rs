...
use crate::inference::http::error::insert_retry_after_header;
...
impl ProtocolError {
    pub(crate) fn anthropic(error: InferenceError) -> Self {
        Self {
    ...
    pub(crate) fn gemini(error: InferenceError) -> Self {
        Self {
    ...
    pub(crate) fn invalid(surface: Surface, message: impl Into<String>) -> Self {
        Self {
            surface,
    ...
    pub(crate) fn not_found(surface: Surface, message: impl Into<String>) -> Self {
        Self {
            surface,
    ...
    pub(crate) fn upstream(surface: Surface, message: impl Into<String>) -> Self {
        Self {
            surface,
...
pub(crate) fn problem_response(surface: Surface, problem: Problem) -> Response {
    ...
    match surface {
        Surface::OpenAi => crate::inference::http::error::openai_error_response(
            status,
            &code,
            match status {
...
pub(crate) fn anthropic_error_body(status: StatusCode, message: &str) -> serde_json::Value {
    json!({
        "type": "error",
...
pub(crate) fn gemini_error_body(status: StatusCode, message: &str) -> serde_json::Value {
    json!({
        "error": {
            "code": status.as_u16(),
            "message": message,
...
mod tests {
    ...
    use crate::inference::http::protocol_error::*;
    ...
    async fn a_rejected_provider_credential_is_a_server_fault_on_every_surface() {
        use crate::inference::error::Error as CoreInferenceError;
