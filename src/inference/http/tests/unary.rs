...
async fn unary_openai_route_authenticates_routes_and_encodes() {
    ...
    let (emitter, mut request_metadata) = crate::usage::emitter::Emitter::bounded(4);
    state.replace_request_metadata_for_test(emitter);
    let response = tokio::time::timeout(
        Duration::from_millis(250),
        crate::http::router::gateway_router_for_test(state).oneshot(
...
async fn openai_v1_aliases_route_static_and_dynamic_handlers() {
    ...
    for path in ["/v1/models/default", "/v1/models/default"] {
...
async fn invalid_proxy_key_gets_native_openai_error() {
    ...
    let response = crate::http::router::gateway_router_for_test(state)
        .oneshot(
            Request::post("/v1/chat/completions")
                .header(header::AUTHORIZATION, "Bearer olp_v3_deadbeef0000_bad")
                .header(header::CONTENT_TYPE, "application/json")
...
async fn embeddings_surface_routes_and_encodes_typed_result() {
    ...
    install_result(
        ...
        CanonicalResult::Embeddings(crate::protocols::canonical::results::EmbeddingsResult {
            model: Some("upstream-model".into()),
            data: vec![crate::protocols::canonical::results::EmbeddingVector {
                index: 0,
                values: vec![0.25, -0.5],
            }],
            usage: Some(crate::protocols::canonical::events::Usage {
                input_tokens: 1,
                output_tokens: 0,
                total_tokens: 1,
...
async fn incompatible_unary_result_is_finalized_as_protocol_failure() {
    let (mut state, key) = test_state(false);
    let (emitter, mut request_metadata) = crate::usage::emitter::Emitter::bounded(2);
    state.replace_request_metadata_for_test(emitter);
    install_result(
        &state,
        OperationKind::TokenCount,
        CanonicalResult::ModelList(crate::protocols::canonical::results::ModelListResult {
            models: Vec::new(),
            extensions: crate::protocols::canonical::requests::SourceExtensions::new(
                Surface::OpenAi,
                BTreeMap::new(),
...
async fn selected_openai_unary_surfaces_route_and_encode_native_results() {
    ...
    install_result(
        ...
        CanonicalResult::Moderation(crate::protocols::canonical::results::ModerationResult {
            id: Some("modr-upstream".to_owned()),
            model: Some("omni-moderation-latest".to_owned()),
            results: vec![crate::protocols::canonical::results::ModerationItem {
                flagged: true,
    ...
    let image_result = || {
        CanonicalResult::Images(crate::protocols::canonical::results::ImagesResult {
            created_at: Some(1_800_000_000),
            images: vec![crate::protocols::canonical::results::ImageArtifact {
                source: crate::protocols::canonical::requests::MediaSource::Uri(
    ...
    install_result(
        ...
        CanonicalResult::Transcription(crate::protocols::canonical::results::TranscriptionResult {
...
async fn speech_surface_streams_bounded_spooled_result() {
    ...
    install_result(
        ...
        CanonicalResult::Speech(crate::protocols::canonical::results::SpeechResult {
            audio: artifact,
            extensions: crate::protocols::canonical::requests::SourceExtensions::new(
                Surface::OpenAi,
                BTreeMap::new(),
