...
use crate::protocols::openai::video::OpenAiVideoCreateRequest;
use crate::protocols::openai::video::OpenAiVideoListQuery;
use crate::protocols::openai::video::decode_video_content_with_query;
use crate::protocols::openai::video::decode_video_create;
use crate::protocols::openai::video::decode_video_delete;
use crate::protocols::openai::video::decode_video_get;
use crate::protocols::openai::video::encode_video_delete_response;
use crate::protocols::openai::video::encode_video_list_response;
use crate::protocols::openai::video::encode_video_object;
use axum::Json;
...
async fn complete_video_create(
    ...
    let (result, mut executed) = crate::media::service::creation::complete_video_create(
        &state.media_jobs,
        crate::media::service::creation::VideoCreateAdmission {
...
pub(crate) async fn video_list(
    ...
    let allowed_routes = key
        ...
        .collect::<Vec<_>>();
    let page = crate::media::jobs::queries::media_jobs_after_id(
        &state.request_boundary.pool,
        &MediaJobFilters {
    ...
    let result = crate::protocols::canonical::results::VideoListResult {
...
pub(crate) async fn video_get(
    ...
    let updated = match crate::media::jobs::lifecycle::refresh_media_job(
        &state.request_boundary.pool,
        record.id,
...
pub(crate) async fn video_delete(
    ...
    let record = crate::media::jobs::lifecycle::begin_media_job_deletion(
        &state.request_boundary.pool,
        loaded.id,
    )
    .await
    ...
    if record.lifecycle == MediaJobLifecycle::Deleted {
        let response = encode_video_delete_response(
            &crate::protocols::canonical::results::VideoDeleteResult {
    ...
    let finalized =
        match media_job_deletion_finalized(&state.request_boundary.pool, record.id).await {
...
mod tests {
    ...
    use crate::inference::http::videos::*;
