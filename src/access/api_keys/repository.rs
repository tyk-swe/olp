use crate::access::api_keys::records::ApiKeyMutationResult;
use crate::access::api_keys::records::ApiKeyRecord;
use crate::access::api_keys::records::ApiKeyRotationResult;
...
async fn replace_api_key_associations(
    ...
    for route in allowed_routes {
        sqlx::query("INSERT INTO api_key_route_allowlist (api_key_id, route_slug) VALUES ($1, $2)")
            .bind(id)
            .bind(route.as_str())
            .execute(&mut **transaction)
            .await?;
...
pub async fn list_api_keys(
    ...
    let rows = sqlx::query_as::<_, ListApiKeysRow>(
...
pub async fn get_api_key(pool: &sqlx::PgPool, id: Uuid) -> Result<ApiKeyRecord, Error> {
    ...
    Ok(ApiKeyRecord {
            ...
            created_by: row.created_by,
            created_by_email: row.created_by_email,
            scopes: sqlx::query_scalar::<_, String>("SELECT scope FROM api_key_scopes WHERE api_key_id = $1 ORDER BY scope")
    .bind(id).fetch_all(pool).await?,
            allowed_routes: sqlx::query_scalar::<_, String>("SELECT route_slug FROM api_key_route_allowlist WHERE api_key_id = $1 ORDER BY route_slug")
    .bind(id).fetch_all(pool).await?,
            requests_per_minute: row.requests_per_minute,
            tokens_per_minute: row.tokens_per_minute,
            max_concurrency: row.max_concurrency,
...
pub async fn update_api_key(
    ...
    if updated.rows_affected() != 1 {
        let row = sqlx::query_as::<_, UpdateApiKeyRow>(
...
async fn rotate_api_key_record(
    ...
    .bind(daily_cost_limit_changed)
    .bind(daily_cost_limit)
    .bind(monthly_cost_limit_changed)
    .bind(monthly_cost_limit)
    .bind(etag)
    .bind(id)
    .bind(expected_etag)
    .execute(&mut **transaction)
...
struct ApiKeyDetailRow {
    ...
    daily_cost_limit: Option<rust_decimal::Decimal>,
    monthly_cost_limit: Option<rust_decimal::Decimal>,
