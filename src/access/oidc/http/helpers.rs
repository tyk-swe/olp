...
pub(crate) fn callback_url(state: &ManagementState) -> Result<String, Problem> {
    Ok(state
        .public_origin
        .with_path("/api/v3/oidc/callback")
        .to_string())
}

pub(crate) fn require_master_key(state: &ManagementState) -> Result<&MasterKey, Problem> {
    state
        .master_key
        .as_deref()
...
pub(crate) fn network_policy(state: &ManagementState) -> Policy {
    Policy {
        allow_insecure_test_endpoints: state.oidc_allow_insecure_test_endpoints,
    }
}

pub(crate) fn valid_binding_token(value: &str) -> bool {
    value.len() == 43
        && value
            .bytes()
...
pub(crate) fn oauth_form_component(value: &str) -> String {
...
mod tests {
    ...
    use crate::access::oidc::http::helpers::*;
    ...
    fn callback_and_form_encoding_preserve_origin_and_escape_values() {
        assert_eq!(
            ...
            "https://console.example.test/api/v3/oidc/callback"
        );
