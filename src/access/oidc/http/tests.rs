use crate::access::oidc::http::configuration::OidcConfigurationRequest;
...
use crate::access::oidc::http::callback::matching_login_callback_flow;
use crate::access::oidc::http::claims::bounded_claim;
use crate::access::oidc::http::claims::validate_id_token;
use crate::access::oidc::http::configuration::OidcSecret;
use crate::access::oidc::http::configuration::default_email_claim;
use crate::access::oidc::http::configuration::default_groups_claim;
use crate::access::oidc::http::configuration::default_scopes;
...
fn stateless_login_cookie_is_encrypted_origin_bound_and_state_checked() {
    ...
    let payload = LoginFlowCookiePayload {
        ...
        return_to: crate::http::relative_url::RelativeReturnTo::parse(
...
fn expired_or_tampered_stateless_login_cookie_is_rejected() {
    ...
    payload.expires_at_unix = (Utc::now() + chrono::Duration::minutes(5)).timestamp();
    let encoded = seal_login_flow_cookie(&state, &master_key, &payload).unwrap();
    assert!(consume_login_flow_cookie(&state, &encoded, &callback_state).is_ok());
    for alias in ["v03.", "v+3.", "v2."] {
        let aliased = encoded.replacen("v3.", alias, 1);
...
async fn callback_clears_the_matching_scoped_cookie_when_query_extraction_fails() {
    ...
    let response = crate::http::router::management_router_for_test(state)
        .oneshot(
            axum::http::Request::get(format!(
                "/api/v3/oidc/callback?state={state_value}&code=one&code=two"
            ))
            .header(
                header::COOKIE,
...
fn test_configuration() -> OidcConfiguration {
    OidcConfiguration {
        ...
        client_id: "olp".to_owned(),
        encrypted_client_secret: crate::crypto::envelope::EncryptedSecret {
            key_version: 1,
            nonce: [0; 12],
            ciphertext: vec![0; 16],
        },
