use crate::access::oidc::client::Error;
...
pub(crate) fn invalid_login_flow_cookie() -> Problem {
    Problem::bad_request(
...
pub(crate) fn invalid_callback() -> Problem {
    Problem::bad_request(
...
pub(crate) fn flow_stale() -> Problem {
    Problem::bad_request(
        "oidc_flow_stale",
...
pub(crate) fn authenticated_flow_session_changed() -> Problem {
    Problem::forbidden(
...
pub(crate) fn is_authenticated_flow_session_changed(problem: &Problem) -> bool {
    problem.problem_type.as_ref() == "https://openllmproxy.dev/problems/oidc_flow_session_changed"
}

pub(crate) fn invalid_id_token() -> Problem {
    Problem::unauthorized("The ID token is invalid.")
}

pub(crate) fn field_problem(field: &str, detail: &str) -> Problem {
    Problem::field_validation(field, detail)
}

pub(crate) fn oidc_not_configured() -> Problem {
    Problem::new(
...
pub(crate) fn map_oidc_flow_completion(error: OidcError) -> Problem {
    match error {
...
pub(crate) fn map_discovery_network(error: Error) -> Problem {
...
pub(crate) fn map_token_network(error: Error) -> Problem {
...
#[cfg(test)]
mod tests {
    use crate::access::oidc::http::error::*;
