...
use crate::access::authentication::RecentAuthGrant;
...
use crate::access::authentication::install_recent_authentication;
...
use crate::access::oidc::repository::helpers::AuthenticatedUserRow;
use crate::access::oidc::repository::helpers::authenticated_user_from_row;
use crate::access::oidc::repository::helpers::checked_session_expiry;
use crate::access::oidc::repository::helpers::lock_email;
use crate::access::oidc::repository::helpers::lock_subject;
use crate::access::oidc::repository::helpers::normalize_display_name;
use crate::access::oidc::repository::helpers::normalize_email;
use crate::access::oidc::repository::helpers::require_current_enabled_configuration;
use crate::access::oidc::repository::helpers::validate_subject;
...
pub async fn complete_oidc_login(
    ...
    let linked = sqlx::query_as::<_, LinkedLoginRow>(
    ...
    let (user, security_version) = if let Some(row) = linked {
        crate::access::oidc::repository::identities::sync_linked_oidc_user(
            provenance,
            &mut transaction,
            &input,
            row,
        ...
        crate::access::oidc::repository::identities::provision_oidc_user(
            provenance,
            &mut transaction,
            &input,
            now,
...
async fn sync_linked_oidc_user(
    ...
    if row.oidc_only {
        ...
        if user.role != mapped_role {
            security_version = sqlx::query_scalar::<_, i64>(
...
pub(crate) async fn provision_oidc_user(
    provenance: &crate::database::RequestProvenance,
    transaction: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    input: &CompleteOidcLogin<'_>,
    now: chrono::DateTime<Utc>,
    ...
    let collision: bool = sqlx::query_scalar::<_, bool>(
...
pub async fn complete_oidc_link(
    ...
    .execute(&mut *transaction)
    .await?;
    let security_version: i64 = sqlx::query_scalar::<_, i64>(
...
pub async fn complete_oidc_reauthentication(
    ...
    .await?;
    lock_subject(&mut transaction, input.issuer, input.subject).await?;
    let linked_and_current: bool = sqlx::query_scalar::<_, bool>(
        "SELECT EXISTS ( \
    ...
    if let Some(identity_id) = input.resource_id {
        let target_belongs_to_user: bool = sqlx::query_scalar::<_, bool>("SELECT EXISTS (SELECT 1 FROM oidc_identities WHERE id = $1 AND user_id = $2) AS \"value\"")
    .bind(identity_id)
    .bind(input.user_id)
            .fetch_one(&mut *transaction)
            .await?;
...
pub async fn oidc_identities_for_user(
    ...
    let rows = sqlx::query_as::<_, OidcIdentitiesForUserRow>(
...
pub async fn unlink_oidc_identity(
    ...
    let identities = sqlx::query_as::<_, UnlinkOidcIdentityRow>(
...
async fn lock_oidc_link_authority(
    ...
    input: &CompleteOidcLink<'_>,
    now: chrono::DateTime<Utc>,
) -> Result<crate::access::identity::locks::LockedUser, OidcError> {
    ...
    let initiating_session = sqlx::query_as::<_, UnlinkOidcIdentityRow>(
    ...
    let current_session: bool = sqlx::query_scalar::<_, bool>(
        "SELECT EXISTS ( \
    ...
    let other_identity: bool = sqlx::query_scalar::<_, bool>(
...
#[derive(sqlx::FromRow)]
struct OidcIdentitiesForUserRow {
    id: uuid::Uuid,
    issuer: String,
