...
use crate::access::oidc::repository::types::OidcFlowRecord;
use crate::crypto::session_material::token_digest;
...
pub(crate) fn validated_oidc_flow_actor(
    flow: &NewOidcFlow,
) -> Result<(Uuid, Uuid, i64), OidcError> {
...
pub(crate) async fn authorize_oidc_flow_session(
    ...
    match flow.purpose {
        ...
        OidcFlowPurpose::Reauthenticate => {
            let current: bool = sqlx::query_scalar::<_, bool>(
                "SELECT EXISTS ( \
...
pub async fn create_oidc_flow(
    ...
    let (actor_user_id, actor_session_id, actor_security_version) =
        crate::access::oidc::repository::flows::validated_oidc_flow_actor(&flow)?;
    ...
    crate::access::oidc::repository::flows::authorize_oidc_flow_session(
        provenance,
        &mut transaction,
        &flow,
        actor_user_id,
    ...
    let active_flows: i64 =
        sqlx::query_scalar::<_, i64>("SELECT count(*) AS \"value\" FROM oidc_authorization_flows")
            .fetch_one(&mut *transaction)
            .await?;
    if active_flows >= MAX_ACTIVE_FLOWS {
        return Err(OidcError::FlowCapacity);
    }
    let recent_flows: i64 = sqlx::query_scalar::<_, i64>(
...
pub async fn consume_oidc_login_flow(
    ...
    let consumed: Option<Uuid> = sqlx::query_scalar::<_, uuid::Uuid>(
        "WITH expired AS ( \
...
pub async fn consume_oidc_flow(
    ...
    let mut transaction = pool.begin().await?;
    let row = sqlx::query_as::<_, ConsumeOidcFlowRow>(
...
#[derive(sqlx::FromRow)]
struct ConsumeOidcFlowRow {
    id: uuid::Uuid,
    configuration_id: uuid::Uuid,
    purpose: String,
    actor_user_id: Option<uuid::Uuid>,
    actor_session_id: Option<uuid::Uuid>,
    actor_security_version: Option<i64>,
    recent_auth_purpose: Option<String>,
    recent_auth_resource_id: Option<uuid::Uuid>,
