...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 403, description = "Role cannot view access", body = Problem, content_type = "application/problem+json")
    ),
...
pub(crate) async fn list_users(
    ...
    let (users, next_cursor) =
        crate::access::identity::accounts::list_users(&state.request_boundary.pool, cursor, limit)
            .await
...
pub(crate) async fn get_user(
    ...
    require_permission(&principal, Permission::ReadAccess)?;
    let user = crate::access::identity::accounts::user(&state.request_boundary.pool, user_id)
        .await
        .map_err(map_identity)?
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 409, description = "Last active owner cannot be demoted or deactivated, and no user may change their own role or deactivate themselves", body = Problem, content_type = "application/problem+json"),
        (status = 412, description = "ETag mismatch", body = Problem, content_type = "application/problem+json"),
        (status = 422, description = "Role is invalid", body = Problem, content_type = "application/problem+json")
    ),
...
pub(crate) async fn update_user_role(
    ...
    let user = crate::access::identity::accounts::update_user_access(
...
#[cfg(test)]
mod tests {
    use crate::access::http::users::Role;
    use crate::access::http::users::guard_self_role_change;
