...
pub(crate) fn operation_media_handles(operation: &Operation) -> Vec<MediaHandle> {
    ...
    match operation {
        ...
        Operation::Images(crate::protocols::canonical::requests::ImageOperation::Edit(request)) => {
            handles.extend(request.images.iter().cloned());
            handles.extend(request.mask.iter().cloned());
        }
        Operation::Images(crate::protocols::canonical::requests::ImageOperation::Variation(
            request,
        )) => {
            handles.push(request.image.clone());
        }
        Operation::Transcription(request) => handles.push(request.audio.clone()),
        Operation::Video(crate::protocols::canonical::requests::VideoOperation::Create(
            request,
        )) => {
        ...
        | Operation::Images(crate::protocols::canonical::requests::ImageOperation::Generation(_))
        | Operation::Speech(_)
        | Operation::Video(_)
        | Operation::Models(_) => {}
    }
...
fn capture_content_handles(
    parts: &[crate::protocols::canonical::requests::ContentPart],
    handles: &mut Vec<MediaHandle>,
) {
    for part in parts {
        match part {
            crate::protocols::canonical::requests::ContentPart::Image {
                source: crate::protocols::canonical::requests::MediaSource::Handle(handle),
                ..
            }
            | crate::protocols::canonical::requests::ContentPart::InputAudio {
                media: handle,
                ..
            }
            | crate::protocols::canonical::requests::ContentPart::InputFile {
                media: handle, ..
            } => {
...
impl futures::Stream for CleanupMediaStream {
    type Item = Result<Bytes, crate::inference::transport::MediaSpoolError>;
