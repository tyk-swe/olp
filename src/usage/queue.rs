...
use crate::usage::queue::protocol::AutoClaimPage;
use crate::usage::queue::protocol::StreamEntry;
use crate::usage::queue::protocol::parse_auto_claim_reply;
use crate::usage::queue::protocol::parse_stream_id;
use crate::usage::queue::protocol::parse_xread_reply;

const AUTOCLAIM_SCRIPT: &str = include_str!("claim_request_metadata.lua");
...
async fn drain_own_pending(
    ...
    .await?;
    report_processing_activity(pages.pool, summary, true).await?;
    if summary.shutdown {
        return Ok(None);
    }
...
async fn reclaim_stale_entries(
    ...
    report_deleted_pending_entries(pages.pool, pages.consumer, &page.deleted_ids).await?;
    if !page.entries.is_empty() {
        let reclaimed = u64::try_from(page.entries.len())
            .map_err(|_| Error::InvalidState("reclaimed entry count overflow"))?;
        if let Err(error) =
            crate::observability::workers::report_request_metadata_consumer_activity(
                pages.pool,
...
async fn settle_new_entries(
    ...
    .await?;
    report_processing_activity(pages.pool, summary, false).await?;
    if summary.shutdown {
        return Ok(None);
    }
    if summary.retry {
...
async fn run_request_metadata_consumer_with_policy(
    ...
    loop {
        ...
        if tokio::time::Instant::now() >= health_due {
            checkpoint_request_metadata_consumer_health(pool, &mut *pages.connection, stream)
                .await?;
...
async fn process_entries(
    ...
    for entry in entries {
        ...
        match process_entry(pool, cost_limiter, connection, stream, consumer, entry).await? {
...
async fn process_entry(
    ...
    if entry.deleted_pending_id.is_some() {
        return finish_deleted_pending_marker(pool, connection, stream, consumer, &entry).await;
    }
    let Some(payload) = entry.payload else {
        // Deleted pending entries follow the same loss policy as XAUTOCLAIM.
        error!(stream_id = %entry.id, "request metadata stream event payload is missing");
        let now = Utc::now();
        let result = crate::usage::ingestion::reconciliation::report_request_metadata_gap_once(
            pool,
            Gap {
    ...
    match crate::usage::ingestion::persistence::persist_request_metadata_stream_event(
        pool, &event, &payload,
    )
    .await
    {
...
async fn report_deleted_pending_entries(
    ...
    for id in deleted_ids {
        let now = Utc::now();
        crate::usage::ingestion::reconciliation::report_request_metadata_gap_once(
            pool,
            Gap {
                gateway_instance: consumer.to_owned(),
                event_count: 1,
...
async fn checkpoint_request_metadata_consumer_health(
    ...
    crate::usage::ingestion::delivery_health::report_request_metadata_consumer_health(
        pool,
        pending_events,
        lag_events,
        oldest_pending_at,
