...
#[derive(Clone, Copy)]
pub(crate) enum UsageCountScope {
    Request,
    Provider,
    Model,
    Target,
...
impl UsageCountScope {
    pub(crate) fn for_filters(filters: &Filters) -> Self {
        match (
...
pub(crate) async fn usage_range_coverage(
    pool: &sqlx::PgPool,
    filters: &Filters,
    ...
    let row = query
        ...
        .fetch_one(pool)
        .await?;
    let excluded = crate::database::cursor::checked_u64(
...
pub(crate) fn push_usage_rows_cte(
...
pub(crate) fn validate_usage_range(filters: &Filters) -> Result<(), Error> {
