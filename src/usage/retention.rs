...
pub async fn run_maintenance(pool: &sqlx::PgPool, now: DateTime<Utc>) -> Result<Report, Error> {
    let mut connection = pool.acquire().await?;
    connection.close_on_drop();
    let locked: bool =
        sqlx::query_scalar::<_, bool>("SELECT pg_try_advisory_lock($1) AS \"value\"")
    ...
    let (rollup_rows, usage_rows) = roll_up_attempt_usage(&mut connection, cutoffs.usage).await?;
...
async fn retention_cutoffs(
    ...
    let rows = sqlx::query_as::<_, RetentionCutoffsRow>(
...
async fn attempt_usage_rollup_batch(
    ...
    let batch = sqlx::query_as::<_, AttemptUsageRollupBatchRow>(
        "WITH candidates AS ( \
...
async fn purge_expiring_records(
    ...
    let invitation_rows =
        sqlx::query("DELETE FROM invitations WHERE expires_at <= $1 AND accepted_at IS NULL")
            ...
            .await?
            .rows_affected();
    let idempotency_rows = sqlx::query("DELETE FROM idempotency_records WHERE expires_at <= $1")
        .bind(now)
        .execute(&mut *transaction)
        .await?
        .rows_affected();
    let oidc_flow_rows = sqlx::query("DELETE FROM oidc_authorization_flows WHERE expires_at <= $1")
        .bind(now)
        .execute(&mut *transaction)
        .await?
        .rows_affected()
...
async fn request_metadata_gap_rollup(
    ...
    let batch = sqlx::query_as::<_, RequestMetadataGapRollupRow>(
        "WITH expired AS ( \
...
#[derive(sqlx::FromRow)]
struct RetentionCutoffsRow {
    key: String,
    value: String,
}

#[derive(sqlx::FromRow)]
struct AttemptUsageRollupBatchRow {
    rollup_rows: i64,
    usage_rows: i64,
}

#[derive(sqlx::FromRow)]
struct RequestMetadataGapRollupRow {
    rollup_rows: i64,
    gap_rows: i64,
}
