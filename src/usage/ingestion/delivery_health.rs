...
impl ConsumerStatus {
    ...
    pub fn from_health(health: Option<ConsumerHealth>, now: DateTime<Utc>) -> Self {
        ...
        let age = now
            ...
            .num_seconds()
            .max(0);
        let age_seconds = u64::try_from(age).unwrap_or(u64::MAX);
...
pub async fn report_request_metadata_consumer_health(
    ...
    crate::usage::ingestion::delivery_health::report_request_metadata_consumer_health_sampled_at_inner(pool, pending_events, lag_events, oldest_pending_at, Utc::now())
        .await
}
...
/// [`report_request_metadata_consumer_health`].
pub async fn report_request_metadata_consumer_health_sampled_at(
    ...
    crate::usage::ingestion::delivery_health::report_request_metadata_consumer_health_sampled_at_inner(pool, pending_events, lag_events, oldest_pending_at, checked_at)
        .await
}

pub(crate) async fn report_request_metadata_consumer_health_sampled_at_inner(
    pool: &sqlx::PgPool,
    pending_events: u64,
    lag_events: u64,
    oldest_pending_at: Option<DateTime<Utc>>,
    ...
    let pending_events =
        i64::try_from(pending_events).map_err(|_| PersistenceError::InvalidRequestMetadataGap)?;
    let lag_events =
        i64::try_from(lag_events).map_err(|_| PersistenceError::InvalidRequestMetadataGap)?;
    let mut transaction = pool.begin().await?;
    let database_now = sqlx::query_scalar::<_, chrono::DateTime<chrono::Utc>>(
        "SELECT clock_timestamp() AS \"database_now\"",
    )
    .fetch_one(&mut *transaction)
    .await?;
    let checked_at = checked_at.min(database_now);
    if oldest_pending_at.is_some_and(|oldest| oldest > checked_at + chrono::Duration::minutes(5)) {
        return Err(PersistenceError::InvalidRequestMetadataGap);
    }
    let row = sqlx::query_as::<_, ReportRequestMetadataConsumerHealthSampledAtInnerRow>(
...
pub async fn request_metadata_consumer_status(
    ...
    Ok(ConsumerStatus::from_health(
        crate::usage::ingestion::delivery_health::request_metadata_consumer_health(pool).await?,
        now,
    ))
}
...
mod tests {
    ...
    use crate::usage::ingestion::delivery_health::*;
...
#[derive(sqlx::FromRow)]
struct ReportRequestMetadataConsumerHealthSampledAtInnerRow {
    pending_events: i64,
    lag_events: i64,
