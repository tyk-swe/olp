...
use crate::usage::ingestion::persistence::AttemptChargeStatus;
use crate::usage::ingestion::persistence::PersistedAttemptFact;
...
async fn price_attempt(
    ...
    let pricing = sqlx::query_as::<_, PriceAttemptRow>(
...
pub(super) async fn insert_attempt_usage_fact<'a>(
...
) -> Result<Option<PersistedAttemptFact>, Error> {
    ...
    .bind(event.observed_at)
    .bind(charge_status.as_str())
    .bind(attempt.usage.observed)
    .bind(usage_complete)
    .bind(attempt.usage.input_tokens)
    .bind(attempt.usage.output_tokens)
    .bind(attempt.usage.cached_input_tokens)
...
#[derive(sqlx::FromRow)]
struct PriceAttemptRow {
    pricing_revision_id: Option<uuid::Uuid>,
    currency: Option<String>,
    pricing_complete: bool,
    estimated_cost: Option<rust_decimal::Decimal>,
}
