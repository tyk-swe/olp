...
pub(crate) struct StreamEntry {
    ...
    pub(crate) deleted_pending_id: Option<String>,
}

#[derive(Debug)]
pub(crate) struct AutoClaimPage {
    pub(crate) next_start: String,
    pub(crate) entries: Vec<StreamEntry>,
    pub(crate) deleted_ids: Vec<String>,
}

pub(crate) fn parse_xread_reply(
    reply: Value,
    expected_stream: &str,
    batch_size: usize,
...
pub(crate) fn parse_auto_claim_reply(
    reply: Value,
    batch_size: usize,
    ...
    let deleted = items.pop().expect("XAUTOCLAIM reply length was validated");
...
pub(crate) fn parse_stream_id(id: &str) -> Result<(u64, u64), Error> {
