...
pub(crate) async fn prepare_runtime_mutation(
    ...
    // publications from transactions that pinned their snapshot before the lock.
...
pub(crate) async fn compile_snapshot(
    ...
    for row in sqlx::query_as::<_, CompileSnapshotRow>(
...
async fn compile_routes(
    ...
    let route_rows = sqlx::query_as::<_, CompileRoutesRow>(
    ...
    for row in route_rows {
        ...
        let route = Route {
            ...
            routing_id: RouteId::from_uuid(row.routing_id),
            slug: slug.clone(),
...
fn compile_target(
    ...
    Ok(Target {
        ...
        routing_id: TargetId::from_uuid(routing_id),
...
#[cfg(test)]
mod tests {
    use crate::runtime::publication::compiler::*;
...
#[derive(sqlx::FromRow)]
struct CompileSnapshotRow {
    id: uuid::Uuid,
    revision_id: uuid::Uuid,
    name: String,
    kind: String,
    credential_version_id: Option<uuid::Uuid>,
}
