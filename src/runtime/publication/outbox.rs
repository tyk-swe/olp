...
use crate::observability::workers::WorkerCounterDeltas;
use crate::observability::workers::WorkerTask;
use crate::observability::workers::WorkerTaskCheckpointOutcome;
use crate::observability::workers::checkpoint_worker_task_on;
use crate::observability::workers::increment_worker_counters_on;
use sqlx::PgPool;

use crate::runtime::publication::OutboxRecord;
...
pub async fn acquire_runtime_outbox_leader(
    pool: &sqlx::PgPool,
) -> Result<RuntimeOutboxLeader, Error> {
...
pub async fn try_acquire_runtime_outbox_leader(
    ...
    let contender =
        crate::runtime::publication::outbox::runtime_outbox_leader_contender(pool).await?;
    match contender.try_acquire(pool).await? {
...
pub async fn runtime_outbox_leader_contender(
    ...
    Ok(RuntimeOutboxLeaderContender {
        connection: pool.acquire().await?.detach(),
    })
}

pub(crate) async fn report_runtime_outbox_contention(pool: &sqlx::PgPool) -> Result<(), Error> {
    ...
    .bind(RUNTIME_OUTBOX_STALE_AFTER_SECONDS as f64)
...
pub async fn runtime_outbox_status(pool: &sqlx::PgPool) -> Result<RuntimeOutboxStatus, Error> {
    let backlog = sqlx::query_as::<_, RuntimeOutboxStatusRow>(
    ...
    let state =
        if heartbeat_age_seconds > u64::try_from(RUNTIME_OUTBOX_STALE_AFTER_SECONDS).unwrap_or(0) {
...
impl RuntimeOutboxLeaderContender {
    pub async fn try_acquire(
        ...
        let acquired: bool =
            sqlx::query_scalar::<_, bool>("SELECT pg_try_advisory_lock($1) AS \"acquired\"")
                .bind(OUTBOX_LEADER_LOCK_ID)
                .fetch_one(&mut self.connection)
                .await?;
        if !acquired {
            crate::runtime::publication::outbox::report_runtime_outbox_contention(pool).await?;
...
impl RuntimeOutboxLeader {
    async fn record_acquired(&mut self) -> Result<(), Error> {
        ...
        let previous = sqlx::query_as::<_, RecordAcquiredRow>(
    ...
    pub async fn release(mut self) -> Result<(), Error> {
        ...
        .await?;
        let released =
            sqlx::query_scalar::<_, bool>("SELECT pg_advisory_unlock($1) AS \"released\"")
                .bind(OUTBOX_LEADER_LOCK_ID)
    ...
    pub async fn backend_pid(&mut self) -> Result<i32, Error> {
        Ok(
            sqlx::query_scalar::<_, i32>("SELECT pg_backend_pid() AS \"pid\"")
                .fetch_one(&mut self.connection)
                .await?,
        )
    }
...
struct RuntimeOutboxHealthRow {
    ...
    last_progress_age_seconds: Option<i64>,
}

#[derive(sqlx::FromRow)]
struct RecordAcquiredRow {
    owner_active: bool,
    claimed_rows: i64,
}
