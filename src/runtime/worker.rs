...
use crate::limits::valkey::RuntimeHintPublisher;
...
async fn outbox_loop(
    ...
    let mut contender =
        Some(crate::runtime::publication::outbox::runtime_outbox_leader_contender(&pool).await?);
    let mut leader = loop {
        tokio::select! {
            result = contender
                .take()
...
async fn publish_outbox_record<P: RuntimeHintPublication>(
    ...
    publisher: &mut P,
    shutdown: &mut watch::Receiver<bool>,
    record: &crate::runtime::publication::OutboxRecord,
