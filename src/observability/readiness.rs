...
use crate::observability::workers::WorkerRecoveryCounters;
use crate::observability::workers::WorkerTask;
use crate::observability::workers::WorkerTaskHealthSummary;
use crate::runtime::publication::outbox::RuntimeOutboxState;
use crate::runtime::publication::outbox::RuntimeOutboxStatus;
use crate::usage::ingestion::delivery_health::ConsumerStatus;
use crate::usage::ingestion::reconciliation::EpochHealth;
...
use crate::observability::cache::CachedReadiness;
use crate::observability::cache::attach_snapshot_freshness;
use crate::observability::cache::snapshot_age_seconds;
use crate::observability::cache::snapshot_is_current;
...
pub(crate) async fn live() -> axum::Json<HealthResponse> {
...
pub(crate) async fn collect_readiness(
    state: &ObservabilityState,
) -> Result<HealthResponse, Problem> {
    let generation = state.inference.runtime.active_generation_ordinal();
    ...
    let degraded_media = probe
        ...
        .as_ref()
        .is_some_and(|summary| summary.stale > 0 || summary.failed > 0)
    ...
    let flags = ReadinessFlags {
        ...
        limits: if limits_healthy {
            "ok"
        } else if state.inference.limiter.is_configured() {
            "unavailable"
        } else {
            "not_configured"
        },
...
async fn probe_store(
    ...
    match crate::database::ping(&state.pool).await {
        Ok(()) => {
            let (media, consumer, epochs, outbox, tasks, counters) = tokio::join!(
                ...
                crate::usage::ingestion::reconciliation::request_metadata_gateway_epoch_health(
                    &state.pool,
                ),
                ...
                crate::observability::workers::worker_recovery_counters(&state.pool,),
            );
...
async fn limiter_is_healthy(state: &ObservabilityState) -> bool {
    let limiter = state.inference.limiter.current();
...
fn readiness_response(
    ...
    HealthResponse {
        ...
        media_spool_used_bytes: state.inference.media_spool.used_bytes(),
        media_spool_capacity_bytes: state.inference.media_spool.capacity_bytes(),
    }
}
...
fn request_metadata_consumer_is_current(status: ConsumerStatus) -> bool {
    matches!(
        status.state,
        crate::usage::ingestion::delivery_health::ConsumerState::Healthy
            | crate::usage::ingestion::delivery_health::ConsumerState::Backlogged
    )
}
...
fn expected_worker_tasks(state: &ObservabilityState) -> &'static [WorkerTask] {
    if state.inference.limiter.is_configured() {
        &WorkerTask::ALL
    } else {
...
pub(crate) fn asynchronous_plane_flags(
...
fn asynchronous_plane_state(
    ...
    if current && drained {
        ...
        || matches!(
            consumer.state,
            crate::usage::ingestion::delivery_health::ConsumerState::Unknown
        )
...
pub(crate) fn datetime_age_seconds(
    now: chrono::DateTime<chrono::Utc>,
