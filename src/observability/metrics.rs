...
use crate::limits::admission::LimitDimension;
use crate::media::jobs::MediaReconciliationSummary;
use crate::observability::providers::PrometheusOperationsSummary;
...
use crate::observability::metrics::async_workers::append_async_worker_metrics;

use crate::observability::cache::CachedMetrics;
...
pub(crate) async fn collect_metrics(state: &ObservabilityState) -> String {
    let request_metadata = state
        .inference
        .request_metadata
        .as_ref()
        .map(Emitter::snapshot);
    let limiter_available = state.inference.limiter.current().is_some();
    ...
    append_media_spool_metrics(
        &mut body,
        state.inference.media_spool.capacity_bytes(),
        state.inference.media_spool.used_bytes(),
    );
...
fn append_runtime_and_metadata_gauges(
    ...
    let _ = write!(
        ...
        u8::from(matches!(
            ...
            crate::usage::ingestion::delivery_health::ConsumerState::Stale
        )),
...
fn append_limiter_circuit_and_media_gauges(
    ...
    let _ = write!(
        ...
        state.inference.limiter.fail_open_total(),
        state
            .inference
            .limiter
            .budget_rejections(LimitDimension::DailyCost),
        state
            .inference
            .limiter
            .budget_rejections(LimitDimension::MonthlyCost),
        state.inference.circuits.open_count(),
