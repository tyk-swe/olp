...
pub(crate) async fn increment_worker_counters_on(
    ...
    .bind(metadata_reclaimed)
    .bind(metadata_recovered)
    .bind(metadata_duplicates)
    .bind(metadata_processed)
    .bind(outbox_attempts)
    .bind(outbox_retry_scheduled)
    .bind(outbox_repeated_attempts)
    .bind(outbox_published)
    .bind(outbox_duplicate_publications)
    .bind(outbox_abandoned_ownership)
    .bind(outbox_abandoned_claims)
    .bind(outbox_failed_takeovers)
    .execute(connection)
    .await?;
    Ok(())
}
...
pub async fn worker_recovery_counters(
    pool: &sqlx::PgPool,
) -> Result<WorkerRecoveryCounters, Error> {
    let row = sqlx::query_as::<_, WorkerRecoveryCountersRow>(
    ...
    Ok(WorkerRecoveryCounters {
        ...
        runtime_outbox_abandoned_ownership: checked(row.runtime_outbox_abandoned_ownership_total)?,
...
pub async fn worker_task_health(pool: &sqlx::PgPool) -> Result<WorkerTaskHealthSummary, Error> {
    let rows = sqlx::query_as::<_, WorkerTaskHealthRow>(
    ...
    for task in WorkerTask::ALL {
        ...
        let last_success_age_seconds = row.last_success_age_seconds.map(checked_age).transpose()?;
...
mod tests {
    ...
    use crate::observability::workers::*;
...
#[derive(sqlx::FromRow)]
struct WorkerRecoveryCountersRow {
    request_metadata_reclaimed_total: i64,
    request_metadata_recovered_total: i64,
    request_metadata_duplicates_total: i64,
    request_metadata_processed_total: i64,
    runtime_outbox_attempts_total: i64,
    runtime_outbox_retry_scheduled_total: i64,
    runtime_outbox_repeated_attempts_total: i64,
    runtime_outbox_published_total: i64,
    runtime_outbox_duplicate_publications_total: i64,
    runtime_outbox_abandoned_ownership_total: i64,
    runtime_outbox_abandoned_claims_total: i64,
    runtime_outbox_failed_takeovers_total: i64,
}

#[derive(sqlx::FromRow)]
struct WorkerTaskHealthRow {
    task: String,
