...
impl BoundedSpanProcessor {
    pub(super) fn new(
        exporter: crate::observability::tracing::CountingExporter,
...
impl SpanProcessor for BoundedSpanProcessor {
    ...
    fn on_end(&self, span: SpanData) {
        ...
        if self.spans.try_send(span).is_err() {
            crate::observability::tracing::record_export_drops(1);
        }
    }
...
impl Worker {
    ...
    async fn shutdown_exporter(&mut self) -> OTelSdkResult {
        ...
        if result.is_err() && !self.pending.is_empty() {
            crate::observability::tracing::record_export_drops(
...
mod tests {
    ...
    use crate::observability::tracing::processor::BoundedSpanProcessor;
    ...
    fn full_queue_counts_each_dropped_span_without_blocking() {
        ...
        let before =
            crate::observability::tracing::TRACE_EXPORT_DROPPED_TOTAL.load(Ordering::Relaxed);
