...
pub async fn prometheus_operations_summary(
    ...
    let row = sqlx::query_as::<_, PrometheusOperationsSummaryRow>(
    ...
    Ok(PrometheusOperationsSummary {
        ...
        success_count: checked_u64(row.success_count, "success count")?,
        cancelled_attempt_count: checked_u64(
            row.cancelled_attempt_count,
...
pub async fn provider_health(
    ...
    let rows = sqlx::query_as::<_, ProviderHealthRow>(
    ...
    let items =
        rows.into_iter()
            .map(|row| {
                ...
                let attempt_count = checked_u64(row.attempt_count, "attempt count")?;
                let success_count = checked_u64(row.success_count, "success count")?;
                ...
                Ok(ProviderHealthRecord {
                    ...
                    attempt_count,
                    success_count,
                    rate_limit_count: checked_u64(row.rate_limit_count, "rate-limit count")?,
                    server_error_count: checked_u64(row.server_error_count, "server-error count")?,
...
#[derive(sqlx::FromRow)]
struct PrometheusOperationsSummaryRow {
    request_count: i64,
    success_count: i64,
    p95_latency_ms: Option<f64>,
    p99_latency_ms: Option<f64>,
    cancelled_attempt_count: i64,
}
