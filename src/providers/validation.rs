...
pub fn validate(
    ...
    for field in Field::ALL {
        match (spec.field(field), configuration_value(configuration, field)) {
    ...
    if let (Some(auth), Some(credential_present)) = (auth, credential_present) {
...
fn configuration_value(
    ...
    match field {
        Field::Endpoint => configuration.endpoint.as_deref(),
        Field::CloudRegion => configuration.cloud_region.as_deref(),
        Field::CloudProject => configuration.cloud_project.as_deref(),
        Field::Deployment => configuration.deployment.as_deref(),
        Field::ApiVersion => configuration.api_version.as_deref(),
        Field::Model => configuration.probe_model.as_deref(),
    }
}
...
mod tests {
    ...
    fn validate(input: Configuration<'_>) -> Vec<Violation> {
        let config = crate::providers::configuration::ProviderConfiguration {
            ...
            cloud_region: input.cloud_region.map(str::to_owned),
            cloud_project: input.cloud_project.map(str::to_owned),
            deployment: input.deployment.map(str::to_owned),
            api_version: input.api_version.map(str::to_owned),
            probe_model: input.model.map(str::to_owned),
        };
        super::validate(&config, input.credential_present)
    }
