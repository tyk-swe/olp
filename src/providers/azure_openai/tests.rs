...
async fn spawn_response_sequence(
    ...
    spawn_sequence(
        "",
        responses.into_iter().map(MockResponse::immediate).collect(),
    )
    .await
}
...
fn error_response(status: &str) -> Vec<u8> {
    ...
    status_response(status, "application/json", body)
}
...
async fn returns_typed_embedding_result_on_azure_path() {
    ...
    assert!(matches!(
        *result,
        crate::protocols::canonical::results::CanonicalResult::Embeddings(_)
    ));
    let request = String::from_utf8(captured.await.unwrap()).unwrap();
    assert!(request.starts_with(
