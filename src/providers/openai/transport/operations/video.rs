...
use crate::protocols::canonical::requests::VideoListRequest;
use crate::protocols::canonical::results::CanonicalResult;
use crate::protocols::openai::video::OpenAiVideoDeleteResponse;
use crate::protocols::openai::video::OpenAiVideoListResponse;
use crate::protocols::openai::video::OpenAiVideoObject;
use crate::protocols::openai::video::decode_video_content_body;
use crate::protocols::openai::video::decode_video_delete_response;
use crate::protocols::openai::video::decode_video_list_response;
use crate::protocols::openai::video::decode_video_object;
use crate::protocols::openai::video::encode_video_create;
use crate::protocols::openai::video::encode_video_list;
use ::http::StatusCode;
use ::http::header;
use reqwest::Method;
...
pub(crate) async fn execute(
    ...
    match operation {
        crate::protocols::canonical::requests::VideoOperation::Create(operation) => {
            execute_create(connector, &request, operation).await
        }
        crate::protocols::canonical::requests::VideoOperation::List(operation) => {
            execute_list(connector, &request, operation).await
        }
        crate::protocols::canonical::requests::VideoOperation::Get(operation) => {
            execute_get(connector, &request, operation).await
        }
        crate::protocols::canonical::requests::VideoOperation::Content(operation) => {
            execute_content(connector, &request, operation).await
        }
        crate::protocols::canonical::requests::VideoOperation::Delete(operation) => {
            execute_delete(connector, &request, operation).await
        }
    }
}
...
async fn execute_create(
    ...
    let result =
        decode_video_object(wire).map_err(|error| protocol_decode_error("video create", error))?;
...
async fn execute_list(
    connector: &Connector,
    request: &ProviderRequest,
    operation: &VideoListRequest,
) -> Result<ProviderOutput, TransportError> {
    let wire =
        encode_video_list(operation).map_err(|error| protocol_encode_error("video list", error))?;
...
async fn execute_get(
    ...
    let result =
        decode_video_object(wire).map_err(|error| protocol_decode_error("video get", error))?;
    Ok(ProviderOutput::Result(Box::new(CanonicalResult::VideoJob(
        result,
    ))))
}
...
async fn execute_content(
    ...
    let content_type = response
        ...
        .ok_or_else(|| protocol_body_error("OpenAI video content used an invalid content type"))?
        .to_owned();
    let spool = request.media.as_ref().ok_or_else(|| {
...
async fn execute_delete(
    ...
    if response.status() == StatusCode::NOT_FOUND && reconcile_missing {
        return Ok(ProviderOutput::Result(Box::new(
            CanonicalResult::VideoDelete(crate::protocols::canonical::results::VideoDeleteResult {
