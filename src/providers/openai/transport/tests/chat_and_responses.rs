use crate::providers::openai::transport::media::{hydrate_chat_media, hydrate_responses_media};
use crate::providers::openai::transport::operations::validate_transport_mode;
use crate::providers::openai::transport::tests::*;
use std::sync::Arc;
...
async fn same_protocol_inline_audio_and_file_handles_are_rehydrated() {
    ...
    let marker = crate::protocols::canonical::requests::inline_media_marker(&handle);
    ...
    let mut input: ResponseInput = serde_json::from_value(serde_json::json!([{
        "type":"message","role":"user","content":[{
            "type":"input_file","filename":"brief.pdf","file_data":crate::protocols::canonical::requests::inline_media_marker(&handle)
        }]
    }]))
    .unwrap();
        ...
        serde_json::from_value(serde_json::json!({
            ...
            "input":[{"type":"message","role":"user","content":[
                ...
                {"type":"input_file","filename":"brief.pdf",
                 "file_data":crate::protocols::canonical::requests::inline_media_marker(&handle)}
            ]}]
        }))
        .unwrap();
    let operation =
