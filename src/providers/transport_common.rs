...
pub(crate) fn inject_trace_context(headers: &mut HeaderMap, enabled: bool) {
    if !enabled {
        return;
    }
...
pub(crate) fn inject_current_trace_context(injector: &mut dyn Injector) {
...
pub(crate) fn request_id_header(
...
pub(crate) fn upstream_response_error(
    phase: TransportPhase,
    status: StatusCode,
    headers: &HeaderMap,
    message: impl Into<String>,
...
pub(crate) fn upstream_failure_class(status: StatusCode) -> AttemptFailureClass {
...
pub(crate) async fn read_inline_media(
    marker: &str,
    spool: Option<&std::sync::Arc<dyn crate::inference::transport::MediaSpool>>,
    maximum_bytes: usize,
    ...
    let handle = crate::protocols::canonical::requests::media_handle_from_inline_marker(marker)
...
mod tests {
    ...
    use crate::inference::transport::MediaByteStream;
    ...
    fn retry_after_reads_delta_seconds_and_http_dates() {
        assert_eq!(
            crate::providers::transport_common::parse_retry_after(&headers_with_retry_after("60")),
            Some(Duration::from_secs(60))
        );
        assert_eq!(
            crate::providers::transport_common::parse_retry_after(&headers_with_retry_after(" 0 ")),
            Some(Duration::ZERO)
        );
        // A date already in the past means "retry now", not "unparsable".
        assert_eq!(
            crate::providers::transport_common::parse_retry_after(&headers_with_retry_after(
                "Wed, 21 Oct 2015 07:28:00 GMT"
            )),
            Some(Duration::ZERO)
        );
        let future = crate::providers::transport_common::parse_retry_after(
            &headers_with_retry_after("Fri, 01 Jan 2100 00:00:00 GMT"),
        )
        ...
        assert_eq!(
            crate::providers::transport_common::parse_retry_after(&HeaderMap::new()),
            None
        );
    }
    ...
    fn an_upstream_response_error_keeps_its_status_and_class() {
        for (status, class) in [
            ...
            let error = crate::providers::transport_common::upstream_response_error(
                TransportPhase::FirstByte,
                status,
