...
use crate::providers::queries::LockedProvider;
...
pub(crate) fn transport_changed(current: &LockedProvider, update: &UpdateProvider) -> bool {
    ...
    update.configuration.options != current.configuration.options
        ...
        || trimmed(&update.configuration.cloud_project)
            != current.configuration.cloud_project.as_deref()
        || trimmed(&update.configuration.deployment) != current.configuration.deployment.as_deref()
        || trimmed(&update.configuration.api_version)
            != current.configuration.api_version.as_deref()
        || update.configuration.auth_mode != current.configuration.auth_mode
}
...
pub(crate) fn validate_provider_update(update: &UpdateProvider) -> Result<(), Error> {
    ...
    for value in [
        ...
        &update.configuration.cloud_region,
        &update.configuration.cloud_project,
        &update.configuration.deployment,
        &update.configuration.api_version,
    ]
    .into_iter()
    .flatten()
    {
...
#[cfg(test)]
mod tests {
    use crate::providers::record_validation::*;
    ...
    fn provider_update() -> UpdateProvider {
        UpdateProvider {
            ...
            configuration: crate::providers::configuration::ProviderConfiguration {
                ...
                deployment: None,
                api_version: None,
                auth_mode: crate::providers::types::ProviderAuthMode::ApiKey,
    ...
    fn configuration_validators_reject_implicit_capabilities() {
        ...
        assert!(
            "unknown"
                .parse::<crate::protocols::canonical::identity::Surface>()
                .is_err()
        );
    }
    ...
    fn transport_change_ignores_renames_and_whitespace_but_not_connector_settings() {
        ...
        renamed.configuration.endpoint = Some("  https://api.example.test  ".to_owned());
        assert!(!transport_changed(&current, &renamed));

        let mutators: [fn(&mut UpdateProvider); 6] = [
            |update| update.configuration.endpoint = Some("https://other.example.test".to_owned()),
            |update| update.configuration.cloud_region = Some("eu-west-1".to_owned()),
            |update| update.configuration.cloud_project = Some("project".to_owned()),
            |update| update.configuration.deployment = Some("deployment".to_owned()),
            |update| update.configuration.api_version = Some("2024-06-01".to_owned()),
            |update| {
                update.configuration.auth_mode =
                    crate::providers::types::ProviderAuthMode::ApplicationDefault;
            },
        ];
    ...
    fn provider_and_model_text_limits_are_character_based_and_closed() {
        ...
        update.configuration.endpoint = Some("é".repeat(2_000));
        ...
        let mutators: [fn(&mut UpdateProvider); 3] = [
            ...
            |update: &mut UpdateProvider| {
                update.configuration.cloud_region = Some("x".repeat(2_001))
            },
        ];
        for mutate in mutators {
            let mut candidate = provider_update();
