...
use crate::providers::connectors::certification::CapabilityCertificationEvidence;
...
use crate::providers::validation::CredentialRequirement;
use crate::providers::validation::Field;
use crate::providers::validation::ProviderKindSpec;
use crate::providers::validation::ProviderPresetSpec;
use crate::providers::validation::provider_kind_specs;
use axum::Json;
...
use crate::providers::http::manage::load_provider_detail;
...
pub(crate) struct ProviderModelListResponse {
    pub provider: crate::providers::http::manage::ProviderDetailResponse,
...
#[utoipa::path(
    get,
    path = "/api/v3/providers/{provider_id}/models",
    tag = "providers",
    params(
...
pub(crate) async fn list_provider_models(
    ...
    let page = crate::providers::models::list_provider_models(
        &state.request_boundary.pool,
        provider_id,
        cursor,
        limit,
...
fn capability_record(input: CapabilityInput) -> Result<CapabilityRecord, Problem> {
    Ok(CapabilityRecord {
        ...
        source: crate::providers::types::CapabilitySource::Declared,
        certified_at: None,
    })
}
...
#[utoipa::path(
    post,
    path = "/api/v3/providers/{provider_id}/discovery",
    tag = "providers",
...
pub(crate) async fn discover_provider_models(
    ...
    let etag = crate::providers::models::discover_provider_models(
        pool,
        &provenance,
        provider_id,
        if_match(&headers)?,
...
#[utoipa::path(
    patch,
    path = "/api/v3/providers/{provider_id}/models/{model_id}",
    tag = "providers",
    params(("provider_id" = Uuid, Path), ("model_id" = Uuid, Path), ("If-Match" = String, Header)),
    request_body = SetModelRequest,
    responses((status = 200, body = ProviderDetailResponse), (status = 412, body = Problem, content_type = "application/problem+json"))
, security(("sessionCookie" = [], "csrfToken" = [])))]
pub(crate) async fn set_provider_model(
    ...
    let request = json_payload(payload)?;
    let pool = &state.request_boundary.pool;
    let etag = crate::providers::models::set_provider_model_enabled(
        pool,
        &provenance,
        provider_id,
        model_id,
...
#[utoipa::path(
    post,
    path = "/api/v3/providers/{provider_id}/models/{model_id}/certify",
    tag = "providers",
    params(
        ("provider_id" = Uuid, Path),
        ("model_id" = Uuid, Path),
    ...
    responses(
        ...
        (status = 409, description = "Provider is active", body = Problem, content_type = "application/problem+json"),
        (status = 412, description = "Provider or reviewed capabilities changed", body = Problem, content_type = "application/problem+json"),
        (status = 422, description = "Provider or capability set cannot be certified", body = Problem, content_type = "application/problem+json")
    ),
    security(("sessionCookie" = [], "csrfToken" = [])))]
pub(crate) async fn certify_provider_model(
    ...
    if provider.state != crate::providers::types::ProviderState::Draft {
        return Err(map_configuration(Error::InUse));
    }
    let model = crate::providers::models::get_provider_model(pool, provider_id, model_id)
        .await
        .map_err(map_configuration)?;
    ...
    let outcomes = results
        ...
        .collect::<Result<Vec<_>, _>>()?;
    let applied = crate::providers::models::apply_compatible_capability_certification(
        pool,
        &provenance,
        provider_id,
        model_id,
...
const fn transport_failure_code(
    class: crate::inference::transport::AttemptFailureClass,
) -> &'static str {
    match class {
        crate::inference::transport::AttemptFailureClass::Connect => "connect_failed",
        crate::inference::transport::AttemptFailureClass::Timeout => "timeout",
        crate::inference::transport::AttemptFailureClass::RateLimit => "rate_limited",
        crate::inference::transport::AttemptFailureClass::UpstreamServer => "upstream_server_error",
        crate::inference::transport::AttemptFailureClass::UpstreamClient => {
            "upstream_rejected_probe"
        }
        crate::inference::transport::AttemptFailureClass::Protocol => "protocol_mismatch",
        crate::inference::transport::AttemptFailureClass::Cancelled => "cancelled",
        crate::inference::transport::AttemptFailureClass::Ambiguous => "ambiguous_result",
    }
}

#[cfg(test)]
mod tests {
    use crate::providers::http::models::*;
    ...
    fn discovery_imports_stay_within_the_revision_diff_ceiling() {
        ...
        assert!(
            problem.errors.get("models").unwrap()[0]
                .message
                .contains(&DISCOVERY_MODEL_LIMIT.to_string())
        );
...
#[cfg(test)]
pub mod provider_kind_catalog_tests;
