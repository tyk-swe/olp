...
fn pricing_provider_kind_uses_current_wire_names_only() {
    assert_eq!(
        serde_json::to_value(crate::providers::runtime_model::ProviderKind::OpenAi).unwrap(),
        "openai"
    );
    assert_eq!(
        serde_json::to_value(crate::providers::runtime_model::ProviderKind::AzureOpenAi).unwrap(),
        "azure_openai"
    );
    assert_eq!(
        serde_json::to_value(crate::providers::runtime_model::ProviderKind::OpenAiCompatible)
            .unwrap(),
        "openai_compatible"
    );
    for legacy in ["open_ai", "azure_open_ai", "open_ai_compatible"] {
        assert!(
            serde_json::from_value::<crate::providers::runtime_model::ProviderKind>(legacy.into())
                .is_err(),
...
fn usage_series_and_breakdown_publish_flat_query_parameters() {
    ...
    for (path, endpoint_parameter) in [
        ("/api/v3/usage/time-series", "granularity"),
        ("/api/v3/usage/breakdown", "dimension"),
    ] {
...
fn request_metadata_gateway_epochs_have_their_own_api_namespace() {
    ...
    assert!(
        document["paths"]
            .get("/api/v3/request-metadata/gateway-epochs")
            .is_some()
    );
    assert!(
        document["paths"]
            .get("/api/v3/usage/gateway-epochs")
            .is_none()
    );
}
