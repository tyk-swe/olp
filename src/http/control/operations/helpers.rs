...
pub(crate) fn query_parameters<T>(query: Result<Query<T>, QueryRejection>) -> Result<T, Problem> {
...
pub(crate) fn optional_filter(value: Option<&String>) -> Option<String> {
    value
...
pub(crate) fn timestamp_cursor(value: Option<&str>) -> Result<Option<Timestamp>, Problem> {
    value
        .map(Timestamp::parse)
        .transpose()
...
pub(crate) fn not_found() -> Problem {
    Problem::new(
...
pub(crate) fn map_operations(error: Error) -> Problem {
    match error {
        Error::InvalidCursor => {
