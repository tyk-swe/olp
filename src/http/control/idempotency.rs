...
use crate::database::idempotency::ReplayedMutation;
...
impl<'a> ReplayableMutation<'a> {
    ...
    async fn replayed(&self) -> Result<Option<Response>, Problem> {
        ...
        let replayed = crate::database::idempotency::replayed_mutation(
            &self.state.request_boundary.pool,
            self.actor,
            self.operation,
            &self.key,
            replay,
    ...
    async fn respond_at<T: Serialize>(
        ...
        let response = crate::database::idempotency::Response::json(
            status.as_u16(),
            body,
        ...
        if let Some(replay) = self.replay
            && let Err(error) = crate::database::idempotency::record_mutation_response(
                &self.state.request_boundary.pool,
                self.actor,
                self.operation,
                &self.key,
