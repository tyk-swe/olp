...
use crate::providers::http::models::certification_item;
...
fn compatible_certification_contract_requires_etag_and_reports_evidence() {
    ...
    let action =
        &document["paths"]["/api/v3/providers/{provider_id}/models/{model_id}/certify"]["post"];
    assert!(
        action["parameters"]
            .as_array()
            .unwrap()
...
fn provider_probe_is_connectivity_only_and_etag_bound() {
    let document = serde_json::to_value(crate::http::control::openapi::document()).unwrap();
    let action = &document["paths"]["/api/v3/providers/{provider_id}/probe"]["post"];
...
fn provider_revision_restore_contract_never_exposes_or_restores_credentials() {
    ...
    let action = &document["paths"]["/api/v3/providers/{provider_id}/revisions/{revision_id}/restore-as-draft"]
        ["post"];
...
fn provider_and_revision_model_inventories_are_bounded_pages() {
    ...
    let inventory_parameters = document["paths"]["/api/v3/provider-models"]["get"]["parameters"]
        .as_array()
        .unwrap();
    assert!(
...
fn provider_revision_diff_contract_documents_hard_response_ceilings() {
    let document = serde_json::to_value(crate::http::control::openapi::document()).unwrap();
    let action = &document["paths"]["/api/v3/providers/{provider_id}/revisions/diff"]["get"];
...
fn api_key_budget_contract_is_nested_exact_and_rotation_body_is_optional() {
    ...
    let rotate = &document["paths"]["/api/v3/api-keys/{api_key_id}/rotate"]["post"];
