...
use crate::limits::admission::LimitRequest;
use crate::limits::admission::Reservation;
use crate::limits::admission::outage_reservation;
use crate::protocols::canonical::identity::Surface;
use crate::protocols::openai::embeddings::EmbeddingWireInput;
use axum::body::Body;
...
pub(crate) fn authenticate_inference_headers(
    state: &RequestBoundaryState,
    headers: &HeaderMap,
    surface: Surface,
    gateway_capability: Option<GatewayCapability>,
) -> Result<Principal, crate::http::problem::Problem> {
    let token = native_inference_token(headers, surface).ok_or_else(|| {
    ...
    {
        return Err(crate::http::problem::Problem::unauthorized(
...
fn reservation_route_timeout(
    routes: &std::collections::BTreeMap<RouteSlug, crate::routes::model::Route>,
...
pub(crate) async fn reserve_http_inference_limits(
    state: &RequestBoundaryState,
    principal: &Principal,
    route: Option<&crate::ids::RouteSlug>,
    ...
    {
        return Err(crate::inference::error::Error::request_exceeds_token_limit(
            requested_tokens,
            limit,
        )
        .into());
...
pub(crate) const fn estimate_http_non_json_request_tokens(category: TokenEstimate) -> i64 {
    match category {
...
mod tests {
    ...
    use crate::http::request_admission::limits::*;
