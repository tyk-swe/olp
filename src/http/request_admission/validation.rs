...
use crate::http::body_limits::MAX_HTTP_HEADER_BYTES;
use crate::http::body_limits::MAX_HTTP_HEADER_COUNT;
...
pub(crate) fn validate_body_framing_and_encoding(
...
pub(crate) fn content_encoding_unsupported() -> Problem {
    Problem::new(
...
pub(crate) struct BodyAdmission {
    maximum: usize,
    pub(crate) multipart_content_type: Option<String>,
...
impl BodyAdmission {
    ...
    pub(crate) fn enforce_declared_size(&self, request: &Request<Body>) -> Result<(), Problem> {
        if request
            .headers()
...
pub(crate) fn request_body_timeout() -> Problem {
    Problem::new(
...
pub(crate) fn payload_too_large(maximum: usize) -> Problem {
    Problem::new(
...
mod tests {
    ...
    use crate::http::request_admission::validation::*;
    ...
    fn body_admission_classifies_media_types_and_declared_size() {
        ...
        let endpoint =
            InferenceEndpoint::classify(&Method::POST, "/v1/audio/transcriptions").unwrap();
