...
use crate::http::request_admission::limits::authenticate_inference_headers;
use crate::http::request_admission::limits::estimate_http_json_request_tokens;
use crate::http::request_admission::limits::estimate_http_non_json_request_tokens;
use crate::http::request_admission::limits::reserve_http_inference_limits;
use crate::http::request_admission::multipart::MultipartRequestAdmission;
use crate::http::request_admission::multipart::preauthorize_multipart;
use crate::http::request_admission::multipart::validate_multipart_boundary;
use crate::http::request_admission::validation::BodyAdmission;
use crate::http::request_admission::validation::ContentEncoding;
use crate::http::request_admission::validation::JsonBodyReadError;
use crate::http::request_admission::validation::content_encoding_unsupported;
use crate::http::request_admission::validation::decompress_gzip_bounded;
use crate::http::request_admission::validation::payload_too_large;
use crate::http::request_admission::validation::read_json_body;
use crate::http::request_admission::validation::request_body_timeout;
use crate::http::request_admission::validation::validate_body_framing_and_encoding;
use crate::http::request_admission::validation::validate_json_depth;
use crate::http::request_admission::validation::validate_target_and_headers;
...
async fn admit_json_request(
    ...
    let requested_route = requested_route
        .as_deref()
        .and_then(|slug| crate::ids::RouteSlug::parse(slug).ok());
...
async fn enforce_request_limits_inner(
    ...
    let local_metadata = principal.as_ref().and_then(|principal| {
        metadata_policy.map(|metadata| LocalRequestMetadata {
            request_metadata: state.inference.request_metadata.as_ref().cloned(),
            request_started_at,
...
async fn preauthorize_first_owner_setup(
    ...
    let pool = &state.pool;
    if !crate::access::identity::setup::setup_required(pool)
        .await
    ...
    let supplied_token = headers
        .get(crate::access::sessions::SETUP_TOKEN_HEADER)
    ...
    crate::access::sessions::enforce_origin(&state.public_origin, headers)?;
