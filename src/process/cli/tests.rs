...
use crate::crypto::rotation::EncryptedTable;
use crate::crypto::rotation::KeyVersionReference;
...
use crate::process::cli::lifecycle::wait_for_shutdown;
...
use crate::usage::worker::request_metadata_consumer_name_from;
...
#[test]
fn migration_cli_requires_only_postgres() {
    let cli = Cli::try_parse_from(["olp", "migrate", "--database-url", "postgres://example/olp"])
        .unwrap();
    let Command::Migrate(args) = cli.command else {
        panic!("expected migrate command");
    };
    assert_eq!(args.database_url, "postgres://example/olp");
}
...
async fn request_metadata_writer_failure_stops_listeners_and_surfaces_error() {
    ...
    let reporter = tokio::spawn(async move {
        ...
        drain_started_receiver.await.unwrap();
        status_sender
            .send(Err(std::io::Error::other("accounting writer failed").into()))
            .unwrap();
        release_drain.send(true).unwrap();
    });
    ...
    assert_eq!(error.unwrap().to_string(), "accounting writer failed");
    assert!(*background_receiver.borrow());
}
