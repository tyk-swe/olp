...
use crate::process::cli::config::MasterKeyAction;
use crate::process::cli::config::MasterKeyArgs;
use crate::process::cli::validation::connect_database;
use crate::process::cli::validation::ensure_keyring_covers_references;
use crate::process::cli::validation::load_master_key;

pub(crate) async fn master_key_command(args: MasterKeyArgs) -> AppResult<()> {
    ...
    match args.action {
        MasterKeyAction::Status { batch_size } => {
            ...
            let verified = crate::crypto::rotation::verify_master_key_envelopes(
                &pool,
                &master_key,
                batch_size,
            )
        ...
        MasterKeyAction::Reencrypt {
            ...
            reencrypt_master_key(&pool, &master_key, batch_size, dry_run).await?;
        }
        MasterKeyAction::VerifyRetirement {
            ...
            report_master_key_status(&master_key, &status);
            ensure_keyring_covers_references(&master_key, &status)?;
            let verified = crate::crypto::rotation::verify_master_key_retirement(
                &pool,
                &master_key,
                version,
                batch_size,
...
async fn reencrypt_master_key(
    ...
    if dry_run {
        let verified =
            crate::crypto::rotation::verify_master_key_envelopes(pool, master_key, batch_size)
                .await?;
        info!(
            active_version = master_key.version(),
            rows_verified = verified.rows_verified,
    ...
    loop {
        ...
        let batch =
            crate::crypto::rotation::reencrypt_master_key_batch(pool, master_key, batch_size)
                .await?;
