...
use crate::process::cli::AppError;
use crate::process::cli::AppResult;
use crate::process::cli::config::InternalPreStopArgs;

pub(crate) async fn internal_pre_stop(args: InternalPreStopArgs) -> AppResult<()> {
    tokio::time::sleep(Duration::from_secs(args.seconds)).await;
    Ok(())
}

pub(crate) async fn shutdown_reason<Signal>(
    signal: Signal,
...
pub(crate) async fn resolve_request_metadata_writer_error(
...
pub(crate) async fn coordinate_shutdown<Public, Observability, Signal>(
...
pub(crate) async fn stop_background_tasks(mut tasks: Vec<JoinHandle<()>>, timeout: Duration) {
