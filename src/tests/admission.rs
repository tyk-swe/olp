...
fn public_auth_source_policy_is_exact_and_complete() {
    let expected = [
        (axum::http::Method::POST, "/api/v3/setup"),
        (axum::http::Method::POST, "/api/v3/sessions"),
        (axum::http::Method::POST, "/api/v3/invitations/accept"),
        (axum::http::Method::GET, "/api/v3/oidc/login"),
        (axum::http::Method::POST, "/api/v3/oidc/login"),
    ];
    ...
    for (method, path) in [
        (axum::http::Method::GET, "/api/v3/setup"),
        (axum::http::Method::GET, "/api/v3/sessions"),
        (axum::http::Method::HEAD, "/api/v3/oidc/login"),
        (axum::http::Method::OPTIONS, "/api/v3/oidc/login"),
        (axum::http::Method::PUT, "/api/v3/oidc/login"),
        (axum::http::Method::POST, "/api/v3/sessions/"),
        (axum::http::Method::POST, "/api/v3/sessions/current"),
        (axum::http::Method::POST, "/api/v3/sessions-extra"),
        (axum::http::Method::POST, "/prefix/api/v3/sessions"),
        (axum::http::Method::POST, "/api/v3/%73essions"),
        (axum::http::Method::POST, "/api/v3/oidc//login"),
        (axum::http::Method::POST, "/api/v3/oidc/../oidc/login"),
        (axum::http::Method::POST, "/api/v3/profile/reauthenticate"),
        (axum::http::Method::POST, "/api/v3/oidc/link"),
    ] {
        assert_eq!(
    ...
    assert_eq!(
        PublicAuthRoute::classify(&unknown, "/api/v3/sessions"),
        None
    );
}
...
fn public_auth_query_strings_do_not_change_source_policy() {
    let request = Request::get("/api/v3/oidc/login?return_to=%2Fproviders")
        .body(Body::empty())
        .unwrap();
...
fn multipart_reservations_scale_with_media_cap() {
    ...
    assert_eq!(reservation("/v1/images/edits", default), 64 * mib);
    assert_eq!(reservation("/v1/images/variations", default), 55 * mib);
    assert_eq!(reservation("/v1/audio/transcriptions", default), 30 * mib);
    assert_eq!(reservation("/v1/videos", default), 25 * mib);

    let doubled = BodyLimits {
        media_body_bytes: 128 * mib,
        ..default
    };
    assert_eq!(reservation("/v1/images/edits", doubled), 128 * mib);
    assert_eq!(reservation("/v1/images/variations", doubled), 110 * mib);
    assert_eq!(reservation("/v1/audio/transcriptions", doubled), 60 * mib);
    assert_eq!(reservation("/v1/videos", doubled), 50 * mib);
}
...
fn multipart_admission_is_post_only_and_recovers_after_a_parser_drops() {
    assert!(
        InferenceEndpoint::classify(&axum::http::Method::GET, "/v1/videos")
            .unwrap()
            .multipart(BodyLimits::default())
            .is_none()
    );
    assert!(
        InferenceEndpoint::classify(&axum::http::Method::POST, "/v1/videos")
            .unwrap()
            .multipart(BodyLimits::default())
            .is_some()
    );
...
async fn malformed_trusted_proxy_chain_precedes_all_public_auth_json_handling() {
    ...
    for (path, requires_origin) in [("/api/v3/sessions", false), ("/api/v3/oidc/login", true)] {
...
fn local_metadata_detection_is_method_and_surface_exact() {
    assert_eq!(
        InferenceEndpoint::classify(&axum::http::Method::GET, "/v1/models")
            .unwrap()
            .metadata()
...
async fn authenticated_multipart_routes_reject_non_multipart_content_types() {
    ...
    for content_type in [None, Some("application/json")] {
        let mut request = Request::post("/v1/images/edits")
