...
fn replicated_worker_metrics_declare_types_and_render_durable_values() {
    ...
    use crate::observability::workers::WorkerTaskState;
    use crate::observability::workers::WorkerTaskStatus;
    ...
    let consumer = ConsumerStatus::from_health(
        Some(crate::usage::ingestion::delivery_health::ConsumerHealth {
            pending_events: 2,
            lag_events: 3,
...
async fn trace_boundary_marks_authentication_headers_sensitive() {
    let service = ServiceBuilder::new()
        ...
        .service(service_fn(|request: Request<Body>| async move {
            ...
            response.headers_mut().insert(
                HeaderName::from_static(crate::access::sessions::CSRF_HEADER),
    ...
    request.headers_mut().insert(
        HeaderName::from_static(crate::access::sessions::SETUP_TOKEN_HEADER),
    ...
    assert!(
        response
            ...
            .get(HeaderName::from_static(
                crate::access::sessions::CSRF_HEADER
            ))
            .unwrap()
            .is_sensitive(),
...
async fn management_extractor_rejections_are_rfc9457_without_query_reflection() {
    ...
    for (uri, expected_instance) in [
        (
            "/api/v3/providers?limit=not-a-number&secret=must-not-reflect",
            "/api/v3/providers",
        ),
        (
            "/api/v3/providers/not-a-uuid",
            "/api/v3/providers/not-a-uuid",
        ),
    ] {
        let response = app
